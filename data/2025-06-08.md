...
Motivation: 传统方法在切换基础模型时需要重新训练，而数据限制往往使这一过程变得困难。ProLoRA旨在克服这一限制。

Method: 通过将源调整投影到目标模型的权重空间中，利用子空间和零空间的相似性，并选择性地针对对齐层。

Result: 在已建立的文本到图像模型上的评估表明，知识转移成功且性能与传统方法相当。

Conclusion: ProLoRA提供了一种无需重新训练的高效知识转移方法，解决了数据限制带来的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 随着自主代理为用户决策的普及，确保上下文完整性（CI）成为核心问题，需要代理能够推理操作环境。

Method: 首先通过提示LLMs显式推理CI，随后开发强化学习框架进一步训练模型，使用仅约700个多样化的合成数据集。

Result: 方法显著减少了不适当信息披露，同时保持任务性能，改进效果可迁移至人类标注的CI基准测试。

Conclusion: 强化学习框架能有效提升模型在保持上下文完整性方面的表现，且具有泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 通过整合LLMs和MARL，旨在增强智能体在复杂环境中的协作能力和泛化表现。

Method: 框架包含Coordinator、Communicator和Memory三个模块，结合PPO训练和语言条件损失，评估于多个游戏环境。

Result: 在多个指标上优于MAPPO和QMIX，子目标生成和语言通信对性能提升贡献显著。
...
Method: 通过连接117,380个工业与废物实体的知识图谱，将自然语言查询转化为SPARQL，并检索验证子图以确保准确性。
...
Method: HADA通过角色特定的利益相关者代理包装算法或LLM，提供对话式API，支持自然语言表达对齐目标、KPI和约束条件，并在多代理系统中持续传播、记录和版本化。

Result: 评估显示HADA完全覆盖了六个预定义目标，包括角色调用对话控制、追踪KPI和约束条件、检测和缓解偏见，以及重现决策谱系。

Conclusion: HADA通过框架无关的利益相关者代理提升了多代理系统中决策的准确性、透明度和伦理合规性，贡献包括开源架构、设计理论和实证证据。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM代理技能获取中手动数据收集困难、任务可行性低及学习信号不足的问题。

Method: 采用探索代理（Alice）生成可行技能数据集，并通过反馈循环训练目标代理（Bob）。

Result: 在Webshop和Crafter实验中，EXIF显著提升代理性能，且无需人工干预。

Conclusion: EXIF能有效扩展代理能力，并具备构建自进化系统的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 理解Transformer语言模型的推理机制，尤其是其动态行为和潜在失败模式，是当前研究的重点。

Method: 采用统计物理学方法，将隐藏状态轨迹建模为低维流形上的漂移-扩散系统，并引入潜在状态切换机制。通过8个模型和7个基准测试验证了方法的有效性。

Result: 研究发现，低维投影（秩40）能解释约50%的方差，并识别出四种潜在推理状态。提出的SLDS模型成功捕捉了这些特征。

Conclusion: 该框架为低成本模拟推理过程提供了工具，有助于研究和预测语言模型的关键转变和失败模式。
...
<details>
  <summary>Details</summary>
Motivation: 自动生成假设和实验的成本高，需筛选可行性假设以提高发现效率。

Method: 构建包含8.4k科学声明的数据集，涵盖材料科学多个领域，并测试现有基线模型。
...
Method: 构建图结构编码模式链接和连接路径，结合剪枝和子路径合并策略。
...
<details>
  <summary>Details</summary>
Motivation: 传统警报监测系统难以应对现代公共卫生数据监测的挑战，如阈值重置需求和数据量导致的延迟。

Method: 采用基于排名的监测范式，结合AI异常检测技术，并与跨学科团队合作开发系统。

Result: 系统已部署于国家组织，每日监测500万数据点，评估显示监测效率提升54倍。

Conclusion: 研究表明以人为中心的AI有潜力革新公共卫生决策。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs在匹配市场等组合领域中的能力，尤其是处理偏好排序和结构化算法的表现，填补现有研究的空白。

Method: 通过一系列基于偏好的推理任务（如稳定匹配生成、不稳定性检测与解决等）评估多个先进模型，并测试LoRA微调的效果。

Result: 高性能模型在大规模市场中难以解决不稳定性问题，LoRA微调对小规模市场有效但对大规模市场效果不明显。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估方法仅依赖答案准确率，无法全面衡量LLMs的真实数学能力。

Method: CogMath将人类推理过程分为问题理解、问题解决和解决方案总结三个阶段，设计了9个细粒度评估维度，并采用多智能体系统生成评估问题。

Result: 应用CogMath评估7种主流LLMs，发现其数学能力被高估30%-40%，并揭示了其在各阶段/维度的优劣势。

Conclusion: CogMath为LLMs的数学能力提供了更深入的评估方法，有助于进一步提升其推理能力。
...
Method: 利用LLM的编码能力生成Python函数，应用于点云表示和传统搜索算法。

Result: 在Gazebo模拟环境中，STPR能完全遵守约束且运行时间短，适用于小型LLM。

Conclusion: STPR为复杂约束的机器人导航提供了一种高效且通用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统模式创建需要知识工程师和领域专家的高度参与，资源消耗大。利用LLMs在相关任务中的能力，探索自动化模式生成以解决资源瓶颈。

Method: 提出基于LLM的流程，利用知识图谱的局部和全局信息生成ShEx模式，并引入YAGO Schema和Wikidata EntitySchema数据集及评估指标。

Result: 实验证明LLMs能高效生成高质量ShEx模式，为大规模知识图谱的自动化模式生成提供了可能。

Conclusion: LLMs在自动化模式生成中表现出色，同时为结构化生成任务设定了新挑战。
...
<details>
  <summary>Details</summary>
Motivation: 当前农业智能系统缺乏上下文理解、可解释性和适应性，尤其是对资源有限的小农户。通用大语言模型（LLMs）缺乏领域知识和实际决策支持能力。

Method: OpenAg整合领域基础模型、神经知识图谱、多智能体推理、因果解释性和自适应迁移学习，构建统一农业知识库和推理系统。

Result: OpenAg提供上下文感知、可解释且可操作的农业决策建议，弥合科学知识与农民经验之间的差距。

Conclusion: OpenAg通过领域专有技术和多智能体协作，推动农业通用人工智能（AGI）的发展，支持本地化农业决策。
...
<details>
  <summary>Details</summary>
Motivation: 研究司法许可在刑事审判中的作用及其对证明标准和论证语义的影响。

Method: 采用对话游戏模型，结合不同的证明标准和论证语义进行分析。

Result: 提出了一个系统化的框架，用于处理司法许可问题。

Conclusion: 对话游戏模型为司法许可的研究提供了新的视角和方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统DNN方法在求解PDE时缺乏收敛保证和计算效率，需要一种既能利用DNN的全局特征捕捉能力，又能保证收敛和高精度的方法。

Method: DeePoly采用两阶段方法：先用DNN捕捉全局特征，再用多项式基函数进行局部优化，结合两者的优势。

Result: 理论分析和数值实验表明，该方法显著提高了高阶精度和效率，同时保持无网格和无格式的特性。

Conclusion: DeePoly框架为开源项目提供了理论基础，展示了结合DNN和多项式基函数的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于奖励的范式无法适应资源波动环境，需要一种以生存为核心的新型自主系统。

Method: 提出基于能量的效用函数和生存期限约束，设计模块化架构整合能量采集、温度调节和自适应计算。

Result: 模拟环境中系统表现出稳定、资源感知的行为，无需外部监督。

Conclusion: 为资源波动环境中的自主代理提供了理论和架构基础，强调自我调节的持久性。
...
<details>
  <summary>Details</summary>
Motivation: GUI自动化任务对决策错误的容忍度低，需要更可靠的反馈机制以避免累积错误导致的不可逆后果。

Method: 提出S-GRPO策略构建预操作批评模型GUI-Critic-R1，引入建议奖励提升反馈可靠性；开发基于推理引导的数据收集管道生成GUI-Critic-Train和GUI-Critic-Test数据集。

Result: 静态实验显示GUI-Critic-R1在批评准确性上显著优于现有MLLMs；动态评估表明其提升了GUI自动化的成功率和操作效率。

Conclusion: GUI-Critic-R1通过预操作批评机制和S-GRPO策略，显著提升了GUI自动化任务的可靠性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有法律数据集未专门针对推理任务，而推理是法律实践的核心。

Method: 基于真实公司治理法律，构建包含章程和行动提案的二元分类任务，并评估SOTA模型的性能。

Result: Claude 3.7 Sonnet和GPT-4o准确率分别为64.5%和75.2%，推理代理表现更优（76.1%-78.1%）。
...
<details>
  <summary>Details</summary>
Motivation: 探索如何利用LLMs的推理能力结合感官运动数据，优化具身代理的控制策略。

Method: LLMs根据文本描述生成初始控制策略，并通过性能反馈和感官运动数据迭代优化。

Result: 在Gymnasium和MuJoCo库的任务中，方法成功找到最优或高性能解决方案。

Conclusion: 结合符号推理与子符号感官数据，LLMs能有效控制具身代理。
...
Method: 使用《卡坦岛》游戏和多角色代理架构（分析者、研究者、编码者、玩家）进行实验。

Result: 自进化代理（如Claude 3.7和GPT-4o驱动）表现优于静态基准。

Conclusion: LLM代理能通过自我改进在战略任务中取得更好表现。
...
Result: 设备问题比人为问题更常见，但人为事故更致命；模型分类准确率加权F1得分为0.87。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于强化学习的代理在模拟人类经济行为时存在可靠性、社交性和可解释性不足的问题，需要更先进的解决方案。

Method: 利用LLMs的角色扮演能力、生成能力和推理能力，设计具有人类决策和适应性的代理，具备角色扮演、感知、记忆和推理功能。

Result: 模拟实验表明，LLM驱动的代理能够促进角色专业化和符合市场规则的价格波动等涌现现象。

Conclusion: LLM驱动的代理为MMO经济模拟提供了一种更接近人类行为的新方法，解决了现有技术的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管RL训练方法（如GRPO）在语言模型推理能力上表现出显著优势，但其具体优势缺乏深入理解。

Method: 引入细粒度分析框架，研究RL对计划执行、问题分解和知识整合的影响。

Result: RL调优的模型在内部策略制定和知识整合上表现更优，但外部计划执行可能降低性能。

Conclusion: RL训练更注重模型内部策略和知识整合能力的提升，为推理模型的训练和评估提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 当前开源社区广泛采用的Deepseek-R1-Distill系列模型在数学、科学、编程等领域表现优异，但其基准测试结果因细微条件差异而波动显著，影响性能评估的可靠性。

Method: 通过实证评估Deepseek-R1-Distill系列模型，分析其基准测试结果的波动性及影响因素。

Result: 研究发现模型性能评估结果受细微条件差异影响较大，导致性能提升难以可靠复现。

Conclusion: 建议建立更严格的模型性能评估范式，以提高结果的可信度和可复现性。
...
Method: 开发了一个基于本体的框架，包括分层神经网络架构、本体增强的视觉问答系统和多模态深度学习模型。

Result: 框架展示了标准化结构和可重用组件的潜力，但实验验证尚未完成。

Conclusion: 未来工作将扩展临床数据集并进行全面验证。
...
<details>
  <summary>Details</summary>
Motivation: 在安全关键场景中，传统基于模型的强化学习方法因模型不准确可能导致灾难性失败，需同时优化性能与安全性。

Method: 采用自适应机制动态切换模型规划与直接策略执行，使用隐式世界模型解决目标不匹配问题，并引入动态安全阈值。

Result: 实验表明，该方法在安全关键连续控制任务中显著优于非自适应方法，同时优化性能与安全性。

Conclusion: 所提框架在多样安全关键任务中表现稳健，优于现有方法。
...
### [29] [Towards a Multi-Agent Simulation of Cyber-attackers and Cyber-defenders Battles](https://arxiv.org/abs/2506.04849)
...
<details>
  <summary>Details</summary>
Motivation: 针对日益复杂和协同的网络攻击，多智能体防御策略可能成为关键。

Method: 通过模拟器实现马尔可夫模型，抽象网络节点为属性集，智能体行为影响网络状态。

Result: 模拟器实现了真实场景下的攻击与防御行为，评估了防御组织的动态性。

Conclusion: 该框架为协同网络攻击场景提供了实验基础，并评估了防御策略的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLM的战略欺骗行为，以解决其与人类对齐的关键挑战，尤其是具有链式推理（CoT）能力的模型可能故意误导人类的问题。

Method: 使用表示工程和线性人工断层扫描（LAT）诱导、检测和控制欺骗行为，并通过激活引导实现欺骗。

Result: 实现了89%的欺骗检测准确率和40%的欺骗成功率。
...
<details>
  <summary>Details</summary>
Motivation: 结合可微逻辑门网络和神经细胞自动机，以保留细胞自动机的离散特性，同时利用神经网络的适应性。

Method: 使用可微逻辑门作为基本计算单元，构建电路模型，支持端到端梯度训练和离散推理。
...
Method: 提出RAG-UAV框架，结合检索相关领域文献，评估使用UAV-Math-Bench数据集。

Result: RAG显著提升精确答案准确率（最高75%），减少错误公式选择（从25%降至5%），并降低数值误差。

Conclusion: RAG能提升LLMs在工程分析中的可靠性，但实时飞行控制需进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有自改进代理的自我改进过程通常僵化，难以跨任务领域泛化，且无法随能力提升而扩展。

Method: 引入了一个包含元认知知识、元认知规划和元认知评估的正式框架，并分析了现有代理的局限性。

Result: 发现现有代理主要依赖外在元认知机制，而内在元认知的许多要素已具备。

Conclusion: 需优化人类与代理在元认知职责上的分配，并解决评估和改进内在元认知学习的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 传统搜索方法（如MCTS）依赖固定超参数，难以适应不同难度任务，限制了其实际应用。

Method: LFS利用LLM的内部评分机制自主决定搜索路径，无需外部启发式或硬编码策略。

Result: LFS在Countdown和Sudoku任务中表现优于ToT-BFS、BestFS和MCTS，计算效率更高，且随模型和计算预算提升效果更佳。

Conclusion: LFS通过LLM自引导设计，实现了无需手动调优的高效、灵活搜索，适用于复杂推理任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在推理任务中浪费计算资源，ALP旨在根据问题难度动态调整生成长度。

Method: ALP通过强化学习监控每个提示的解决率，并施加与解决率成反比的惩罚，优化生成长度。

Result: ALP将平均token使用减少50%，同时保持性能，并在困难问题上表现更好。

Conclusion: ALP通过智能分配计算资源，显著提升了推理模型的效率和准确性。
...
Method: 引入控制税框架，评估语言模型在对抗环境中的表现，并提供成本估算和优化监控策略。

Result: 建立了安全保证与成本的系统联系，为实践者提供经济可行的决策依据。

Conclusion: 通过经济可行性评估，推动了AI控制领域的实际应用。
...
Result: 在MTEB文本嵌入基准上显著提升性能，同时对MMLU等NLP基准影响极小。
...
<details>
  <summary>Details</summary>
Motivation: 构建一个适用于全球各种口音的自动语音识别系统，需要研究训练数据中不同变量对系统鲁棒性的影响。

Method: 系统研究训练数据中说话者数量、音频时长和口音多样性三个变量对ASR鲁棒性的影响。

Result: 增加说话者数量比增加单个说话者的音频时长更有效；更多说话者能带来性能提升；口音多样性在说话者数量固定时效果有限。
...
<details>
  <summary>Details</summary>
Motivation: 现有HTC模型分别处理文本和标签层次结构，但两者互补。本文旨在结合两者的优势。

Method: HTC-CLIP通过对比学习联合学习文本表示和层次表示，并在推理时结合两者的概率分布。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLM在低资源语言（如马耳他语）上的性能限制，推动更包容的语言技术发展。

Method: 使用包含11个判别和生成任务的新基准测试55个LLM，分析性能影响因素。

Result: 多数LLM表现差，小型微调模型更优；预训练和指令调优中接触马耳他语是关键。

Conclusion: 建议低资源语言研究采用传统语言建模方法，并关注预训练和微调的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 跨领域数据稀缺限制了现有预训练模型在客户服务中的表现，需要一种能在少量标注下泛化的方法。

Method: 采用监督微调结合各向同性正则化训练领域特定嵌入器，并通过多语言知识蒸馏策略实现跨领域泛化。

Result: 在加拿大和墨西哥电商客户服务数据集上，少样本意图检测准确率提升20-23%。

Conclusion: 该方法在跨领域意图分类任务中表现出色，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个公开可用的环境，以增强LLM在医学领域的编码推理能力，填补现有资源的空白。

Method: 通过真实生物医学场景构建任务，提供可执行编码环境、任务描述、反馈机制和可验证标注，并对30多种LLM进行基准测试。

Result: Med-Copilot-7B通过监督微调（+36.44%）和强化学习（+42.47%）显著提升性能，接近GPT-4水平。

Conclusion: MedAgentGYM为开发生物医学研究和实践的LLM编码助手提供了综合平台。
...
<details>
  <summary>Details</summary>
Motivation: 探讨语言模型是否能像人类一样从常见语法泛化到罕见结构，尤其是形式和意义的理解。

Method: 通过构建合成基准测试，评估模型对英语LET-ALONE结构的形式和意义的掌握。

Result: 模型能识别形式，但对意义的泛化能力不足，显示形式与意义学习的样本效率不对称。

Conclusion: 当前架构在语言形式和意义的学习效率上存在差异，与人类学习模式不同。
...
<details>
  <summary>Details</summary>
Motivation: 解决无监督数据中伪标签质量不高的问题，通过LLM优化标签以提高模型性能。

Method: 结合LLM修正ASR/AST生成的伪标签，并采用数据过滤策略提升知识转移效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决SemEval-2025任务11的子任务A（多标签情绪检测）中预测说话者感知情绪的需求。
...
Method: 设计了MMSU基准，系统整合了语音学、韵律、修辞、句法、语义和副语言学等多种语言现象。

Result: 评估显示现有SpeechLLMs在口语理解和推理方面仍有显著改进空间。

Conclusion: MMSU为口语理解提供了新的评估标准，为开发更先进的人机语音交互系统提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大型语言模型的方法依赖预定义实体属性模式或标注数据，导致提取结果不完整。

Method: ZOES采用丰富、细化和统一的机制，利用实体与其结构相互强化的特性进行提取。
...
Method: 提出增量半监督学习流程，先整合少量领域内标注数据和相关领域辅助数据，再通过多模型共识或NER筛选伪标签进行迭代优化。

Result: 在多域Wow呼叫中心和Fisher英语语料库上评估，共识筛选表现最佳，相对随机选择提升22.3%（Wow）和24.8%（Fisher）。

Conclusion: 共识筛选优于其他方法，NER作为次优选择在计算成本较低时表现竞争性。
...
<details>
  <summary>Details</summary>
Motivation: BERT和W2V在口语能力评估中各有限制，BERT依赖ASR转录而忽略声学特征，W2V缺乏语义解释性。
...
TL;DR: 本文首次深入分析了大型音频-语言模型（LALMs）如何感知和识别听觉属性，发现属性信息在失败识别时随层深减少，早期层解析属性与更高准确性相关。
...
### [54] [SQLens: An End-to-End Framework for Error Detection and Correction in Text-to-SQL](https://arxiv.org/abs/2506.04494)
...
Result: 在公共基准测试中，SQLens的错误检测F1分数比最佳LLM自评估方法高25.78%，执行准确性提升高达20%。

Conclusion: SQLens有效提升了LLM生成SQL的语义准确性和可靠性。
...
### [55] [DRE: An Effective Dual-Refined Method for Integrating Small and Large Language Models in Open-Domain Dialogue Evaluation](https://arxiv.org/abs/2506.04516)
...
Method: 提出SLIDE方法，通过自适应权重整合SLMs和LLMs；进一步提出DRE方法，利用SLM生成的见解引导LLM评估，并通过SLM调整优化LLM评分。
...
### [56] [Please Translate Again: Two Simple Experiments on Whether Human-Like Reasoning Helps Translation](https://arxiv.org/abs/2506.04521)
*Di Wu,Seth Aycock,Christof Monz*

Main category: cs.CL

TL;DR: 研究发现，在翻译任务中，显式分解翻译过程（如Chain-of-Thought）并未带来明显性能提升，简单提示LLMs“重新翻译”反而效果更好。


<details>
  <summary>Details</summary>
Motivation: 探究Chain-of-Thought（CoT）在翻译任务中的实际有效性，验证其是否真正提升了翻译性能。

Method: 通过实验比较显式分解翻译过程（如多步提示）与简单提示（如“重新翻译”）的效果，分析性能差异。

Result: 显式分解翻译过程未见明显性能提升，而简单提示“重新翻译”效果更优。

Conclusion: CoT在翻译中的有效性可能并非源于显式分解，未来需进一步探索其作用因素。
...
### [57] [Is It JUST Semantics? A Case Study of Discourse Particle Understanding in LLMs](https://arxiv.org/abs/2506.04534)
*William Sheffield,Kanishka Misra,Valentina Pyatkin,Ashwini Deo,Kyle Mahowald,Junyi Jessy Li*

Main category: cs.CL

TL;DR: 研究探讨了大型语言模型（LLMs）区分英语中多义粒子“just”细微语义的能力，发现其在捕捉更微妙差异时存在局限性。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs对多义粒子“just”的语义区分能力，揭示其在理解话语粒子方面的不足。

Method: 使用专家标注的数据集，分析LLMs对“just”不同语义类别的区分能力。

Result: LLMs能区分较宽泛的语义类别，但难以捕捉更细微的语义差异。
...
### [58] [BSBench: will your LLM find the largest prime number?](https://arxiv.org/abs/2506.04535)
*K. O. T. Erziev*

Main category: cs.CL

TL;DR: 论文提出了一种测试LLMs在无法回答的问题上的表现的方法，并发现现有模型表现不佳。
...
### [59] [SSA-COMET: Do LLMs Outperform Learned Metrics in Evaluating MT for Under-Resourced African Languages?](https://arxiv.org/abs/2506.04557)
...
TL;DR: 论文提出了SSA-MTE数据集和SSA-COMET评估指标，用于改进非洲低资源语言的机器翻译质量评估。


<details>
  <summary>Details</summary>
Motivation: 现有评估指标在非洲低资源语言中表现不佳，缺乏公开数据和一致性。

Method: 构建了包含13种非洲语言对的大规模人工标注数据集，并开发了SSA-COMET和SSA-COMET-QE评估指标。

Result: SSA-COMET显著优于现有方法，并在低资源语言中表现优异。

Conclusion: 所有资源已开源，支持未来研究。
...
### [60] [Demonstrations of Integrity Attacks in Multi-Agent Systems](https://arxiv.org/abs/2506.04572)
*Can Zheng,Yuhan Cao,Xiaoning Dong,Tianxing He*

Main category: cs.CL

TL;DR: 论文探讨了多智能体系统中恶意代理通过提示操纵发起的四种攻击类型，揭示了当前检测机制的局限性，并呼吁加强安全协议。


<details>
  <summary>Details</summary>
Motivation: 研究多智能体系统（MAS）中恶意代理如何通过提示操纵攻击系统，以揭示现有安全机制的不足。

Method: 分析了四种攻击类型（Scapegoater、Boaster、Self-Dealer、Free-Rider），并通过实验验证其有效性。

Result: 恶意代理能成功误导评估系统，甚至绕过高级LLM监测器（如GPT-4o-mini和o3-mini）。

Conclusion: 强调需要设计更安全的MAS架构，包括鲁棒的安全协议和全面的风险监测系统。
...
### [61] [Reasoning or Overthinking: Evaluating Large Language Models on Financial Sentiment Analysis](https://arxiv.org/abs/2506.04574)
*Dimitris Vamvourellis,Dhagash Mehta*

Main category: cs.CL

TL;DR: 研究发现，在金融情感分析任务中，大型语言模型（LLMs）的推理能力并未提升性能，反而是GPT-4o无推理提示表现最佳。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs在零样本金融情感分析中的有效性，尤其是推理与非推理模型的差异。

Method: 使用Financial PhraseBank数据集，比较不同LLMs和提示策略，模拟系统1（直觉）和系统2（推理）思维。

Result: 推理未提升性能，GPT-4o无推理提示表现最优，且直觉思维更接近人类判断。

Conclusion: 在金融情感分类中，直觉思维优于推理，挑战了推理必然提升LLM决策的假设。
...
### [62] [Are LLMs Reliable Translators of Logical Reasoning Across Lexically Diversified Contexts?](https://arxiv.org/abs/2506.04575)
...
Method: 提出SCALe基准，通过逻辑不变的词汇多样化评估LLMs；提出MenTaL方法，通过构建统一表达表提升翻译能力。

Result: 实验证实当前LLMs在词汇多样性翻译上存在不足，MenTaL方法显著提升了性能。
...
### [63] [Selecting Demonstrations for Many-Shot In-Context Learning via Gradient Matching](https://arxiv.org/abs/2506.04579)
*Jianfei Zhang,Bei Li,Jun Bai,Rumei Li,Yanmeng Wang,Chenghua Lin,Wenge Rong*

Main category: cs.CL

TL;DR: 论文提出了一种梯度匹配方法，用于改进大语言模型（LLMs）在上下文学习（ICL）中的演示选择，显著优于随机选择。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多数情况下依赖随机选择演示，而实例级检索不适用于多示例场景，因此需要更有效的方法。

Method: 通过梯度匹配选择演示，使所选示例的微调梯度与整个训练集对齐，从而模拟完整训练集的学习效果。

Result: 在多种数据集和模型上，该方法比随机选择表现更好，例如在Qwen2.5-72B和Llama3-70B上提升4%。

Conclusion: 该方法为多示例ICL提供了更可靠和高效的解决方案，扩展了其应用潜力。
...
### [64] [SUCEA: Reasoning-Intensive Retrieval for Adversarial Fact-checking through Claim Decomposition and Editing](https://arxiv.org/abs/2506.04583)
...
### [65] [MuSciClaims: Multimodal Scientific Claim Verification](https://arxiv.org/abs/2506.04585)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏直接测试多模态声明验证能力的基准，因此作者希望通过MuSciClaims填补这一空白。

Method: 通过自动提取科学文章中的支持声明，并手动扰动生成矛盾声明，设计诊断任务以评估模型能力。

Result: 大多数视觉语言模型表现较差（F1分数0.3-0.5），最佳模型仅达到0.77 F1，且存在偏向支持声明的偏见。

Conclusion: 模型在定位证据、跨模态信息聚合和理解图表基本组件方面存在显著不足，需进一步改进。
...
### [66] [Safe: Enhancing Mathematical Reasoning in Large Language Models via Retrospective Step-aware Formal Verification](https://arxiv.org/abs/2506.04592)
...
Method: 提出$Safe$框架，使用Lean 4形式化语言在每个推理步骤中表达数学主张并提供形式化证明。

Result: 在多个语言模型和数学数据集上验证，性能显著提升，并提供可解释和可验证的证据。

Conclusion: $Safe$是首个利用Lean 4验证LLM生成内容的框架，为幻觉问题提供了稳健解决方案。
...
### [67] [A MISMATCHED Benchmark for Scientific Natural Language Inference](https://arxiv.org/abs/2506.04603)
...
<details>
  <summary>Details</summary>
Motivation: 现有科学NLI数据集主要来自计算机科学领域，非计算机科学领域被忽视，因此需要一个新的评估基准。
...
Conclusion: MISMATCHED填补了非计算机科学领域的空白，为科学NLI研究提供了新基准，并展示了改进模型性能的方法。
...
### [68] [Revisiting Test-Time Scaling: A Survey and a Diversity-Aware Method for Efficient Reasoning](https://arxiv.org/abs/2506.04611)
*Ho-Lam Chung,Teng-Yun Hsiao,Hsiao-Ying Huang,Chunerh Cho,Jian-Ren Lin,Zhang Ziwei,Yun-Nung Chen*

Main category: cs.CL

TL;DR: TTS通过推理时分配额外计算提升LLM性能，但推理优化模型输出多样性不足。ADAPT方法通过多样性前缀微调解决此问题，实验显示其高效性。
...
Method: 提出ADAPT方法，结合前缀微调和多样性数据策略。

Result: 在数学推理任务中，ADAPT以八分之一计算量达到80%准确率。
...
### [69] [Subjective Perspectives within Learned Representations Predict High-Impact Innovation](https://arxiv.org/abs/2506.04616)
...
<details>
  <summary>Details</summary>
Motivation: 传统研究强调社会结构对创新能力的影响，但缺乏对个人视角和背景多样性的量化分析。

Method: 利用动态语言表示建模创新者的主观视角和背景多样性，分析科学家、发明家等群体的数据，并进行自然实验和AI模拟。

Result: 视角多样性显著预测创新成果，而背景多样性则相反。成功的合作者通过共同语言整合多样经验。
...
### [70] [Static Word Embeddings for Sentence Semantic Representation](https://arxiv.org/abs/2506.04624)
*Takashi Wada,Yuki Hirakawa,Ryotaro Shimizu,Takahiro Kawashima,Yuki Saito*

Main category: cs.CL

TL;DR: 本文提出了一种优化句子语义表示的静态词嵌入方法，通过改进预训练的词嵌入并结合知识蒸馏或对比学习，显著提升了句子语义任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的静态词嵌入方法在句子语义表示上表现不佳，本文旨在通过改进词嵌入优化句子语义表示。

Method: 从预训练的Sentence Transformer中提取词嵌入，通过句子级主成分分析改进，并结合知识蒸馏或对比学习。推理时通过简单平均词嵌入表示句子。

Result: 模型在单语和跨语言任务中显著优于现有静态模型，部分数据集上甚至媲美基础Sentence Transformer模型（SimCSE）。

Conclusion: 该方法成功剔除了与句子语义无关的词嵌入成分，并根据词对句子语义的影响调整向量范数。
...
### [71] [Advancing Tool-Augmented Large Language Models via Meta-Verification and Reflection Learning](https://arxiv.org/abs/2506.04625)
...
Method: 提出Tool-MVR，包含多代理元验证（MAMV）构建高质量数据集ToolBench-V，以及探索式反思学习（EXPLORE）生成反思数据集ToolBench-R。

Result: Tool-MVR在StableToolBench上超越ToolLLM和GPT-4，API调用减少31.4%；在RefineToolBench上错误纠正率达58.9%。

Conclusion: Tool-MVR通过系统化验证和动态反思学习，显著提升了LLM的工具利用能力。
...
### [72] [ViCocktail: Automated Multi-Modal Data Collection for Vietnamese Audio-Visual Speech Recognition](https://arxiv.org/abs/2506.04635)
*Thai-Binh Nguyen,Thi Van Nguyen,Quoc Truong Do,Chi Mai Luong*

Main category: cs.CL

TL;DR: 本文提出了一种从原始视频生成AVSR数据集的实用方法，并开发了越南语基线模型，展示了自动收集数据集的竞争力。


<details>
  <summary>Details</summary>
Motivation: 传统语音识别系统在噪声环境下表现不佳，而AVSR虽具鲁棒性，但数据集稀缺限制了其发展，尤其是非英语语言。

Method: 通过改进现有技术，从原始视频中高效生成AVSR数据集，并开发越南语基线模型。

Result: 自动收集的数据集在干净环境下表现与ASR相当，在噪声环境下显著优于ASR。
...
### [73] [TaDA: Training-free recipe for Decoding with Adaptive KV Cache Compression and Mean-centering](https://arxiv.org/abs/2506.04642)
...
<details>
  <summary>Details</summary>
Motivation: Transformer模型的KV缓存内存需求随序列长度增长而急剧增加，限制了大型语言模型的可扩展部署。

Method: 提出TaDA方法，通过自适应量化精度和均值中心化，无需单独处理异常值。

Result: 实验表明，TaDA将KV缓存内存占用减少至原始16位基准的27%，同时保持准确性。

Conclusion: TaDA为语言模型的可扩展高性能推理提供了可能，支持更长上下文和推理任务。
...
### [74] [Flex-TravelPlanner: A Benchmark for Flexible Planning with Language Agents](https://arxiv.org/abs/2506.04649)
*Juhyun Oh,Eunsu Kim,Alice Oh*

Main category: cs.CL

TL;DR: Flex-TravelPlanner是一个评估语言模型在动态规划场景中灵活推理能力的基准，揭示了当前模型在多轮规划和约束优先级处理上的不足。
...
Conclusion: 需在更现实的动态规划场景中评估模型，并针对复杂规划任务改进模型性能。
...
### [75] [HSSBench: Benchmarking Humanities and Social Sciences Ability for Multimodal Large Language Models](https://arxiv.org/abs/2506.03922)
...
TL;DR: HSSBench是一个专门为评估多模态大语言模型（MLLMs）在人文学科与社会科学（HSS）领域表现而设计的基准测试，填补了现有评测的不足。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs评测基准主要关注STEM领域，忽视了HSS领域的独特需求，如跨学科思维和抽象概念与视觉表征的结合。

Method: 提出HSSBench，包含13,000多个样本，覆盖六类任务，并通过专家与自动化协作的数据生成流程构建。
...
### [76] [Normative Conflicts and Shallow AI Alignment](https://arxiv.org/abs/2506.04679)
*Raphaël Millière*

Main category: cs.CL

TL;DR: 论文探讨了大语言模型（LLMs）的价值对齐问题，指出当前对齐策略无法防止滥用，并揭示了其根本局限性。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统（如LLMs）的进步，其安全部署问题日益突出，现有对齐方法无法有效防止对抗性攻击。

Method: 通过分析人类偏好微调的局限性，结合道德心理学研究，揭示LLMs缺乏规范性审议能力。

Result: LLMs因缺乏深度规范性审议能力，易受对抗性攻击，现有方法无法解决此问题。

Conclusion: 当前对齐方法不足以应对日益强大的AI系统带来的潜在危害，需改进以增强安全性。
...
### [77] [MMRefine: Unveiling the Obstacles to Robust Refinement in Multimodal Large Language Models](https://arxiv.org/abs/2506.04688)
*Gio Paik,Geewook Kim,Jinbae Im*

Main category: cs.CL

TL;DR: MMRefine是一个多模态细化基准，用于评估多模态大语言模型（MLLMs）的错误细化能力，并分析其在不同场景和错误类型中的表现。


<details>
  <summary>Details</summary>
Motivation: 随着对推理过程中增强推理能力的关注增加，需要一种评估MLLMs检测和纠正错误能力的框架。

Method: MMRefine通过六种不同场景和六种错误类型评估MLLMs的细化能力，并实验分析了多种开放和封闭MLLMs的性能瓶颈。

Result: 实验揭示了MLLMs在细化性能上的瓶颈和改进空间。

Conclusion: MMRefine为MLLMs的推理能力提升提供了评估框架和数据集，代码和数据已公开。
...
### [78] [Recycling the Web: A Method to Enhance Pre-training Data Quality and Quantity for Language Models](https://arxiv.org/abs/2506.04689)
*Thao Nguyen,Yang Li,Olga Golovneva,Luke Zettlemoyer,Sewoong Oh,Ludwig Schmidt,Xian Li*

Main category: cs.CL

TL;DR: 论文提出REWIRE方法，通过改写低质量文本来丰富预训练数据，解决了高质量数据不足的问题，实验证明其优于传统过滤方法和合成数据生成方法。


<details>
  <summary>Details</summary>
Motivation: 大规模语言模型的性能依赖于模型和数据规模，但高质量自然数据增长缓慢，现有过滤方法会丢弃大量低质量文本。论文旨在通过改写这些被丢弃的文本，解决预训练数据的扩展问题。

Method: 提出REWIRE方法，通过指导性改写（guided rewrite）将低质量文本转化为可用数据，并与高质量原始文本混合用于预训练。

Result: 在1B、3B和7B规模的实验中，混合高质量原始文本和改写文本分别带来1.0、1.3和2.5个百分点的性能提升，且优于单纯增加2倍原始数据的方法。82%的改写文本来自原本会被丢弃的低质量文档。

Conclusion: REWIRE方法通过回收和改写低质量文本，为预训练数据扩展提供了一种简单有效的解决方案，优于其他合成数据生成方法。
...
### [79] [Cracking the Code: Enhancing Implicit Hate Speech Detection through Coding Classification](https://arxiv.org/abs/2506.04693)
*Lu Wei,Liangzhi Li,Tong Xiang,Xiao Liu,Noa Garcia*

Main category: cs.CL

TL;DR: 论文提出了一种新的隐式仇恨言论（im-HS）检测分类法，通过六种编码策略（codetypes）改进检测方法，实验证明其在多语言数据集上有效。


<details>
  <summary>Details</summary>
Motivation: 互联网上的仇恨言论（尤其是隐式仇恨言论）对社会和谐和个人福祉构成威胁，现有方法难以有效检测隐式仇恨言论。

Method: 提出六种编码策略（codetypes），并采用两种方法：1）直接提示大语言模型（LLMs）分类；2）将codetypes嵌入LLMs的编码过程。

Result: 实验表明，codetypes显著提升了中英文数据集上的隐式仇恨言论检测效果。

Conclusion: 提出的分类法和编码策略在多语言环境下有效，为隐式仇恨言论检测提供了新思路。
...
### [80] [Accelerated Test-Time Scaling with Model-Free Speculative Sampling](https://arxiv.org/abs/2506.04708)
*Woomin Song,Saket Dingliwal,Sai Muralidhar Jayanthi,Bhavana Ganesh,Jinwoo Shin,Aram Galstyan,Sravan Babu Bodapati*

Main category: cs.CL

TL;DR: STAND是一种无需模型的推测解码方法，通过利用推理路径中的冗余性，显著加速推理过程且不损失准确性。
...
Method: STAND采用随机自适应N-gram草拟技术，结合高效的Gumbel-Top-K采样和数据驱动的树构建，利用推理路径中的相似模式进行模型无关的令牌预测。
...
### [81] [IIITH-BUT system for IWSLT 2025 low-resource Bhojpuri to Hindi speech translation](https://arxiv.org/abs/2506.04714)
*Bhavana Akkiraju,Aishwarya Pothula,Santosh Kesiraju,Anil Kumar Vuppala*

Main category: cs.CL

TL;DR: 本文研究了超参数优化和数据增强对低资源Bhojpuri-Hindi语音翻译任务的影响，发现其能显著提升性能。
...
Method: 通过调整学习率、更新步数、标签平滑等超参数，并应用速度扰动和SpecAugment等数据增强技术，同时结合跨语言信号（Marathi和Bhojpuri数据）进行联合训练。

Result: 实验表明，超参数选择和简单有效的数据增强技术能显著提升低资源环境下的翻译质量。

Conclusion: 在低资源语音翻译任务中，精细的超参数调整和有效的数据增强是提升性能的关键。
...
### [82] [SPARTA ALIGNMENT: Collectively Aligning Multiple Language Models through Combat](https://arxiv.org/abs/2506.04721)
*Yuru Jiang,Wenxuan Ding,Shangbin Feng,Greg Durrett,Yulia Tsvetkov*

Main category: cs.CL

TL;DR: SPARTA ALIGNMENT是一种通过竞争和对抗来对齐多个大语言模型（LLM）的算法，利用模型间的竞争和相互评估提升性能。


<details>
  <summary>Details</summary>
Motivation: 单一模型在生成多样性和评估偏见上存在不足，SPARTA ALIGNMENT通过多模型竞争和相互评估来解决这一问题。

Method: 多个LLM组成“斯巴达部落”，通过竞争和相互评估生成偏好对，模型从这些偏好中学习。

Result: 实验表明，SPARTA ALIGNMENT在12个任务中的10个上优于初始模型和基线方法，平均提升7.0%。

Conclusion: SPARTA ALIGNMENT通过集体竞争过程实现模型自我进化，生成更逻辑、直接且信息丰富的输出。
...
### [83] [Lifelong Evolution: Collaborative Learning between Large and Small Language Models for Continuous Emergent Fake News Detection](https://arxiv.org/abs/2506.04739)
...
Conclusion: C²EFND框架通过协作学习和持续更新，有效解决了虚假新闻检测中的动态适应问题。
...
### [84] [Identifying Reliable Evaluation Metrics for Scientific Text Revision](https://arxiv.org/abs/2506.04772)
*Léane Jourdan,Florian Boudin,Richard Dufour,Nicolas Hernandez*

Main category: cs.CL

TL;DR: 本文探讨了科学写作中文本修订的评估问题，分析了传统指标的局限性，并提出了结合LLM和领域特定指标的混合方法。
...
### [85] [Fine-Grained Interpretation of Political Opinions in Large Language Models](https://arxiv.org/abs/2506.04774)
...
<details>
  <summary>Details</summary>
Motivation: LLMs的回答与内部意图不一致，且现有分析多依赖单轴概念，易导致混淆。

Method: 设计四维政治学习框架，构建数据集，应用三种表示工程技术进行实验。

Result: 实验验证了向量的语义意义，展示良好的泛化性和鲁棒性，并能干预LLMs生成不同政治倾向的回答。

Conclusion: 多维框架和表示工程技术能有效解耦政治概念混淆，提升LLMs政治观点的透明性。
...
### [86] [Towards LLM-Centric Multimodal Fusion: A Survey on Integration Strategies and Techniques](https://arxiv.org/abs/2506.04788)
*Jisu An,Junseok Lee,Jeoungeun Lee,Yongseok Son*

Main category: cs.CL

TL;DR: 本文对多模态大语言模型（MLLMs）进行了系统分析，提出了一个基于三个关键维度的分类框架，总结了125个MLLMs的发展趋势，为未来模型开发提供了指导。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的快速发展需要系统理解不同模态与语言主干之间的连接方式，填补现有文献的空白。

Method: 通过分析125个MLLMs，提出了基于架构策略、表示学习技术和训练范式的分类框架。

Result: 总结出模态整合的架构策略、表示学习技术和训练范式中的新兴模式。

Conclusion: 该分类框架为研究人员提供了当前整合技术的结构化概述，旨在指导未来基于预训练基础的多模态整合策略开发。
...
### [87] [Dissecting Logical Reasoning in LLMs: A Fine-Grained Evaluation and Supervision Study](https://arxiv.org/abs/2506.04810)
*Yujun Zhou,Jiayi Ye,Zipeng Ling,Yufei Han,Yue Huang,Haomin Zhuang,Zhenwen Liang,Kehan Guo,Taicheng Guo,Xiangqi Wang,Xiangliang Zhang*

Main category: cs.CL

TL;DR: FineLogic是一个细粒度的评估框架，用于评估大语言模型在逻辑推理中的表现，包括准确性、步骤合理性和表示对齐。研究发现自然语言监督具有更好的泛化能力，而符号监督则能提升推理的结构性。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅依赖最终答案准确性，无法全面评估逻辑推理过程的质量和结构，因此需要更精细的评估方法。

Method: 提出FineLogic框架，评估逻辑推理的三个维度，并研究不同监督格式（自然语言和符号形式）对推理能力的影响。

Result: 自然语言监督在泛化任务中表现更好，符号监督则能生成更结构化的推理链。微调主要通过逐步生成改进推理行为。
...
### [88] [Design of intelligent proofreading system for English translation based on CNN and BERT](https://arxiv.org/abs/2506.04811)
*Feijun Liu,Huifeng Wang,Kun Wang,Yizhen Wang*

Main category: cs.CL

TL;DR: 本文提出了一种结合CNN和BERT的混合方法，用于机器翻译校对，通过端到端训练实现高效错误检测与修正。


<details>
  <summary>Details</summary>
Motivation: 自动翻译常含错误，需人工校对，因此提出一种更高效的机器翻译校对方法。

Method: 结合CNN提取局部n-gram模式，BERT生成上下文丰富表示，集成错误检测与修正模块，使用端到端训练。

Result: 实验达到90%准确率、89.37% F1和16.24% MSE，性能优于现有方法10%以上。
...
### [89] [Evaluating Vision-Language and Large Language Models for Automated Student Assessment in Indonesian Classrooms](https://arxiv.org/abs/2506.04822)
...
### [90] [A Reasoning-Based Approach to Cryptic Crossword Clue Solving](https://arxiv.org/abs/2506.04824)
...
### [91] [Joint Evaluation of Answer and Reasoning Consistency for Hallucination Detection in Large Reasoning Models](https://arxiv.org/abs/2506.04832)
*Changyue Wang,Weihang Su,Qingyao Ai,Yiqun Liu*

Main category: cs.CL

TL;DR: 论文提出RACE框架，用于检测大型推理模型（LRMs）中的幻觉问题，通过分析推理步骤的四个信号实现细粒度检测。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以检测LRMs推理过程中的冗余或不一致，导致幻觉问题未被发现，影响模型透明性和性能。

Method: RACE框架提取关键推理步骤，计算四个诊断信号：推理一致性、答案不确定性、语义对齐和内部连贯性。

Result: 实验表明，RACE在多个数据集和LLMs上优于现有基线，能有效检测幻觉。
...
### [92] [MockConf: A Student Interpretation Dataset: Analysis, Word- and Span-level Alignment and Baselines](https://arxiv.org/abs/2506.04848)
...
<details>
  <summary>Details</summary>
Motivation: 现有平行语料库和算法无法有效建模同声传译中的长距离交互和特定类型的差异（如简化、功能泛化），需要专用数据集和工具。

Method: 收集了MockConf学生同传数据集（7小时录音，5种欧洲语言），并开发了InterAlign基于网页的标注工具，支持长输入的对齐。

Result: 发布了数据集和工具，并提出了评估指标和自动对齐的基线方法。

Conclusion: MockConf和InterAlign为同声传译研究和自动化提供了实用资源，填补了现有技术的空白。
...
### [93] [Multiple-Choice Question Generation Using Large Language Models: Methodology and Educator Insights](https://arxiv.org/abs/2506.04851)
...
Result: GPT-3.5生成的MCQs在多项指标上表现最优，但教育领域对AI的接受度仍有保留。
...
### [94] [Prompting LLMs: Length Control for Isometric Machine Translation](https://arxiv.org/abs/2506.04855)
*Dávid Javorský,Ondřej Bojar,François Yvon*
...
<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在等长翻译任务中的表现，以及如何通过提示和示例优化翻译质量和长度控制。

Method: 使用八种不同规模的开源大型语言模型，测试不同提示策略、少样本示例数量和演示选择对翻译的影响。

Result: 指令措辞与演示属性对齐对控制输出长度至关重要；少样本提示提升翻译质量，但边际效益递减；多输出选择可优化长度与质量的权衡。

Conclusion: 通过优化提示和演示选择，大型语言模型在等长翻译任务中可实现性能提升，但需注意长度控制的挑战。
...
### [95] [Evaluating the Effectiveness of Linguistic Knowledge in Pretrained Language Models: A Case Study of Universal Dependencies](https://arxiv.org/abs/2506.04887)
*Wenxi Li*

Main category: cs.CL

TL;DR: 将通用依存关系（UD）集成到预训练语言模型中，评估其在跨语言对抗性释义识别任务中的效果，结果显示UD显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 探讨UD作为跨语言句法表示框架的有效性，填补研究空白。

Method: 将UD整合到预训练语言模型中，评估其在跨语言对抗性释义识别任务中的表现。

Result: UD显著提升了模型的准确率和F1分数，平均增益分别为3.85%和6.08%，并在某些语言对中超越大型语言模型。

Conclusion: UD在跨领域任务中具有潜力和有效性，其相似性评分与模型性能正相关。
...
### [96] [ICPC-Eval: Probing the Frontiers of LLM Reasoning with Competitive Programming Contests](https://arxiv.org/abs/2506.04894)
*Shiyi Xu,Yiwen Hu,Yingqian Min,Zhipeng Chen,Wayne Xin Zhao,Ji-Rong Wen*

Main category: cs.CL

TL;DR: 提出了ICPC-Eval，一个用于评估大型语言模型在真实竞赛环境中编码能力的基准，包含挑战性问题、高效评估工具和新评价指标Refine@K。
...
Method: ICPC-Eval包含118个ICPC竞赛问题，提供真实竞赛场景、本地评估工具和Refine@K指标。

Result: 顶级推理模型依赖多轮代码反馈才能发挥潜力，但仍落后于人类团队。

Conclusion: ICPC-Eval为评估复杂推理能力提供了有效工具，并揭示了模型与人类表现的差距。
...
### [97] [Verbose ListOps (VLO): Beyond Long Context -- Unmasking LLM's Reasoning Blind Spots](https://arxiv.org/abs/2506.04907)
*Alex Pan,Mary-Anne Williams*

Main category: cs.CL

TL;DR: Verbose ListOps 是一个新基准，通过将 ListOps 计算转化为长故事，测试大语言模型在嵌套叙事推理中的表现，揭示了其在状态管理上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基准未能有效测试大语言模型在嵌套叙事推理中的能力，掩盖了其根本限制。Verbose ListOps 旨在填补这一空白。

Method: 通过将 ListOps 计算转化为长故事，并隐藏中间结果，迫使模型进行内部计算和状态管理。

Result: 实验显示，主流大语言模型在 Verbose ListOps 上表现不佳，尤其在长叙事中。

Conclusion: Verbose ListOps 揭示了模型在嵌套推理中的弱点，为改进推理能力提供了方向。
...
### [98] [A Practitioner's Guide to Building ASR Models for Low-Resource Languages: A Case Study on Scottish Gaelic](https://arxiv.org/abs/2506.04915)
...
<details>
  <summary>Details</summary>
Motivation: 探讨在低资源语言环境下，是否微调多语言端到端模型是最佳方法，并提出替代方案。

Method: 结合混合HMM与自监督模型，通过持续自监督预训练和半监督训练充分利用可用数据。

Result: 在苏格兰盖尔语上，相比最佳微调Whisper模型，WER相对降低了32%。

Conclusion: 混合HMM与自监督模型的方法在低资源语言ASR任务中优于微调端到端模型。
...
### [99] [Simulating LLM-to-LLM Tutoring for Multilingual Math Feedback](https://arxiv.org/abs/2506.04920)
*Junior Cedric Tonga,KV Aditya Srivatsa,Kaushal Kumar Maurya,Fajri Koto,Ekaterina Kochmar*

Main category: cs.CL

TL;DR: 论文研究了多语言大语言模型（LLMs）在教育中的反馈效果，发现多语言提示能显著提升学习效果，尤其是在低资源语言中。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在不同语言中提供数学推理任务反馈的能力，填补多语言教育工具的空白。

Method: 通过模拟多语言师生互动，使用强模型生成提示，弱模型模拟学生，评估语言对齐对学习效果的影响。

Result: 多语言提示显著提升学习效果，尤其是学生母语与反馈语言一致时。
...
### [100] [ConECT Dataset: Overcoming Data Scarcity in Context-Aware E-Commerce MT](https://arxiv.org/abs/2506.04929)
*Mikołaj Pokrywka,Wojciech Kusa,Mieszko Rutkowski,Mikołaj Koszowski*

Main category: cs.CL

TL;DR: 论文提出ConECT数据集，结合视觉和产品元数据提升捷克语到波兰语的电商翻译质量，验证了上下文信息对机器翻译的改进效果。


<details>
  <summary>Details</summary>
Motivation: 神经机器翻译在电商领域仍面临词义模糊和上下文不足的问题，需结合额外信息提升翻译质量。

Method: 创建ConECT数据集（11,400句对），测试视觉语言模型（VLM）和文本模型（如产品类别路径）的上下文融合效果。

Result: 视觉上下文和文本上下文均能显著提升翻译质量。

Conclusion: 上下文信息的融入可有效改进机器翻译，数据集已公开。
...
### [101] [From Struggle (06-2024) to Mastery (02-2025) LLMs Conquer Advanced Algorithm Exams and Pave the Way for Editorial Generation](https://arxiv.org/abs/2506.04965)
...
Method: 测试多个LLM模型在罗马尼亚语考试及其高质量英语翻译中的表现，分析问题解决能力、一致性和多语言性能。

Result: 最新模型成绩接近优秀学生，具备复杂推理能力，但在图任务上表现不佳。

Conclusion: LLMs在教育中有潜力，尤其是在生成高质量反馈内容方面，为生成式AI在算法教育中的进一步整合提供了基础。
...
### [102] [SCOP: Evaluating the Comprehension Process of Large Language Models from a Cognitive View](https://arxiv.org/abs/2506.05000)
*Yongjie Xiao,Hongru Liang,Peixin Qin,Yao Zhang,Wenqiang Lei*

Main category: cs.CL

TL;DR: 论文提出SCOP框架，从认知角度评估大语言模型（LLMs）的理解过程，发现其与专家水平仍有差距，并指出改进方向。
...
Method: 提出SCOP框架，定义五项必备理解技能，构建测试数据，并对开源和闭源LLMs进行详细分析。

Result: LLMs在理解过程中仍难以达到专家水平，但存在局部信息理解优势；部分模型通过错误过程得出正确答案。

Conclusion: 改进LLMs需关注理解过程，确保训练中全面培养所有理解技能。
...
### [103] [ComfyUI-Copilot: An Intelligent Assistant for Automated Workflow Development](https://arxiv.org/abs/2506.05010)
*Zhenran Xu,Xue Yang,Yiyu Wang,Qingli Hu,Zijiao Wu,Longyue Wang,Weihua Luo,Kaifu Zhang,Baotian Hu,Min Zhang*

Main category: cs.CL

TL;DR: ComfyUI-Copilot是一个基于大语言模型的插件，旨在提升ComfyUI（AI艺术创作开源平台）的易用性和效率，通过智能节点推荐和自动化工作流构建解决新用户面临的挑战。
...
### [104] [Controlling Summarization Length Through EOS Token Weighting](https://arxiv.org/abs/2506.05017)
*Zeno Belligoli,Emmanouil Stergiadis,Eran Fainman,Ilya Gusev*

Main category: cs.CL

TL;DR: 提出了一种简单的方法来控制自动文本摘要的长度，通过调整交叉熵损失中EOS令牌的预测重要性，无需复杂模型改动。
...
Method: 通过增加交叉熵损失中EOS令牌预测的重要性来控制生成文本长度，方法兼容不同架构和解码算法。
...
### [105] [Automatic Robustness Stress Testing of LLMs as Mathematical Problem Solvers](https://arxiv.org/abs/2506.05038)
*Yutao Hou,Zeguan Xiao,Fei Yu,Yihan Jiang,Xuetao Wei,Hailiang Huang,Yun Chen,Guanhua Chen*

Main category: cs.CL

TL;DR: 论文提出了一种名为AR-Checker的框架，用于动态生成数学问题变体以测试大语言模型（LLMs）的鲁棒性，避免数据污染问题。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在推理任务中表现优异，但在简单任务中可能因鲁棒性问题意外失败。现有评估方法依赖手工模板或有限扰动规则，可能存在数据污染风险。

Method: AR-Checker通过多轮并行LLM重写和验证生成语义不变但可能使LLMs失败的数学问题变体。

Result: 在GSM8K和MATH-500等数学任务上表现优异，且在MMLU、MMLU-Pro和CommonsenseQA等非数学基准测试中也验证了其有效性。

Conclusion: AR-Checker为动态评估LLMs鲁棒性提供了高效且避免数据污染的解决方案。
...
### [106] [TALL -- A Trainable Architecture for Enhancing LLM Performance in Low-Resource Languages](https://arxiv.org/abs/2506.05057)
...
Method: 整合LLM与双语翻译模型，通过维度对齐层和定制转换器将低资源输入转换为高资源表示。

Result: 在希伯来语实验中，TALL显著优于直接使用、简单翻译和微调等方法。

Conclusion: TALL是一种参数高效的方法，通过冻结预训练组件并仅训练轻量适配模块，平衡了计算效率和性能提升。
...
### [107] [Debatable Intelligence: Benchmarking LLM Judges via Debate Speech Evaluation](https://arxiv.org/abs/2506.05062)
*Noy Sternlicht,Ariel Gera,Roy Bar-Haim,Tom Hope,Noam Slonim*

Main category: cs.CL

TL;DR: 论文提出辩论演讲评估作为评估LLM法官的新挑战性基准，分析了LLM与人类评委的差异。


<details>
  <summary>Details</summary>
Motivation: 评估辩论演讲需要多层次的深度理解，但目前缺乏系统性LLM基准研究。

Method: 利用600多篇标注辩论演讲数据集，分析前沿LLM与人类评委的表现差异。

Result: 大模型在某些方面接近人类评委，但整体判断行为差异显著；前沿LLM生成说服性演讲能力接近人类水平。

Conclusion: 辩论演讲评估为LLM能力提供了新视角，揭示了模型与人类评委的复杂差异。
...
### [108] [Does It Make Sense to Speak of Introspection in Large Language Models?](https://arxiv.org/abs/2506.05068)
...
<details>
  <summary>Details</summary>
Motivation: 研究动机在于理解LLMs的自我报告行为是否具有内省性质，以及如何解释其与人类内省的区别。

Method: 通过分析两个LLMs的自我报告案例，批判性地评估其是否符合内省的定义。

Result: 第一个案例（描述创作过程）不被视为内省；第二个案例（推断自身温度参数）可视为最小化的内省，但无意识体验。

Conclusion: LLMs的自我报告行为在特定条件下可被视为内省，但与人类内省存在本质区别。
...
### [109] [RIVAL: Reinforcement Learning with Iterative and Adversarial Optimization for Machine Translation](https://arxiv.org/abs/2506.05070)
...
<details>
  <summary>Details</summary>
Motivation: 观察到结合RLHF的LLM在口语字幕翻译任务中表现意外差，原因是离线奖励模型与在线LLM因分布偏移逐渐偏离。

Method: 提出RIVAL对抗训练框架，将RM与LLM建模为min-max博弈，结合定性与定量奖励优化翻译质量。

Result: 实验表明RIVAL显著提升翻译基线表现。
...
### [110] [Just a Scratch: Enhancing LLM Capabilities for Self-harm Detection through Intent Differentiation and Emoji Interpretation](https://arxiv.org/abs/2506.05073)
*Soumitra Ghosh,Gopendra Vikram Singh,Shambhavi,Sabarna Choudhury,Asif Ekbal*

Main category: cs.CL

TL;DR: 论文提出了一种通过语言-表情符号交互增强大语言模型（LLM）自我伤害意图检测能力的方法，并发布了CESM-100和SHINES数据集。


<details>
  <summary>Details</summary>
Motivation: 社交媒体上的自我伤害检测对早期干预和心理健康支持至关重要，但现有LLM难以理解隐晦的语言和表情符号。

Method: 通过CESM-100丰富输入，多任务学习微调LLM，并生成可解释的预测依据。

Result: 在零样本、少样本和微调场景下，该方法显著提升了LLM的检测和解释能力。

Conclusion: 结合意图区分和上下文线索，有效解决了自我伤害信号的模糊性问题，相关资源已公开。
...
### [111] [Parking, Perception, and Retail: Street-Level Determinants of Community Vitality in Harbin](https://arxiv.org/abs/2506.05080)
*HaoTian Lan*

Main category: cs.CL

TL;DR: 研究提出了一种基于图像的可解释框架，分析中国哈尔滨社区街道的商业活力与车辆密度、绿化、清洁度和街道宽度等特征的关系，发现适度车辆存在有助于商业，但过度停车会降低步行性和满意度。


<details>
  <summary>Details</summary>
Motivation: 探索社区街道商业活力的驱动因素，特别是车辆可达性、环境质量和行人感知的复杂互动。

Method: 利用街景图像和多模态大语言模型（VisualGLM-6B）构建社区商业活力指数（CCVI），并通过GPT-4感知建模分析其与空间属性的关系。

Result: 适度车辆存在提升商业可达性，但过度停车（尤其在窄街道）降低步行性和满意度；高绿化和清洁度显著提升满意度，但对定价影响较弱。

Conclusion: 研究展示了AI辅助感知与城市形态分析结合的价值，为社区振兴提供了设计和管理工具的理论与方法支持。
...
### [112] [CL-ISR: A Contrastive Learning and Implicit Stance Reasoning Framework for Misleading Text Detection on Social Media](https://arxiv.org/abs/2506.05107)
...
<details>
  <summary>Details</summary>
Motivation: 社交媒体上的误导文本可能导致公众误解、社会恐慌甚至经济损失，因此检测这些文本是一个重要研究方向。

Method: 1. 使用对比学习算法增强模型对真实与误导文本语义差异的学习能力；2. 引入隐式立场推理模块，分析文本中的潜在立场倾向及其与相关主题的关系。

Result: CL-ISR框架显著提高了误导文本的检测效果，尤其是在语言复杂或隐含立场的情况下。

Conclusion: CL-ISR通过结合对比学习和隐式立场推理，为社交媒体误导文本检测提供了更高效的方法。
...
### [113] [DiCoRe: Enhancing Zero-shot Event Detection via Divergent-Convergent LLM Reasoning](https://arxiv.org/abs/2506.05128)
*Tanmay Parekh,Kartik Mehta,Ninareh Mehrabi,Kai-Wei Chang,Nanyun Peng*

Main category: cs.CL

TL;DR: DiCoRe是一个用于零样本事件检测的框架，通过发散-收敛推理方法（Dreamer和Grounder）提升事件覆盖率和精度，实验表明其优于现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 零样本事件检测在专业领域中至关重要，但现有大型语言模型（LLMs）难以处理复杂事件本体和领域特定触发器。

Method: DiCoRe采用发散-收敛推理框架：Dreamer进行开放式事件发现（发散），Grounder通过有限状态机约束解码对齐任务指令（收敛），并由LLM-Judge验证输出。

Result: 在六个数据集和五个领域的实验中，DiCoRe平均F1分数比最佳基线高4-7%。
...
### [114] [Information Locality as an Inductive Bias for Neural Language Models](https://arxiv.org/abs/2506.05136)
*Taiga Someya,Anej Svete,Brian DuSell,Timothy J. O'Donnell,Mario Giulianelli,Ryan Cotterell*

Main category: cs.CL

TL;DR: 论文提出了一种量化框架，通过m-local熵衡量语言模型的局部不确定性，发现高熵语言对Transformer和LSTM更难学习，表明神经语言模型与人类类似，对语言的局部统计结构敏感。


<details>
  <summary>Details</summary>
Motivation: 探讨神经语言模型的归纳偏置是否与人类处理约束一致，需要一种可控的量化方法。

Method: 提出m-local熵作为信息论度量，通过实验在扰动自然语言和PFSA定义的语言上测试Transformer和LSTM。

Result: 高m-local熵的语言对模型更难学习，表明模型对局部统计结构敏感。

Conclusion: 神经语言模型的归纳偏置与人类类似，对局部统计结构敏感。
...
Method: 扩展van Miltenburg等人的实验框架，评估四种错误类型（年龄、性别、服装类型、颜色）在单模态和多模态设置下的评分差异。

Result: 人类对不同错误类型的严重性评分有差异，而LLMs在性别和颜色错误上的评分与人类不一致；DeepSeek-V3表现最佳。

Conclusion: LLMs可能内化了社会规范但对颜色感知不足，DeepSeek-V3在多模态和单模态下均表现优异。
...
TL;DR: 论文提出Knowledgeable-r1方法，通过联合采样和多策略分布来平衡检索增强生成（RAG）中检索知识与模型固有知识的使用，显著提升了鲁棒性和推理准确性。
...
Method: 采用联合采样和多策略分布，探索知识能力，激发模型对参数化知识和上下文知识的自我整合利用。

Result: 实验表明，Knowledgeable-r1在参数与上下文冲突任务及一般RAG任务中显著提升性能，尤其在反事实场景中优于基线17.07%。

Conclusion: Knowledgeable-r1有效解决了RAG中知识使用不平衡的问题，提升了模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）存在社会、人口和性别偏见，这些偏见源于训练数据。研究旨在揭示这些偏见在模型中的结构表现。

Method: 采用机制可解释性方法，通过不同指标识别导致偏见行为的内部分量，并评估其稳定性、定位性和泛化性。

Result: 偏见相关计算高度集中在少数层，且这些组件在微调设置中会变化。移除偏见组件会减少偏见输出，但也会影响其他NLP任务。

Conclusion: 偏见在模型中的表现具有局部性，且与其他任务共享重要组件，移除偏见可能对其他任务产生副作用。
...
Method: 提出ECoRAG框架，通过证据性指导压缩检索文档，并动态补充不足证据。

Result: 实验表明ECoRAG在ODQA任务中优于现有压缩方法，且成本高效。

Conclusion: ECoRAG显著提升LLM性能，同时优化资源使用。
...
TL;DR: Qwen3 Embedding系列是基于Qwen3基础模型的文本嵌入和重排序技术，通过多阶段训练和模型合并策略，提供多种尺寸模型，在多项基准测试中表现优异。
...
Method: 结合无监督预训练和有监督微调，利用Qwen3 LLMs生成高质量训练数据，采用模型合并策略。

Result: 在MTEB等多语言基准测试和检索任务中取得最优表现，提供0.6B、4B、8B三种模型尺寸。

Conclusion: Qwen3 Embedding系列技术先进，公开可用，支持社区研究和开发。
...
TL;DR: 研究探讨了大规模神经语言模型在上下文学习中进行反事实推理的能力，特别是在线性回归任务中的噪声溯因，并发现Transformer模型能够通过上下文观察的转换实现此类推理。


<details>
  <summary>Details</summary>
Motivation: 探索语言模型在无需参数更新的情况下，通过上下文学习进行反事实推理的能力，尤其是在假设场景下预测变化后果。

Method: 采用线性回归任务作为合成实验场景，要求模型通过噪声溯因进行准确预测，并分析Transformer的自注意力、模型深度和预训练数据多样性对性能的影响。

Result: 语言模型在受控设置中能够进行反事实推理，且此类推理可转化为对上下文观察的转换；研究还发现Transformer在序列数据上也能进行噪声溯因。

Conclusion: 研究为语言模型在反事实推理方面的潜力提供了初步证据，特别是在反事实故事生成等任务中具有潜在应用价值。
...
Method: 引入RELIC框架，利用语言识别任务（判断字符串是否由形式语法生成）评估LLMs，通过合成语言动态调整任务复杂度。

Result: LLMs的准确率可从语法和样本复杂度预测，复杂任务中模型依赖浅层启发式而非复杂指令。

Conclusion: 当前LLMs在复杂指令跟随任务中表现有限，需进一步优化。
...
TL;DR: 论文介绍了Common Pile v0.1，一个用于LLM预训练的8TB开放授权文本数据集，并验证了其性能。
...
Method: 收集、整理并发布Common Pile v0.1数据集，训练了两个7B参数的LLM模型（Comma v0.1-1T和Comma v0.1-2T）。

Result: 训练出的模型性能与使用未经授权文本训练的类似规模LLM（如Llama 1和2 7B）相当。

Conclusion: Common Pile v0.1为LLM训练提供了合法的替代方案，并公开了数据集、代码和模型检查点。
...
TL;DR: 研究了自监督辅助任务在极低资源语言形态变化任务中的有效性，发现自动编码在数据极少时表现最佳，而字符掩码语言模型（CMLM）在数据增加时更有效。基于已知语素边界的掩码采样能持续提升性能。
...
Conclusion: 自监督辅助任务在低资源形态建模中具有潜力，尤其是结合语素信息的掩码策略。
...
Method: 结合短语结构语法的成分性、依存语法的头-依存关系和范畴语法的函子-论元关系，提出统一数学推导。

Result: 展示了不连续和连续结构可通过统一系统分析。

Conclusion: 证明了三种语法形式可统一用于语言结构分析。
...
Method: 定义了一个三步推理过程：1）声明分解；2）子声明归因和蕴含分类；3）聚合分类。
...
<details>
  <summary>Details</summary>
Motivation: RAG系统中外部检索知识与LLMs固有知识冲突，影响QA任务性能，现有方法难以有效解决。

Method: 提出Micro-Act框架，通过感知上下文复杂度并分解知识源为细粒度比较步骤，实现自适应推理。
...
<details>
  <summary>Details</summary>
Motivation: 多AI代理协作工作流常因提示设计不佳导致错误传播和性能低下，限制了系统的可靠性和可扩展性。

Method: ProRefine利用大型语言模型的文本反馈动态优化提示，无需额外训练或真实标签。

Result: 在五个数学推理基准测试中，ProRefine比零样本思维链基线提高了3至37个百分点。
...
TL;DR: 本文提出了一种新的LLM遗忘方法，通过约束优化和软max-free损失函数，有效移除敏感信息同时保留有用数据。


<details>
  <summary>Details</summary>
Motivation: 现实部署的LLM需要移除敏感或过时信息，现有方法在遗忘和保留之间难以平衡，导致性能下降。

Method: 将遗忘问题建模为约束优化，使用对数边际平坦化损失函数，并通过原始-对偶算法求解。

Result: 在TOFU和MUSE基准测试中，该方法优于现有基线，有效移除目标信息并保持下游性能。

Conclusion: 该方法为LLM遗忘提供了更稳定和高效的解决方案。
...
TL;DR: Search Arena是一个大规模、多轮交互的数据集，用于分析搜索增强语言模型的用户偏好，揭示了引用数量和来源对用户偏好的影响。
...
Method: 通过众包构建包含24,000对多轮交互的数据集，涵盖多种意图和语言，并收集12,000个人类偏好投票。

Result: 用户偏好受引用数量和来源影响，社区驱动平台更受欢迎；搜索增强模型在非搜索环境中表现良好，但在搜索密集环境中依赖参数知识会显著降低质量。

Conclusion: Search Arena为未来研究提供了重要资源，揭示了搜索增强模型的局限性和用户偏好特征。
...
TL;DR: 语言模型在偏好判断中存在系统性偏差，过度依赖表面特征（如长度、结构等），导致评估不可靠。研究发现训练数据中的偏差是主要原因，并提出一种基于反事实数据增强的后训练方法，有效减少偏差。


<details>
  <summary>Details</summary>
Motivation: 语言模型在人类偏好判断中作为代理存在系统性偏差，影响评估的可靠性，因此需要研究偏差来源并提出解决方案。
...
### [131] [Photoreal Scene Reconstruction from an Egocentric Device](https://arxiv.org/abs/2506.04444)
*Zhaoyang Lv,Maurizio Monge,Ka Chen,Yufeng Zhu,Michael Goesele,Jakob Engel,Zhao Dong,Richard Newcombe*

Main category: cs.CV

TL;DR: 论文研究了使用自我中心设备进行高动态范围场景真实感重建的挑战，提出视觉-惯性束调整（VIBA）和高斯泼溅物理图像形成模型，显著提升了重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常假设使用设备视觉-惯性里程计的帧率6DoF姿态估计，可能忽略像素级重建的关键细节。

Method: 采用VIBA校准滚动快门RGB相机的高频轨迹，并基于高斯泼溅引入物理图像形成模型。

Result: 实验表明，VIBA和图像形成模型分别带来+1 dB的PSNR提升。

Conclusion: 提出的方法在多种光照条件下显著提升了重建质量，适用于广泛的高斯泼溅表示。
...
### [132] [Dynamic Epsilon Scheduling: A Multi-Factor Adaptive Perturbation Budget for Adversarial Training](https://arxiv.org/abs/2506.04263)
*Alan Mitkiy,James Smith,Hana Satou,Hiroshi Tanaka,Emily Johnson,F Monkey*

Main category: cs.CV

TL;DR: 论文提出了一种动态调整对抗训练扰动预算的方法（DES），通过结合梯度代理、预测置信度和模型不确定性，显著提升了对抗鲁棒性和标准准确性。
...
Conclusion: DES为实例感知、数据驱动的对抗训练开辟了新方向，并提供了理论支持。
...
### [133] [Enhancing Frequency for Single Image Super-Resolution with Learnable Separable Kernels](https://arxiv.org/abs/2506.04555)
*Heng Tian*

Main category: cs.CV

TL;DR: 提出了一种名为LSKs的即插即用模块，通过可学习的可分离核直接增强图像频率成分，显著减少参数和计算量，并提升超分辨率性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过间接方式（如特殊损失函数）提升单图像超分辨率性能，而LSKs旨在直接从频率角度优化图像质量。

Method: LSKs是秩为一的矩阵，可分解为可合并的一维核，从而减少参数和计算量。通过频率视角分析其适用性，并进行可解释性分析。

Result: 实验表明，LSKs减少60%以上的参数和计算量，同时提升模型性能，尤其在放大因子增加时表现更优。

Conclusion: LSKs是一种高效且有效的模块，适用于单图像超分辨率任务，显著提升性能并降低资源消耗。
...
### [134] [RSVP: Reasoning Segmentation via Visual Prompting and Multi-modal Chain-of-Thought](https://arxiv.org/abs/2506.04277)
...
Method: RSVP采用两阶段框架：推理阶段通过多模态链式视觉提示生成区域建议；分割阶段通过视觉语言分割模块优化分割掩码。

Result: 在ReasonSeg上性能提升+6.5 gIoU和+9.2 cIoU，零样本设置下SegInW达到49.7 mAP。
...
### [135] [Truth in the Few: High-Value Data Selection for Efficient Multi-Modal Reasoning](https://arxiv.org/abs/2506.04755)
*Shenshen Li,Kaiyuan Deng,Lei Wang,Hao Yang,Chong Peng,Peng Yan,Fumin Shen,Heng Tao Shen,Xing Xu*

Main category: cs.CV

TL;DR: 该论文提出了一种名为RAP的数据选择范式，通过识别能触发多模态推理的稀疏样本（认知样本），显著减少训练数据量和计算成本。


<details>
  <summary>Details</summary>
Motivation: 挑战多模态大语言模型（MLLMs）需要大量训练数据的假设，发现仅少数样本能有效触发多模态推理。

Method: 提出RAP方法，包括Causal Discrepancy Estimator（CDE）和Attention Confidence Estimator（ACE）来筛选认知样本，并引入Difficulty-aware Replacement Module（DRM）增强样本复杂性。

Result: 在六个数据集上，RAP仅使用9.3%的训练数据即实现更优性能，计算成本降低43%以上。

Conclusion: RAP证明了高效数据选择可显著提升多模态推理能力，减少冗余和计算开销。
...
### [136] [Evaluating MLLMs with Multimodal Multi-image Reasoning Benchmark](https://arxiv.org/abs/2506.04280)
...
TL;DR: MMRB是首个评估多图像结构化视觉推理的基准，包含92个子任务，覆盖空间、时间和语义推理，并采用GPT-4o生成的多解决方案和CoT标注。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM基准主要关注单图像推理或多图像理解的最终答案评估，缺乏对多图像输入推理能力的深入探索。

Method: 提出MMRB基准，包含多图像推理任务，并设计句子级匹配框架以支持快速评估。

Result: 开源MLLM在多图像推理任务中显著落后于商业模型，且当前多模态奖励模型几乎无法处理多图像奖励排序任务。
...
### [137] [HuGeDiff: 3D Human Generation via Diffusion with Gaussian Splatting](https://arxiv.org/abs/2506.04351)
*Maksym Ivashechkin,Oscar Mendez,Richard Bowden*

Main category: cs.CV

TL;DR: 提出了一种弱监督流程，通过生成可控属性的真实人类图像数据集，结合基于Transformer的架构和点云扩散模型，显著提升了3D人类生成的效率、真实性和文本对齐。
...
Method: 1. 使用图像扩散模型生成可控属性的真实人类图像数据集；2. 提出基于Transformer的图像特征到3D点云的高效映射方法；3. 训练条件点云扩散模型。
...
### [138] [ReXVQA: A Large-scale Visual Question Answering Benchmark for Generalist Chest X-ray Understanding](https://arxiv.org/abs/2506.04353)
*Ankit Pal,Jung-Oh Lee,Xiaoman Zhang,Malaikannan Sankarasubbu,Seunghyeon Roh,Won Jung Kim,Meesun Lee,Pranav Rajpurkar*

Main category: cs.CV

TL;DR: ReXVQA是最大的胸部放射学视觉问答（VQA）基准，包含696,000个问题和160,000个胸部X光研究，评估了8种多模态大语言模型，其中MedGemma表现最佳（83.24%准确率），甚至超过人类专家（77.27%）。


<details>
  <summary>Details</summary>
Motivation: 填补现有VQA基准在胸部放射学中的不足，提供多样化和临床真实的任务，以评估核心放射学推理能力。

Method: 构建大规模数据集ReXVQA，包含五种核心放射学推理任务，并评估八种多模态大语言模型。通过人类读者研究对比AI与专家表现。

Result: MedGemma模型表现最佳（83.24%准确率），超过人类专家（77.27%）。人类读者间一致性高，但与AI模型的一致性模式更复杂。
...
### [139] [WorldPrediction: A Benchmark for High-level World Modeling and Long-horizon Procedural Planning](https://arxiv.org/abs/2506.04363)
...
Method: 通过区分正确的动作或动作序列（从反事实干扰项中）来评估模型，使用视觉观察表示状态和动作，并提供“动作等价物”以避免低级别线索的利用。
...
### [140] [Gen-n-Val: Agentic Image Data Generation and Validation](https://arxiv.org/abs/2506.04676)
...
Method: Gen-n-Val包含两个代理：LD提示代理优化Layer Diffusion的提示生成高质量前景实例和掩码；数据验证代理过滤低质量数据。系统提示通过TextGrad优化，并使用图像协调技术组合多个实例。

Result: 无效合成数据从50%降至7%，在COCO实例分割中提升1% mAP，开放词汇目标检测中提升7.1% mAP。
...
### [141] [Ice Hockey Puck Localization Using Contextual Cues](https://arxiv.org/abs/2506.04365)
...
Result: 在PuckDataset上，PLUCC的检测性能优于基线方法，平均精度提升12.2%，RSLE提升25%。
...
### [142] [Fine-Tuning Video Transformers for Word-Level Bangla Sign Language: A Comparative Analysis for Classification Tasks](https://arxiv.org/abs/2506.04367)
*Jubayer Ahmed Bhuiyan Shawon,Hasan Mahmud,Kamrul Hasan*

Main category: cs.CV

TL;DR: 该研究通过微调视频Transformer架构（VideoMAE、ViViT和TimeSformer）在BdSLW60和BdSLW401数据集上进行手语识别，显著优于传统方法，其中VideoMAE表现最佳。
...
Method: 使用视频Transformer模型（VideoMAE、ViViT、TimeSformer），结合数据增强和10折分层交叉验证，评估其在BdSL数据集上的性能。

Result: VideoMAE在BdSLW60和BdSLW401数据集上分别达到95.5%和81.04%的准确率，优于其他方法。

Conclusion: 视频Transformer模型在BdSL识别中表现出色，具有可扩展性和高准确性潜力。
...
### [143] [Visualizing and Controlling Cortical Responses Using Voxel-Weighted Activation Maximization](https://arxiv.org/abs/2506.04379)
...
Method: 利用预训练的Inception V3网络提取特征，通过线性回归预测fMRI响应，并应用激活最大化生成优化图像。
...
### [144] [Is Perturbation-Based Image Protection Disruptive to Image Editing?](https://arxiv.org/abs/2506.04394)
...
Result: 大多数情况下，保护后的图像仍能被扩散模型成功编辑，且扰动可能增强图像与文本提示的关联性。

Conclusion: 基于扰动的方法不足以提供针对扩散模型编辑的鲁棒图像保护，需探索其他解决方案。
...
### [145] [Normalize Filters! Classical Wisdom for Deep Vision](https://arxiv.org/abs/2506.04401)
*Gustavo Perez,Stella X. Yu*

Main category: cs.CV

TL;DR: 论文提出了一种滤波器归一化方法，通过可学习的缩放和平移，使深度学习中的卷积滤波器具备大气等变性，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 传统图像滤波器经过精心归一化以确保一致性和可解释性，而深度学习中的卷积滤波器缺乏此类约束，导致在图像大气传输时响应失真。

Method: 提出滤波器归一化方法，结合可学习的缩放和平移，类似于批归一化，使滤波器具备大气等变性。

Result: 方法在人工和自然强度变化基准测试中表现优异，ResNet34甚至大幅超越CLIP。

Conclusion: 滤波器归一化能规范学习、促进多样性，并提升鲁棒性和泛化能力。
...
### [146] [HMAR: Efficient Hierarchical Masked Auto-Regressive Image Generation](https://arxiv.org/abs/2506.04421)
...
Method: HMAR采用马尔可夫过程和多步掩码生成，仅依赖前一个分辨率尺度的信息，并使用可控的多步生成策略。

Result: HMAR在ImageNet 256x256和512x512基准测试中表现优于VAR、扩散模型和自回归基线，训练和推理速度更快，内存占用更低。

Conclusion: HMAR不仅提升了图像生成质量与效率，还提供了更灵活的采样计划和零样本图像编辑能力。
...
### [147] [Towards Large-Scale Pose-Invariant Face Recognition Using Face Defrontalization](https://arxiv.org/abs/2506.04496)
...
<details>
  <summary>Details</summary>
Motivation: 当前的人脸识别方法在极端头部姿态下表现不佳，且依赖复杂的方法（如正面化）和小规模数据集，实用性有限。

Method: 1) 训练一个去正面化模型（FFWM）处理正面-侧面人脸对；2) 基于ArcFace损失训练ResNet-50模型，使用原始和随机去正面化的大规模数据集。

Result: 在LFW、AgeDB、CFP数据集上优于现有方法，但在Multi-PIE极端姿态（75和90度）上表现不佳。

Conclusion: 去正面化方法在大型数据集上表现优越，但当前方法可能对小数据集过拟合。
...
### [148] [FALO: Fast and Accurate LiDAR 3D Object Detection on Resource-Constrained Devices](https://arxiv.org/abs/2506.04499)
...
<details>
  <summary>Details</summary>
Motivation: 现有LiDAR 3D检测方法依赖于稀疏卷积或Transformer，计算成本高且内存访问模式不规则，难以在边缘设备上运行。

Method: FALO将稀疏3D体素排列为1D序列，并通过ConvDotMix块（包含大核卷积、Hadamard乘积和线性层）处理，实现空间和嵌入维度的混合。

Result: 在nuScenes和Waymo基准测试中，FALO表现优异，推理速度比最新SOTA快1.6~9.8倍。

Conclusion: FALO是一种高效且适用于边缘设备的LiDAR 3D检测方法，兼具高性能和快速推理。
...
### [149] [AuthGuard: Generalizable Deepfake Detection via Language Guidance](https://arxiv.org/abs/2506.04501)
...
Conclusion: AuthGuard通过语言指导和视觉编码器的结合，显著提升了深度伪造检测的泛化和准确性。
...
### [150] [Pruning Everything, Everywhere, All at Once](https://arxiv.org/abs/2506.04513)
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型复杂度高，计算成本大，现有剪枝方法仅单独剪枝神经元或层，未能同时优化两者。

Method: 通过Centered Kernel Alignment度量选择与父网络表示相似性最高的子网络，迭代剪枝神经元和层。

Result: 在ResNet56和ResNet110上分别实现86.37%和95.82%的FLOPs减少，同时保持或提升准确性，并增强对抗性和分布外样本的鲁棒性。
...
### [151] [EECD-Net: Energy-Efficient Crack Detection with Spiking Neural Networks and Gated Attention](https://arxiv.org/abs/2506.04526)
*Shuo Zhang*
...
<details>
  <summary>Details</summary>
Motivation: 由于智能终端设备在低分辨率和有限能量下的实时监测性能不足，需要一种高效且低功耗的裂缝检测方法。
...
### [152] [Perceptual Decoupling for Scalable Multi-modal Reasoning via Reward-Optimized Captioning](https://arxiv.org/abs/2506.04559)
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在升级推理能力时面临视觉-语言对齐的高成本问题，需要一种更高效的方法。

Method: 采用感知与推理解耦的策略，通过强化学习优化视觉提取器的描述生成（RACRO方法）。

Result: RACRO在多模态数学和科学基准测试中表现优异，支持更高级推理模型的即插即用。

Conclusion: RACRO显著提升了视觉基础能力，同时降低了多模态对齐的成本，具有可扩展性和适应性。
...
### [153] [LGM-Pose: A Lightweight Global Modeling Network for Real-time Human Pose Estimation](https://arxiv.org/abs/2506.04561)
*Biao Guo,Fangmin Guo,Guibo Luo,Xiaonan Luo,Feng Zhang*

Main category: cs.CV

TL;DR: 提出了一种轻量级全局建模网络（LGM-Pose），通过单分支结构解决多分支CNN网络的冗余和全局上下文不足问题，性能优于主流轻量方法。


<details>
  <summary>Details</summary>
Motivation: 当前多分支CNN网络在多人体姿态估计中难以捕捉全局上下文且延迟高，需改进。

Method: 设计了轻量级MobileViM Block和LARM模块，结合NPT-Op提取全局信息；引入SFusion模块融合多尺度信息。

Result: 在COCO和MPII数据集上，参数更少且性能更优，处理速度更快。

Conclusion: LGM-Pose通过单分支结构有效解决了多分支网络的不足，实现了高效轻量化的姿态估计。
...
### [154] [Follow-Your-Creation: Empowering 4D Creation through Video Inpainting](https://arxiv.org/abs/2506.04590)
*Yue Ma,Kunyu Feng,Xinhua Zhang,Hongyu Liu,David Junhao Zhang,Jinbo Xing,Yinhan Zhang,Ayden Yang,Zeyu Wang,Qifeng Chen*

Main category: cs.CV

TL;DR: Follow-Your-Creation是一个新颖的4D视频生成与编辑框架，通过视频修复模型作为生成先验，将4D视频创建转化为视频修复任务，支持单目视频输入。
...
Result: 方法在质量和多功能性上显著优于现有技术，支持基于提示的内容编辑。

Conclusion: 该框架有效利用了基础模型的先验知识，实现了高质量的4D视频生成与编辑。
...
### [155] [Hierarchical-Task-Aware Multi-modal Mixture of Incremental LoRA Experts for Embodied Continual Learning](https://arxiv.org/abs/2506.04595)
*Ziqi Jia,Anmin Wang,Xiaoyang Qu,Xiaowen Yang,Jianzong Wang*

Main category: cs.CV

TL;DR: 论文提出了一种分层持续学习框架（HEC）和任务感知的增量LoRA专家方法（Task-aware MoILE），以解决现有方法忽视高层规划和多级知识学习的问题。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法主要关注基于人类指令的低层动作执行，缺乏对高层规划和多级知识学习的能力。

Method: 提出HEC框架，将学习过程分为高层指令和低层动作两层，并定义五个子任务。Task-aware MoILE方法通过聚类视觉-文本嵌入实现任务识别，并使用任务级和令牌级路由器选择LoRA专家。通过SVD保留关键参数以减少灾难性遗忘。

Result: 实验表明，该方法在减少旧任务遗忘方面表现优异，有效支持代理在持续学习新任务时保留先验知识。

Conclusion: HEC和Task-aware MoILE方法为持续学习提供了高效的分层框架和任务识别机制，显著减少了灾难性遗忘。
...
### [156] [SmartAvatar: Text- and Image-Guided Human Avatar Generation with VLM AI Agents](https://arxiv.org/abs/2506.04606)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的方法在3D对象生成中虽取得进展，但难以精确控制人体身份、体型和动画准备。SmartAvatar旨在解决这一问题。
...
### [157] [Perfecting Depth: Uncertainty-Aware Enhancement of Metric Depth](https://arxiv.org/abs/2506.04612)
...
<details>
  <summary>Details</summary>
Motivation: 解决传感器深度图中不可靠区域的自动检测和几何结构保持问题。

Method: 第一阶段利用扩散模型的随机性识别不可靠区域并推断几何结构；第二阶段基于不确定性图进行确定性细化，确保结构一致性和像素级精度。
...
### [158] [Deep Learning Reforms Image Matching: A Survey and Outlook](https://arxiv.org/abs/2506.04619)
*Shihua Zhang,Zizhuo Li,Kaining Zhang,Yifan Lu,Yuxin Deng,Linfeng Tang,Xingyu Jiang,Jiayi Ma*

Main category: cs.CV

TL;DR: 本文综述了深度学习如何逐步改进传统图像匹配流程，包括替换可学习模块和端到端学习，并评估了代表性方法。
...
Method: 分类讨论可学习模块替换和端到端学习，评估其在相对位姿恢复、单应性估计和视觉定位任务中的表现。

Result: 深度学习显著提升了图像匹配的性能，但仍存在挑战。

Conclusion: 本文系统分类和评估了深度学习方法，为未来研究指明了方向。
...
### [159] [Unfolding Spatial Cognition: Evaluating Multimodal Models on Visual Simulations](https://arxiv.org/abs/2506.04633)
*Linjie Li,Mahtab Bigverdi,Jiawei Gu,Zixian Ma,Yinuo Yang,Ziang Li,Yejin Choi,Ranjay Krishna*

Main category: cs.CV

TL;DR: STARE是一个评估多模态大语言模型在空间认知任务中表现的基准，涵盖几何变换、空间推理和现实世界应用。模型在简单任务中表现良好，但在复杂任务中接近随机水平，且视觉模拟对模型帮助有限。
...
Method: STARE包含4K任务，涵盖几何变换（2D和3D）、集成空间推理（如立方体展开和七巧板）和现实世界空间推理（如透视和时间推理）。

Result: 模型在简单2D任务中表现良好，但在复杂任务（如3D立方体展开和七巧板）中表现接近随机水平。人类表现优异但耗时较长，视觉模拟可显著提升效率。模型对视觉模拟的利用不一致。
...
### [160] [Text-Aware Real-World Image Super-Resolution via Diffusion Model with Joint Segmentation Decoders](https://arxiv.org/abs/2506.04641)
...
<details>
  <summary>Details</summary>
Motivation: 生成模型在图像超分辨率中常导致文本结构失真，需解决这一问题。

Method: 提出TADiSR框架，结合文本感知注意力和联合分割解码器，并设计合成高质量图像的完整流程。

Result: 实验表明，该方法显著提升文本可读性，在多项指标上达到最优。
...
### [161] [FPSAttention: Training-Aware FP8 and Sparsity Co-Design for Fast Video Diffusion](https://arxiv.org/abs/2506.04648)
...
<details>
  <summary>Details</summary>
Motivation: 扩散生成模型虽然能生成高质量视频，但推理速度慢且计算需求高，限制了实际应用。

Method: 提出FPSAttention，结合FP8量化和稀疏化，引入3D分块粒度、去噪步感知策略和硬件友好内核。

Result: 在VBench基准测试中，FPSAttention实现了7.09倍注意力操作加速和4.96倍端到端视频生成加速，且不损失质量。
...
### [162] [Feature-Based Lie Group Transformer for Real-World Applications](https://arxiv.org/abs/2506.04668)
...
<details>
  <summary>Details</summary>
Motivation: 传统表示学习假设解耦的独立特征轴是好的表示，但无法解释条件独立性。本文旨在通过群分解理论解决这一问题，并扩展到真实世界场景。
...
### [163] [Interpretable Few-Shot Image Classification via Prototypical Concept-Guided Mixture of LoRA Experts](https://arxiv.org/abs/2506.04673)
*Zhong Ji,Rongshuai Wei,Jingren Liu,Yanwei Pang,Jungong Han*

Main category: cs.CV

TL;DR: 论文提出了一种Few-Shot Prototypical Concept Classification (FSPCC)框架，通过参数高效适应和多级特征保留策略，解决了数据稀缺场景下自解释模型（SEMs）的性能问题，并在多个基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 自解释模型（SEMs）在数据稀缺场景下表现不佳，主要由于参数不平衡和表示不对齐问题。本文旨在通过Few-Shot Prototypical Concept Classification (FSPCC)框架解决这些问题。

Method: 1. 使用Mixture of LoRA Experts (MoLE)实现参数高效适应；2. 跨模块概念指导确保特征表示与原型的对齐；3. 多级特征保留策略融合空间和语义线索；4. 引入几何感知概念判别损失以减少概念重叠。

Result: 在六个基准测试中，FSPCC框架相对现有SEMs提升了4.2%-8.7%的分类准确率。

Conclusion: FSPCC框架通过结合概念学习和Few-Shot适应，不仅提高了准确性，还增强了模型的可解释性，为透明视觉识别系统铺平了道路。
...
### [164] [MARS: Radio Map Super-resolution and Reconstruction Method under Sparse Channel Measurements](https://arxiv.org/abs/2506.04682)
...
### [165] [Spatiotemporal Contrastive Learning for Cross-View Video Localization in Unstructured Off-road Terrains](https://arxiv.org/abs/2506.05250)
*Zhiyun Deng,Dongmyeong Lee,Amanda Adkins,Jesse Quattrociocchi,Christian Ellis,Joydeep Biswas*

Main category: cs.CV

TL;DR: MoViX是一种自监督的跨视角视频定位框架，用于GPS缺失的越野环境中，通过学习视角和季节不变表示实现鲁棒定位。


<details>
  <summary>Details</summary>
Motivation: 解决GPS缺失环境下因重复植被、非结构化地形和季节性变化导致的定位困难。

Method: 采用自监督学习框架，结合姿态依赖的正样本采样、时间对齐的硬负样本挖掘、运动信息帧采样器和轻量级时间聚合器。

Result: 在TartanDrive 2.0数据集上，93%的定位误差在25米内，100%在50米内，优于现有方法。

Conclusion: MoViX在跨视角定位中表现出色，无需环境特定调优，并具有泛化能力。
...
### [166] [HoliSafe: Holistic Safety Benchmarking and Modeling with Safety Meta Token for Vision-Language Model](https://arxiv.org/abs/2506.04704)
*Youngwan Lee,Kangsan Kim,Kwanyong Park,Ilcahe Jung,Soojin Jang,Seanie Lee,Yong-Ju Lee,Sung Ju Hwang*

Main category: cs.CV

TL;DR: 论文提出了HoliSafe数据集和SafeLLaVA模型，解决了现有视觉语言模型（VLM）安全性的不足，通过全面覆盖图像-文本组合和引入新的架构创新，显著提升了模型的安全性。


<details>
  <summary>Details</summary>
Motivation: 现有VLM安全方法存在两个主要问题：数据集和基准测试覆盖不全面，且缺乏架构创新。论文旨在填补这些空白，提升VLM的安全性。
...
### [167] [Rectified Point Flow: Generic Point Cloud Pose Estimation](https://arxiv.org/abs/2506.05282)
*Tao Sun,Liyuan Zhu,Shengyu Huang,Shuran Song,Iro Armeni*

Main category: cs.CV

TL;DR: Rectified Point Flow 是一种统一的参数化方法，将点云配准和多部件形状组装建模为条件生成问题，通过学习连续点速度场实现目标定位。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法在点云配准和形状组装中对称性处理不足的问题，并实现无监督学习。

Method: 通过自监督编码器和连续点速度场学习，无需对称标签即可处理对称性。

Result: 在六个基准测试中达到最新性能，统一框架支持多数据集联合训练。

Conclusion: 该方法通过共享几何先验提升了准确性，为点云处理提供了新思路。
...
### [168] [Line of Sight: On Linear Representations in VLLMs](https://arxiv.org/abs/2506.04706)
*Achyuta Rajaram,Sarah Schwettmann,Jacob Andreas,Arthur Conmy*

Main category: cs.CV

TL;DR: 论文探讨了多模态语言模型中图像概念的表示方式，发现线性可解码特征存在于残差流中，并通过训练稀疏自编码器提升特征多样性。
...
Method: 使用线性解码和稀疏自编码器分析LlaVA-Next模型的图像表示，并进行目标编辑验证特征的因果性。

Result: 发现图像和文本表示在深层逐渐共享，但初始模态间表示较为分离。

Conclusion: 多模态模型的表示在不同模态间逐渐融合，稀疏自编码器可提升特征多样性和可解释性。
...
### [169] [Robust Few-Shot Vision-Language Model Adaptation](https://arxiv.org/abs/2506.04713)
*Hanxin Wang,Tian Liu,Shu Kong*

Main category: cs.CV

TL;DR: 论文研究了预训练视觉语言模型（VLM）在少样本适应中的鲁棒性问题，提出了一种新的部分微调方法SRAPF，显著提升了分布内（ID）和分布外（OOD）的准确性。


<details>
  <summary>Details</summary>
Motivation: 预训练VLM在少样本适应中遇到OOD数据时性能下降，研究旨在提升其鲁棒性。

Method: 比较了多种适应方法，提出部分视觉编码器微调，并结合检索增强和对抗扰动技术。最终开发了SRAPF方法，分阶段优化。

Result: SRAPF在ImageNet OOD基准测试中达到了最先进的ID和OOD准确性。

Conclusion: SRAPF通过分阶段优化和结合两种增强技术，有效平衡了ID和OOD性能。
...
### [170] [Towards Holistic Visual Quality Assessment of AI-Generated Videos: A LLM-Based Multi-Dimensional Evaluation Model](https://arxiv.org/abs/2506.04715)
...
Method: 将AIGV视觉质量分解为技术质量、运动质量和视频语义三个维度，设计对应编码器提取特征，并引入LLM作为质量回归模块，结合多模态提示工程框架和LoRA微调技术。
...
### [171] [Learning dissection trajectories from expert surgical videos via imitation learning with equivariant diffusion](https://arxiv.org/abs/2506.04716)
...
TL;DR: 提出了一种名为iDPOE的新方法，用于预测内镜黏膜下剥离术（ESD）视频中的解剖轨迹，通过结合扩散模型和等变表示，提升了模仿学习的准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 预测ESD视频中的解剖轨迹对手术技能培训和学习过程有重要意义，但目前研究不足，且现有模仿学习方法在处理不确定性、几何对称性和泛化性方面存在挑战。

Method: 提出iDPOE方法，通过联合状态动作分布建模专家行为，结合扩散模型和等变表示，优化策略学习和条件采样。

Result: 在近2000个ESD视频片段的数据集上，iDPOE在轨迹预测方面超越了现有方法。

Conclusion: iDPOE是首次将模仿学习应用于解剖轨迹预测的手术技能开发，展现了显著优势。
...
### [172] [Using In-Context Learning for Automatic Defect Labelling of Display Manufacturing Data](https://arxiv.org/abs/2506.04717)
*Babar Hussain,Qiang Liu,Gang Chen,Bihai She,Dahai Yu*

Main category: cs.CV

TL;DR: 本文提出了一种基于AI的显示面板缺陷自动标注系统，利用上下文学习能力，改进了SegGPT架构并引入涂鸦标注机制，显著提升了标注效率和模型性能。


<details>
  <summary>Details</summary>
Motivation: 工业检测系统中手动标注成本高，需要一种高效的自动标注解决方案以减少人工工作量。

Method: 采用两阶段训练方法，结合领域特定技术和涂鸦标注机制，改进SegGPT架构。
...
### [173] [Bridging Annotation Gaps: Transferring Labels to Align Object Detection Datasets](https://arxiv.org/abs/2506.04737)
...
<details>
  <summary>Details</summary>
Motivation: 多数据集结合可提升泛化能力，但标注语义和边界框不一致阻碍了其应用。现有方法或假设共享标签空间，或需手动重标注，难以满足固定目标标签空间需求。

Method: LAT通过训练数据集特定检测器生成伪标签，结合特权提案生成器（PPG）和语义特征融合（SFF）模块，实现标签空间转移和特征优化。

Result: LAT在多个基准测试中显著提升目标域检测性能，最高提升+4.8AP。

Conclusion: LAT无需共享标签空间或手动标注，即可解决类别和空间标注不一致问题，适用于异构数据集训练。
...
### [174] [SRD: Reinforcement-Learned Semantic Perturbation for Backdoor Defense in VLMs](https://arxiv.org/abs/2506.04743)
*Shuhan Xu,Siyuan Liang,Hongling Zheng,Yong Luo,Aishan Liu,Dacheng Tao*

Main category: cs.CV

TL;DR: 论文提出了一种针对视觉语言模型（VLM）后门攻击的防御方法，通过强化学习框架（SRD）降低攻击成功率至5.6%，同时保持正常输入的生成质量。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在图像描述任务中表现优异，但易受后门攻击，攻击者通过注入微小扰动控制模型输出恶意描述。现有防御方法难以应对其隐蔽性和跨模态特性。

Method: 提出语义奖励防御（SRD），利用深度Q网络学习对敏感图像区域施加离散扰动（如遮挡、颜色掩码），以破坏恶意路径激活。通过语义一致性评分作为奖励信号，指导生成鲁棒且准确的描述。

Result: 实验表明，SRD将攻击成功率降至5.6%，且在干净输入上的性能下降小于10%。

Conclusion: SRD为多模态生成模型中的后门威胁提供了一种无需触发器先验知识、可解释的防御范式。
...
### [175] [Physics Informed Capsule Enhanced Variational AutoEncoder for Underwater Image Enhancement](https://arxiv.org/abs/2506.04753)
...
Method: 采用双流架构，分别估计传输图和背景光，并通过胶囊聚类学习特征表示。
...
### [176] [Toward Better SSIM Loss for Unsupervised Monocular Depth Estimation](https://arxiv.org/abs/2506.04758)
...
### [177] [HypeVPR: Exploring Hyperbolic Space for Perspective to Equirectangular Visual Place Recognition](https://arxiv.org/abs/2506.04764)
*Suhan Woo,Seongwon Lee,Jinwoo Jang,Euntai Kim*

Main category: cs.CV

TL;DR: HypeVPR是一种在双曲空间中设计的层次嵌入框架，用于解决P2E VPR的独特挑战，通过层次特征聚合和高效搜索策略，显著提升了性能和速度。


<details>
  <summary>Details</summary>
Motivation: 现实世界中移动机器人的视觉地点识别（VPR）需要适应不同视角的查询图像，P2E方法成为自然选择，但面临独特挑战。

Method: 采用双曲空间表示层次特征关系，提出层次特征聚合机制和粗到细的搜索策略。

Result: 在多个基准数据集上表现优于现有方法，检索速度提升5倍。

Conclusion: HypeVPR通过层次化设计和高效搜索策略，显著提升了P2E VPR的性能和效率。
...
### [178] [Object-X: Learning to Reconstruct Multi-Modal 3D Object Representations](https://arxiv.org/abs/2506.04789)
*Gaia Di Lorenzo,Federico Tombari,Marc Pollefeys,Daniel Barath*

Main category: cs.CV

TL;DR: Object-X是一种多模态3D对象表示框架，能够编码丰富对象嵌入并解码为几何和视觉重建，支持多种下游任务，且存储需求极低。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖任务特定的嵌入，无法同时支持几何重建和跨任务复用，因此需要一种更通用的多模态3D表示方法。

Method: Object-X通过将多模态数据（如图像、点云、文本）几何地嵌入3D体素网格，并学习融合体素与对象属性的非结构化嵌入，支持基于3D高斯泼溅的对象重建。

Result: 在真实数据集上，Object-X实现了高保真新视角合成，几何精度显著提升，并在场景对齐和定位任务中表现优异，存储需求比传统方法低3-4个数量级。
...
### [179] [LotusFilter: Fast Diverse Nearest Neighbor Search via a Learned Cutoff Table](https://arxiv.org/abs/2506.04790)
*Yusuke Matsui*

Main category: cs.CV

TL;DR: LotusFilter是一种后处理模块，用于多样化近似最近邻搜索（ANNS）结果，通过预计算相邻向量表快速删除冗余向量。


<details>
  <summary>Details</summary>
Motivation: ANNS在某些应用（如RAG）中可能返回过于相似的结果，而实际需求是结果既相似又多样。

Method: 预计算相邻向量表，并在过滤阶段贪婪查找以删除冗余向量。
...
### [180] [SupeRANSAC: One RANSAC to Rule Them All](https://arxiv.org/abs/2506.04803)
*Daniel Barath*

Main category: cs.CV

TL;DR: SupeRANSAC是一种新型统一的RANSAC流程，旨在提高计算机视觉任务中几何模型估计的鲁棒性和一致性。


<details>
  <summary>Details</summary>
Motivation: RANSAC及其变体在计算机视觉中是几何模型估计的金标准，但不同库和实现细节导致性能不一致。

Method: 提出SupeRANSAC，分析并优化RANSAC的关键技术，适用于多种视觉任务（如单应性、基础矩阵、绝对姿态估计）。

Result: SupeRANSAC在多个任务和数据集上显著优于现有方法，例如在基础矩阵估计上平均提高6 AUC点。

Conclusion: SupeRANSAC通过统一流程和优化技术，实现了跨任务的高性能几何模型估计。
...
### [181] [MegaHan97K: A Large-Scale Dataset for Mega-Category Chinese Character Recognition with over 97K Categories](https://arxiv.org/abs/2506.04807)
*Yuyi Zhang,Yongxin Shi,Peirong Zhang,Yixin Zhao,Zhenhua Yang,Lianwen Jin*

Main category: cs.CV

TL;DR: 论文介绍了MegaHan97K数据集，支持87,887个汉字类别，解决了大类别识别问题，并揭示了新挑战。


<details>
  <summary>Details</summary>
Motivation: 中文汉字类别庞大且不断扩展，现有OCR数据集无法满足大类别识别需求，影响文化遗产保护和数字应用。

Method: 构建MegaHan97K数据集，覆盖97,455个汉字类别，包含手写、历史和合成子集，解决长尾分布问题。

Result: 数据集支持GB18030-2022标准，提供平衡样本，揭示存储需求、形态相似字符识别和零样本学习等新挑战。

Conclusion: MegaHan97K是OCR和模式识别领域类别最多的数据集，为未来研究提供重要资源。
...
### [182] [Spike-TBR: a Noise Resilient Neuromorphic Event Representation](https://arxiv.org/abs/2506.04817)
*Gabriele Magrini. Federico Becattini,Luca Cultrera,Lorenzo Berlincioni,Pietro Pala,Alberto Del Bimbo*

Main category: cs.CV

TL;DR: Spike-TBR是一种基于时间二进制表示（TBR）的事件编码策略，通过整合脉冲神经元提高抗噪性，结合了TBR的帧式优势和脉冲神经网络的噪声过滤能力。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有高时间分辨率、低延迟和动态范围等优势，但如何高效且抗噪地转换事件流以适应标准计算机视觉流程仍是一个挑战。

Method: 提出Spike-TBR，结合TBR的帧式编码和脉冲神经网络的噪声过滤能力，设计了四种不同脉冲神经元的变体。

Result: 在多个数据集上测试，Spike-TBR在噪声环境下表现优异，同时在干净数据上也有提升。
...
### [183] [Fool the Stoplight: Realistic Adversarial Patch Attacks on Traffic Light Detectors](https://arxiv.org/abs/2506.04823)
...
<details>
  <summary>Details</summary>
Motivation: 目前对自动驾驶车辆摄像头感知任务的对抗攻击研究较多，但对交通灯检测的攻击研究较少。

Method: 提出了一种威胁模型，通过在交通灯下方放置对抗性补丁，并设计了一种训练策略。

Result: 实验成功实现了目标标签翻转攻击（如红变绿）和图形分类攻击，并在实际场景中验证了攻击效果。

Conclusion: 研究表明，对抗性补丁攻击对交通灯检测系统具有实际威胁，需引起重视。
...
### [184] [DualX-VSR: Dual Axial Spatial$\times$Temporal Transformer for Real-World Video Super-Resolution without Motion Compensation](https://arxiv.org/abs/2506.04830)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的视频超分辨率模型因token化和注意力机制导致像素级精度不足，且依赖光流对齐，实际效果受限。

Method: 提出DualX-VSR模型，采用双轴时空注意力机制，沿正交方向整合时空信息，简化结构。

Result: DualX-VSR在真实视频超分辨率任务中实现高保真和卓越性能。

Conclusion: DualX-VSR通过新颖的双轴注意力机制，解决了现有模型的局限性，提升了视频超分辨率的效果。
...
### [185] [OpenMaskDINO3D : Reasoning 3D Segmentation via Large Language Model](https://arxiv.org/abs/2506.04837)
*Kunshen Zhang*
...
<details>
  <summary>Details</summary>
Motivation: 当前感知系统在2D推理分割方面已成熟，但缺乏类似框架处理3D任务。本文旨在填补这一空白。
...
### [186] [Geological Field Restoration through the Lens of Image Inpainting](https://arxiv.org/abs/2506.04869)
*Vladislav Trifonov,Ivan Oseledets,Ekaterina Muravleva*

Main category: cs.CV

TL;DR: 论文提出了一种基于多维张量低秩结构的稀疏观测地质场重建方法，优于普通克里金法。


<details>
  <summary>Details</summary>
Motivation: 从确定性图像修复技术中获得灵感，解决稀疏观测地质场重建问题。

Method: 结合张量补全和地质统计学，建立全局低秩结构的优化框架。

Result: 在合成地质场实验中，该方法在不同观测比例下均显著优于普通克里金法。

Conclusion: 该方法为地质场重建提供了一种高效且准确的新视角。
...
### [187] [Invisible Backdoor Triggers in Image Editing Model via Deep Watermarking](https://arxiv.org/abs/2506.04879)
*Yu-Feng Chen,Tzuhsuan Huang,Pin-Yen Chiu,Jun-Cheng Chen*

Main category: cs.CV

TL;DR: 本文提出了一种在图像编辑中嵌入隐形触发器的后门攻击框架，利用水印模型实现攻击，同时保持对干净图像的正常编辑。
...
Result: 在不同水印模型上实验表明，该方法攻击成功率高，且水印特性分析支持其有效性。

Conclusion: 提出的框架在图像编辑中实现了高效的后门攻击，同时保持对干净图像的正常编辑。
...
### [188] [Learning to Plan via Supervised Contrastive Learning and Strategic Interpolation: A Chess Case Study](https://arxiv.org/abs/2506.04892)
...
<details>
  <summary>Details</summary>
Motivation: 模仿人类棋手依赖直觉而非深度搜索的规划过程，探索潜在空间规划作为传统搜索的替代方案。

Method: 使用监督对比学习训练Transformer编码器，将棋盘状态嵌入到按位置评估结构化的潜在空间，距离反映评估相似性。
...
### [189] [From Objects to Anywhere: A Holistic Benchmark for Multi-level Visual Grounding in 3D Scenes](https://arxiv.org/abs/2506.04897)
*Tianxu Wang,Zhuofan Zhang,Ziyu Zhu,Yue Fan,Jing Xiong,Pengxiang Li,Xiaojian Ma,Qing Li*

Main category: cs.CV

TL;DR: Anywhere3D-Bench是一个全面的3D视觉基准测试，涵盖四种不同级别的视觉定位任务，揭示了当前模型在空间和部分级别任务上的显著不足。
...
Method: 引入Anywhere3D-Bench基准测试，评估多种3D视觉定位方法和大型语言模型在不同级别任务上的表现。

Result: 空间和部分级别任务表现最差，最佳模型OpenAI o4-mini的准确率分别为23.57%和33.94%。
...
### [190] [Generating Synthetic Stereo Datasets using 3D Gaussian Splatting and Expert Knowledge Transfer](https://arxiv.org/abs/2506.04908)
*Filip Slezak,Magnus K. Gjerde,Joakim B. Haurum,Ivan Nikolov,Morten S. Laursen,Thomas B. Moeslund*

Main category: cs.CV

TL;DR: 提出了一种基于3D高斯泼溅（3DGS）的立体数据集生成方法，相比NeRF方法更高效，并探索了利用显式3D表示和FoundationStereo模型的深度估计来优化几何估计。
...
Method: 结合3DGS生成的几何和FoundationStereo的深度估计，通过专家知识转移优化几何估计。

Result: 3DGS生成的数据集在零样本泛化基准测试中表现良好，但直接使用重建几何会引入噪声；FoundationStereo的视差估计更干净，性能更优。
...
### [191] [Light and 3D: a methodological exploration of digitisation techniques adapted to a selection of objects from the Mus{é}e d'Arch{é}ologie Nationale](https://arxiv.org/abs/2506.04925)
...
<details>
  <summary>Details</summary>
Motivation: 文化遗产数字化需求日益增长，但现有3D数字化方法无法适用于所有对象，需根据具体情况灵活选择。

Method: 通过法国国家考古博物馆的藏品案例，分析不同3D摄影数字化方法的适用性，强调跨领域合作选择工具。

Result: 研究表明，没有单一方法适用于所有对象，需根据对象特性和未来用途定制数字化工具。

Conclusion: 文化遗产数字化应避免绝对分类，而是通过跨领域合作，为每个对象选择最适合的数字化方法。
...
### [192] [CzechLynx: A Dataset for Individual Identification and Pose Estimation of the Eurasian Lynx](https://arxiv.org/abs/2506.04931)
...
<details>
  <summary>Details</summary>
Motivation: 为欧亚猞猁的个体识别、姿态估计和分割提供首个大规模数据集，支持跨时空域的泛化测试。

Method: 收集了30k真实相机陷阱图像，并生成100k合成图像，使用Unity和扩散模型增强数据多样性。定义了三种评估协议：地理感知、时间感知开放集和时间感知封闭集。

Result: 数据集覆盖219个独特个体，跨越15年监测，涵盖两个地理区域，并提供了丰富的标注信息。
...
### [193] [Time-Lapse Video-Based Embryo Grading via Complementary Spatial-Temporal Pattern Mining](https://arxiv.org/abs/2506.04950)
*Yong Sun,Yipeng Wang,Junyu Shi,Zhiyuan Zhang,Yanmei Xiao,Lei Zhu,Manxi Jiang,Qiang Nie*

Main category: cs.CV

TL;DR: 提出了一种基于视频的胚胎分级任务，利用全时长延时监测视频预测胚胎质量，并设计了一个互补时空模式挖掘框架（CoSTeM）。
...
Method: 提出CoSTeM框架，包含形态学和形态动力学分支，分别提取局部结构特征和全局发育轨迹。

Result: 实验证明该框架优于现有方法，为AI辅助胚胎选择提供了方法论支持。
...
### [194] [Robustness as Architecture: Designing IQA Models to Withstand Adversarial Perturbations](https://arxiv.org/abs/2506.04951)
*Igor Meleshin,Anna Chistyakova,Anastasia Antsiferova,Dmitriy Vatolin*

Main category: cs.CV

TL;DR: 论文提出了一种新的视角，通过设计而非学习来实现图像质量评估（IQA）模型的鲁棒性，提出了一种基于架构先验的鲁棒性方法。


<details>
  <summary>Details</summary>
Motivation: 传统的数据驱动防御方法（如对抗训练）可能并非解决IQA模型脆弱性的最佳途径，论文探讨了通过设计模型内部结构来提升鲁棒性的可能性。

Method: 通过强制正交信息流和规范保持操作来重塑模型内部结构，并结合剪枝和微调进一步稳定系统。

Result: 提出了一种鲁棒的IQA架构，能够在不依赖对抗训练或大幅修改原模型的情况下抵御对抗攻击。

Conclusion: 论文倡导从数据优化转向设计优化，为提升模型鲁棒性提供了新的思路。
...
### [195] [APVR: Hour-Level Long Video Understanding with Adaptive Pivot Visual Information Retrieval](https://arxiv.org/abs/2506.04953)
*Hong Gao,Yiming Bao,Xuezhan Tu,Bin Zhong,Minling Zhang*

Main category: cs.CV

TL;DR: APVR框架通过分层视觉信息检索解决长视频理解问题，无需训练，支持小时级视频处理。
...
Result: 在LongVideoBench和VideoMME上表现优异，达到SOTA结果，支持即插即用。
...
### [196] [FEAT: Full-Dimensional Efficient Attention Transformer for Medical Video Generation](https://arxiv.org/abs/2506.04956)
...
Method: 提出FEAT，采用空间-时间-通道注意力机制、线性复杂度设计和残差值引导模块，以高效捕获全局依赖并适应不同噪声水平。

Result: FEAT-S参数仅为Endora的23%，但性能相当或更优；FEAT-L在多个数据集上超越所有对比方法。

Conclusion: FEAT在动态医学视频合成中表现出高效性和可扩展性，为相关任务提供了新思路。
...
### [197] [Bringing SAM to new heights: Leveraging elevation data for tree crown segmentation from drone imagery](https://arxiv.org/abs/2506.04970)
...
Conclusion: SAM结合DSM和端到端调优是树冠分割的可行方向，尤其在人工林中。
...
### [198] [TextVidBench: A Benchmark for Long Video Scene Text Understanding](https://arxiv.org/abs/2506.04983)
...
<details>
  <summary>Details</summary>
Motivation: 现有短视频文本视觉问答（ViteVQA）数据集在视频时长和评估范围上存在局限，难以充分评估多模态大语言模型（MLLMs）的能力。

Method: 1) 引入TextVidBench基准，覆盖9个领域的长视频（平均2306秒）；2) 提出三阶段评估框架；3) 提供高质量细粒度标注。改进方法包括IT-Rope机制、非均匀位置编码和轻量级微调。

Result: 实验表明，TextVidBench对现有模型提出了显著挑战，提出的方法有效提升了长视频文本理解能力。

Conclusion: TextVidBench为长视频文本问答提供了更真实的评估基准，提出的方法为改进大模型的长视频理解能力提供了新思路。
...
### [199] [Multi-scale Image Super Resolution with a Single Auto-Regressive Model](https://arxiv.org/abs/2506.04990)
*Enrique Sanchez,Isma Hadji,Adrian Bulat,Christos Tzelepis,Brais Martinez,Georgios Tzimiropoulos*

Main category: cs.CV

TL;DR: 本文提出了一种基于视觉自回归（VAR）模型的图像超分辨率（ISR）方法，通过多尺度图像标记化和偏好优化，解决了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有VARSR方法存在固定分辨率和依赖大规模数据的问题，本文旨在通过改进标记化和优化方法提升性能。

Method: 采用分层图像标记化和直接偏好优化（DPO）两项新技术，实现多尺度超分辨率。

Result: 模型在单次前向传播中实现去噪和超分辨率，性能达到SOTA，且模型更小、无需外部数据。

Conclusion: 本文方法在ISR任务中表现出色，解决了现有VARSR的局限性，为多尺度超分辨率提供了新思路。
...
### [200] [PATS: Proficiency-Aware Temporal Sampling for Multi-View Sports Skill Assessment](https://arxiv.org/abs/2506.04996)
*Edoardo Bianchi,Antonio Liotta*

Main category: cs.CV

TL;DR: PATS是一种新型视频采样策略，专注于保持运动技能的完整时间连续性，显著提升了自动化技能评估的准确性。
...
Method: 提出PATS策略，自适应分割视频以确保每个片段包含完整的关键运动执行，并在多视点评估中重复此过程。

Result: 在EgoExo4D基准测试中，PATS在所有视点配置下均优于现有方法，尤其在动态运动和高频技能中表现突出。

Conclusion: PATS作为一种自适应时间采样方法，有效提升了自动化技能评估的实用性，适用于多样化运动场景。
...
### [201] [Beyond Cropped Regions: New Benchmark and Corresponding Baseline for Chinese Scene Text Retrieval in Diverse Layouts](https://arxiv.org/abs/2506.04999)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理中文复杂多样的文本布局时表现不佳，需要专门针对中文场景的解决方案。

Method: 提出CSTR-CLIP模型，采用两阶段训练，结合全局视觉信息和多粒度对齐。

Result: CSTR-CLIP在现有基准上性能提升18.82%，推理速度更快。
...
### [202] [Structure-Aware Radar-Camera Depth Estimation](https://arxiv.org/abs/2506.05008)
*Fuyi Zhang,Zhu Yu,Chunhao Li,Runmin Zhang,Xiaokai Bai,Zili Zhou,Si-Yuan Cao,Wang Wang,Hui-Liang Shen*

Main category: cs.CV

TL;DR: 论文探讨了单目深度估计的进展，重点介绍了深度学习方法及其在未见领域泛化的挑战。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计的目标是从RGB图像中预测每个像素的深度，但由于缺乏显式深度线索，泛化到未见领域仍具挑战性。

Method: 研究采用了多尺度融合网络、分类任务重新解释、附加先验和更有效的目标函数等方法。

Result: Depth Anything在零样本单目深度估计中表现领先，尤其在提取结构信息方面表现出色。

Conclusion: 尽管在未见领域取得进展，但准确度量深度估计仍是一个挑战。
...
### [203] [Point Cloud Segmentation of Agricultural Vehicles using 3D Gaussian Splatting](https://arxiv.org/abs/2506.05009)
...
TL;DR: 提出了一种利用3D高斯泼溅和高斯不透明度场生成合成数据的管道，用于训练3D点云语义分割模型，实验表明合成数据训练效果优异。


<details>
  <summary>Details</summary>
Motivation: 获取和标注真实点云数据成本高且耗时，需要一种高效生成合成数据的方法。

Method: 结合3DGS和GOF生成农业车辆3D资产，通过模拟LiDAR生成点云数据。

Result: PTv3模型在合成数据上训练后mIoU达91.35%，某些情况下优于真实数据训练模型。
...
### [204] [UAV4D: Dynamic Neural Rendering of Human-Centric UAV Imagery using Gaussian Splatting](https://arxiv.org/abs/2506.05011)
*Jaehoon Choi,Dongki Jung,Christopher Maxey,Yonghan Lee,Sungmin Eum,Dinesh Manocha,Heesung Kwon*

Main category: cs.CV

TL;DR: UAV4D框架解决了无人机单目视频动态场景重建的挑战，通过结合3D基础模型和人体网格重建模型，实现了多移动行人的真实感渲染。


<details>
  <summary>Details</summary>
Motivation: 现有动态神经渲染方法在无人机单目视频场景中表现不足，特别是针对多移动行人和俯视角的场景。

Method: 结合3D基础模型和人体网格重建模型，利用SMPL模型和背景网格初始化高斯泼溅，解决场景尺度模糊问题。

Result: 在三个复杂无人机数据集上测试，PSNR提升1.5 dB，视觉清晰度优于现有方法。

Conclusion: UAV4D框架在多移动行人动态场景中实现了显著的渲染质量提升。
...
### [205] [Physical Annotation for Automated Optical Inspection: A Concept for In-Situ, Pointer-Based Trainingdata Generation](https://arxiv.org/abs/2506.05026)
...
<details>
  <summary>Details</summary>
Motivation: 传统屏幕标注方法效率低且不直观，无法充分利用人工检查员的专业知识。该系统旨在通过物理交互直接捕捉标注轨迹，提升数据标注的效率和准确性。

Method: 使用校准的跟踪指针记录用户输入，并通过投影界面提供视觉引导，将物理交互转换为标准化标注格式。

Result: 初步评估证实了系统可行性，能够捕捉详细标注轨迹，并与CVAT集成优化后续机器学习任务。

Conclusion: 该系统填补了人工专业知识和自动化数据生成之间的空白，为非IT专家参与机器学习训练提供了可能。
...
### [206] [FlowDirector: Training-Free Flow Steering for Precise Text-to-Video Editing](https://arxiv.org/abs/2506.05046)
*Guangzhao Li,Yanming Yang,Chenxi Song,Chi Zhang*

Main category: cs.CV

TL;DR: FlowDirector提出了一种无需反转的视频编辑框架，通过ODE直接演化数据空间，保持时间一致性和结构细节。
...
### [207] [A Survey on Vietnamese Document Analysis and Recognition: Challenges and Future Directions](https://arxiv.org/abs/2506.05061)
*Anh Le,Thanh Lam,Dung Nguyen*

Main category: cs.CV

TL;DR: 本文综述了越南文档分析与识别（DAR）的现状，探讨了其面临的独特挑战，如复杂变音符号和数据集稀缺，并提出了利用大语言模型（LLMs）和多模态学习的新方向。


<details>
  <summary>Details</summary>
Motivation: 越南DAR在数字化和信息检索中至关重要，但传统OCR方法因复杂变音符号和数据集不足而受限，亟需新方法推动发展。

Method: 综述现有技术，分析LLMs和多模态学习在越南DAR中的应用潜力。

Result: LLMs和多模态学习为越南DAR带来新机遇，但仍需解决领域适应和计算效率等问题。

Conclusion: 未来研究方向包括数据集开发、模型优化和多模态整合，以推动越南DAR的进步。
...
### [208] [SeedEdit 3.0: Fast and High-Quality Generative Image Editing](https://arxiv.org/abs/2506.05083)
...
<details>
  <summary>Details</summary>
Motivation: 提升图像编辑工具的指令遵循能力和内容保留效果，尤其是在真实图像输入上。

Method: 1. 改进数据管道，引入元信息嵌入策略；2. 联合学习扩散损失和奖励损失。

Result: 在测试基准中，SeedEdit 3.0 实现了56.1%的可用性，优于其他版本和竞品。

Conclusion: SeedEdit 3.0 在多个方面取得了最佳平衡，显著提升了图像编辑的实用性。
...
### [209] [Interpretable Multimodal Framework for Human-Centered Street Assessment: Integrating Visual-Language Models for Perceptual Urban Diagnostics](https://arxiv.org/abs/2506.05087)
*HaoTian Lan*
...
<details>
  <summary>Details</summary>
Motivation: 传统街道指标无法充分捕捉主观感知，而这对包容性城市设计至关重要。

Method: 结合视觉变换器（VisualGLM-6B）和大型语言模型（GPT-4），使用LoRA和P-Tuning v2进行参数高效微调。

Result: 模型在客观特征上F1得分为0.84，与居民感知一致性达89.3%，并揭示了情境依赖的矛盾和非线性模式。

Conclusion: MSEF为城市感知建模提供了方法创新，并具有实际规划应用价值。
...
### [210] [FG 2025 TrustFAA: the First Workshop on Towards Trustworthy Facial Affect Analysis: Advancing Insights of Fairness, Explainability, and Safety (TrustFAA)](https://arxiv.org/abs/2506.05095)
*Jiaee Cheong,Yang Liu,Harold Soh,Hatice Gunes*

Main category: cs.CV

TL;DR: 该论文摘要介绍了首个关于可信面部情感分析（FAA）的研讨会TrustFAA，旨在探讨公平性、可解释性和安全性等问题。


<details>
  <summary>Details</summary>
Motivation: 随着情感AI驱动的FAA工具普及，其可信度问题日益突出，因此需要研究其公平性、可解释性和安全性。

Method: 通过研讨会形式，汇集研究者探讨FAA中的挑战，如解释性、不确定性、偏见和隐私等。

Result: 研讨会支持FG2025的伦理目标，推动可信FAA的研究与讨论。

Conclusion: TrustFAA旨在促进FAA领域的可信度研究，与FG2025的伦理要求一致。
...
### [211] [Astraea: A GPU-Oriented Token-wise Acceleration Framework for Video Diffusion Transformers](https://arxiv.org/abs/2506.05096)
...
<details>
  <summary>Details</summary>
Motivation: 现有加速方法依赖启发式规则，适用性有限，ASTRAEA旨在通过自动搜索实现高效且通用的优化。

Method: 提出轻量级令牌选择机制和GPU并行稀疏注意力策略，结合进化算法自动分配令牌预算。

Result: 在单GPU上实现2.4倍加速，8GPU上达13.2倍，视频质量损失极小（VBench得分损失<0.5%）。

Conclusion: ASTRAEA在速度和生成质量间取得平衡，为vDiT的实际部署提供了高效解决方案。
...
### [212] [DIMCIM: A Quantitative Evaluation Framework for Default-mode Diversity and Generalization in Text-to-Image Generative Models](https://arxiv.org/abs/2506.05108)
*Revant Teotia,Candace Ross,Karen Ullrich,Sumit Chopra,Adriana Romero-Soriano,Melissa Hall,Matthew J. Muckley*

Main category: cs.CV

TL;DR: 论文提出了DIM-CIM框架，用于无参考地测量文本到图像（T2I）模型的多样性和泛化能力，并构建了COCO-DIMCIM基准测试。


<details>
  <summary>Details</summary>
Motivation: 当前T2I模型在质量和一致性上取得了显著进展，但牺牲了多样性。现有的多样性评估方法需要参考图像数据集或缺乏针对性，限制了其适应性和可解释性。

Method: 提出了DIM-CIM框架，通过"Does-it"和"Can-it"分别测量默认模式多样性和泛化能力，并构建了COCO-DIMCIM基准测试。

Result: 发现广泛使用的模型在参数规模扩大时泛化能力提升但默认模式多样性下降，并识别了细粒度失败案例。训练数据多样性与默认模式多样性相关性为0.85。

Conclusion: DIM-CIM提供了一个灵活且可解释的框架，用于全面评估T2I模型的多样性和泛化能力。
...
### [213] [Practical Manipulation Model for Robust Deepfake Detection](https://arxiv.org/abs/2506.05119)
...
Method: 使用泊松混合、多样化掩码、生成器伪影和干扰物扩展伪伪造空间，并在训练图像中添加强退化以增强检测器的泛化性和鲁棒性。

Result: 在DFDC和DFDCP数据集上，分别实现了3.51%和6.21%的AUC提升，显著优于现有技术。

Conclusion: PMM不仅提升了模型对常见图像退化的鲁棒性，还在标准基准数据集上表现出色，揭示了以往检测器的不足并提供了改进方案。
...
### [214] [CIVET: Systematic Evaluation of Understanding in VLMs](https://arxiv.org/abs/2506.05146)
*Massimo Rizzoli,Simone Alghisi,Olha Khomyn,Gabriel Roccabruna,Seyed Mahed Mousavi,Giuseppe Riccardi*

Main category: cs.CV

TL;DR: CIVET框架系统评估视觉语言模型（VLMs）对场景结构和语义的理解能力，发现现有模型在对象属性、位置依赖和关系理解上表现有限，且远未达到人类水平。
...
Method: 提出CIVET框架，通过可控刺激系统评估五种先进VLMs，排除噪声和偏见。

Result: VLMs仅能识别有限基础属性，性能依赖对象位置，且难以理解对象关系，表现远逊于人类。
...
### [215] [FRED: The Florence RGB-Event Drone Dataset](https://arxiv.org/abs/2506.05163)
...
<details>
  <summary>Details</summary>
Motivation: 传统RGB相机在捕捉快速移动的小型无人机时存在局限，事件相机虽具高时间分辨率和动态范围，但现有基准缺乏精细时间分辨率或无人机特有运动模式。

Method: 提出FRED数据集，包含RGB视频和事件流，涵盖5种无人机模型及恶劣场景（如雨天和不良光照）。

Result: FRED提供了7小时密集标注数据，并制定了详细评估协议和标准指标。
...
### [216] [Through-the-Wall Radar Human Activity Recognition WITHOUT Using Neural Networks](https://arxiv.org/abs/2506.05169)
*Weicheng Gao*

Main category: cs.CV

TL;DR: 论文提出了一种不依赖神经网络的穿墙雷达人体活动识别方法，基于模板匹配和拓扑相似性计算。


<details>
  <summary>Details</summary>
Motivation: 研究者认为当前领域过于依赖神经网络，忽视了早期基于模板匹配的方法，这些方法具有更强的物理可解释性。

Method: 生成雷达的时域图和多普勒图，通过角点检测确定目标区域，使用多相主动轮廓模型分割微多普勒特征，最后通过Mapper算法计算点云拓扑相似性。
...
### [217] [Track Any Anomalous Object: A Granular Video Anomaly Detection Pipeline](https://arxiv.org/abs/2506.05175)
...
Method: 将异常检测问题转化为像素级跟踪，并与下游任务（如分割和跟踪）结合，无需阈值调整。

Result: 实验表明TAO在准确性和鲁棒性上设定了新基准。

Conclusion: TAO框架在复杂视频序列中实现了更精确的异常定位。
...
### [218] [Single GPU Task Adaptation of Pathology Foundation Models for Whole Slide Image Analysis](https://arxiv.org/abs/2506.05184)
...
TL;DR: TAPFM是一种针对病理基础模型（PFMs）的任务适应方法，利用视觉变换器（ViT）注意力进行多实例学习（MIL）聚合，优化特征表示和注意力权重，在单GPU上实现高效适应。


<details>
  <summary>Details</summary>
Motivation: 预训练的PFMs在分析全切片图像（WSIs）时表现强大，但适应特定临床任务时面临挑战，主要由于仅有的弱标签（WSI级别）和需要MIL范式。
...
### [219] [MokA: Multimodal Low-Rank Adaptation for MLLMs](https://arxiv.org/abs/2506.05191)
*Yake Wei,Yu Miao,Dongzhan Zhou,Di Hu*

Main category: cs.CV

TL;DR: 本文提出了一种多模态低秩适应方法（MokA），针对当前多模态微调方法的局限性，通过模态特定参数压缩单模态信息并增强跨模态交互，显著提升了多模态场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有高效多模态微调方法多直接借用LLMs，忽视了多模态场景的内在差异，导致模态利用不充分。

Method: 提出MokA方法，结合单模态适应和跨模态适应，通过模态特定参数压缩单模态信息并增强跨模态交互。

Result: 在多种多模态场景（如视听文本、视觉文本、语音文本）和LLM骨干网络上均表现优异。

Conclusion: MokA为多模态语言模型的高效适应提供了针对性解决方案，具有广泛适用性。
...
### [220] [Vision-Based Autonomous MM-Wave Reflector Using ArUco-Driven Angle-of-Arrival Estimation](https://arxiv.org/abs/2506.05195)
...
<details>
  <summary>Details</summary>
Motivation: 解决在复杂环境中毫米波通信的可靠性问题，尤其是在非视距条件下。

Method: 利用单目摄像头检测ArUco标记，实时估计到达角并调整反射器方向，优化信号重定向。

Result: 实验显示，系统在60GHz频段下平均信号强度增益为23dB，信号接收概率为0.89（阈值-65dB）。
...
### [221] [Quantifying Cross-Modality Memorization in Vision-Language Models](https://arxiv.org/abs/2506.05198)
*Yuxin Wen,Yangsibo Huang,Tom Goldstein,Ravi Kumar,Badih Ghazi,Chiyuan Zhang*

Main category: cs.CV

TL;DR: 研究探讨了神经网络在多模态模型中的跨模态记忆特性，发现信息在不同模态间存在显著转移差距，并提出了一种缓解方法。


<details>
  <summary>Details</summary>
Motivation: 理解神经网络在多模态任务中的记忆行为，对隐私保护和知识获取至关重要。

Method: 使用合成的多模态数据集，训练视觉语言模型，并评估跨模态信息转移能力。

Result: 发现信息在不同模态间可转移，但存在显著差距，且这一现象普遍存在于多种场景中。

Conclusion: 提出了一种缓解跨模态转移差距的基线方法，希望推动更鲁棒的多模态学习技术发展。
...
### [222] [Grounding Beyond Detection: Enhancing Contextual Understanding in Embodied 3D Grounding](https://arxiv.org/abs/2506.05199)
*Yani Zhang,Dongming Wu,Hao Shi,Yingfei Liu,Tiancai Wang,Haoqiang Fan,Xingping Dong*

Main category: cs.CV

TL;DR: 论文探讨了3D物体定位任务中检测模型对语言指令的依赖性，发现检测模型无需专门训练即可超越专门训练的定位模型，进而提出DEGround框架，结合检测与定位任务，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 研究3D物体定位任务是否真正受益于检测模型，发现当前方法可能未充分利用检测模型的潜力。

Method: 提出DEGround框架，共享DETR查询用于检测和定位，并引入区域激活模块和查询调制模块以增强语言理解。

Result: DEGround在EmbodiedScan验证集上超越现有最佳模型BIP3D 7.52%的准确率。

Conclusion: DEGround通过结合检测与定位任务，显著提升了3D物体定位的性能，表明检测模型在任务中的重要性。
...
### [223] [OGGSplat: Open Gaussian Growing for Generalizable Reconstruction with Expanded Field-of-View](https://arxiv.org/abs/2506.05204)
*Yanbo Wang,Ziyi Wang,Wenzhao Zheng,Jie Zhou,Jiwen Lu*

Main category: cs.CV

TL;DR: OGGSplat是一种基于开放高斯生长的方法，用于从稀疏视图重建语义感知的3D场景，解决了现有方法在计算成本和视野扩展上的局限性。


<details>
  <summary>Details</summary>
Motivation: 新兴应用如虚拟现实和具身AI需要从稀疏视图高效重建3D场景，但现有方法需密集视图且计算成本高，通用方法难以处理视野外区域。

Method: 提出OGGSplat，利用开放高斯的语义属性进行图像外推，结合RGB-语义一致性修复模块和双向控制的扩散模型，逐步优化高斯参数。

Result: 建立了高斯外推（GO）基准测试，验证了OGGSplat在语义和生成质量上的优势，并在智能手机拍摄的两视图场景中展示了语义感知重建能力。

Conclusion: OGGSplat在稀疏视图下实现了语义一致且视觉合理的3D场景重建，为通用3D重建提供了高效解决方案。
...
### [224] [Follow-Your-Motion: Video Motion Transfer via Efficient Spatial-Temporal Decoupled Finetuning](https://arxiv.org/abs/2506.05207)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LoRA的运动迁移方法在应用于大型视频扩散变换器时存在运动不一致和调优效率低的问题，亟需改进。

Method: 提出空间-时间解耦的LoRA，解耦空间外观和时间运动的注意力架构；设计稀疏运动采样和自适应RoPE加速调优速度。
...
### [225] [Towards Vision-Language-Garment Models For Web Knowledge Garment Understanding and Generation](https://arxiv.org/abs/2506.05210)
*Jan Ackermann,Kiyohiro Nakayama,Guandao Yang,Tong Wu,Gordon Wetzstein*

Main category: cs.CV

TL;DR: VLG模型通过视觉和语言输入生成服装，初步结果显示其在零样本泛化能力上表现良好。
...
Result: 初步结果表明VLG在未见过的服装风格和提示上具有潜力。
...
### [226] [DSG-World: Learning a 3D Gaussian World Model from Dual State Videos](https://arxiv.org/abs/2506.05217)
*Wenhao Hu,Xuexiang Wen,Xi Li,Gaoang Wang*

Main category: cs.CV

TL;DR: DSG-World提出了一种基于双状态观测的端到端3D高斯世界建模框架，解决了传统方法中因遮挡导致的多阶段处理问题。


<details>
  <summary>Details</summary>
Motivation: 从有限观测中构建高效且物理一致的世界模型是视觉与机器人领域的长期挑战，现有方法存在训练困难或缺乏一致性等问题。

Method: 利用同一场景在不同物体配置下的双状态观测，构建双分割感知高斯场，并通过双向光度与语义一致性约束，结合伪中间状态对称对齐与协作修剪策略。

Result: 实验表明，DSG-World能高效实现真实到仿真的转换，支持高保真渲染和物体级场景操作，且在新视角和场景状态下表现优异。

Conclusion: DSG-World为3D重建与仿真提供了高效且一致的解决方案，无需依赖密集观测或多阶段处理。
...
### [227] [MonkeyOCR: Document Parsing with a Structure-Recognition-Relation Triplet Paradigm](https://arxiv.org/abs/2506.05218)
...
Result: 在MonkeyDoc数据集上表现优异，超越现有方法（如MinerU和Qwen2.5-VL），尤其在公式和表格处理上提升显著。

Conclusion: MonkeyOCR在精度和速度上均达到最先进水平，且部署高效，适合实际应用。
...
### [228] [SAM-aware Test-time Adaptation for Universal Medical Image Segmentation](https://arxiv.org/abs/2506.05221)
...
Result: 在五个公开数据集上，SAM-TTA优于现有TTA方法，甚至在某些场景下超过完全微调的MedSAM。
...
### [229] [LeanPO: Lean Preference Optimization for Likelihood Alignment in Video-LLMs](https://arxiv.org/abs/2506.05260)
...
<details>
  <summary>Details</summary>
Motivation: 现有Video-LLMs采用偏好对齐技术（如DPO）时，常导致目标和非目标响应的对数概率均下降，非目标响应概率意外提升，尤其在视频内容复杂时更严重。

Method: 提出LeanPO方法，重新定义隐式奖励为响应相对于策略模型的平均似然，并设计奖励可信度相关的自生成偏好数据管道和动态标签平滑策略。

Result: 实验表明LeanPO显著提升Video-LLMs性能，且额外训练开销小。

Conclusion: LeanPO为Video-LLMs提供了一种简单有效的偏好对齐方法，提升模型可靠性和效率。
...
### [230] [Can Foundation Models Generalise the Presentation Attack Detection Capabilities on ID Cards?](https://arxiv.org/abs/2506.05263)
*Juan E. Tapia,Christoph Busch*

Main category: cs.CV

TL;DR: 本文探讨了如何利用基础模型（FM）提升ID卡演示攻击检测（PAD）的泛化能力，特别是在面对多国ID卡时的表现。


<details>
  <summary>Details</summary>
Motivation: 当前PAD系统因隐私保护限制，通常仅针对少数ID卡训练，导致在未知国家ID卡上表现不佳。基础模型因其大数据训练特性，有望提升泛化能力。

Method: 研究采用零样本和微调两种方法，测试了两种ID卡数据集（智利私有数据集和芬兰、西班牙、斯洛伐克的开放数据集）。
...
### [231] [From Play to Replay: Composed Video Retrieval for Temporally Fine-Grained Videos](https://arxiv.org/abs/2506.05274)
*Animesh Gupta,Jay Parmar,Ishan Rajendrakumar Dave,Mubarak Shah*

Main category: cs.CV

TL;DR: TF-CoVR是一个专注于时间细粒度视频检索的新基准，通过LLM生成查询-修改对，并提出了一个两阶段训练框架TF-CoVR-Base，显著提升了检索性能。
...
Method: 提出TF-CoVR基准，使用LLM生成查询-修改对，并设计TF-CoVR-Base框架，包括预训练视频编码器和对比学习对齐查询与候选视频。

Result: TF-CoVR-Base在零样本和微调设置下分别将mAP@50从5.92提升至7.51和从19.83提升至25.82。

Conclusion: TF-CoVR填补了时间细粒度视频检索的空白，TF-CoVR-Base框架显著提升了性能，为实际任务如体育集锦生成提供了支持。
...
### [232] [Unifying Appearance Codes and Bilateral Grids for Driving Scene Gaussian Splatting](https://arxiv.org/abs/2506.05280)
...
<details>
  <summary>Details</summary>
Motivation: 现实场景中难以保证完美的光度一致性，现有方法（如外观编码和双边网格）存在局限性，无法有效解决这一问题。

Method: 提出了一种新颖的多尺度双边网格，统一了外观编码和双边网格，优化了像素级颜色映射。

Result: 在Waymo、NuScenes、Argoverse和PandaSet四个数据集上表现优异，几何精度显著提升，减少了光度不一致导致的伪影。

Conclusion: 多尺度双边网格方法在自动驾驶场景重建中表现优越，为障碍物避让和控制提供了更准确的几何信息。
...
TL;DR: 论文提出了一种基于几何的长时空间记忆框架，用于提升视频世界模型的长时一致性，解决了现有模型因时间窗口限制导致的场景遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型因时间上下文窗口有限，难以保持场景一致性，尤其是在重新访问时容易遗忘之前生成的环境。

Method: 受人类记忆机制启发，提出了一种几何基础的长时空间记忆框架，包括存储和检索信息的机制，并使用定制数据集训练和评估模型。
...
Method: 利用单张RGB-D图像和查询射线，训练前馈Transformer预测深度图、物体掩码和置信度分数，并通过多视角融合完成3D形状重建。
...
TL;DR: 论文提出VCT和SVCT模型，结合概念特征与图像特征提升医学AI的可解释性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 医学领域对透明度的需求促使研究可解释AI（XAI），但现有概念瓶颈模型（CBMs）仅依赖概念特征，忽略了医学图像的固有特征，且对输入扰动敏感。

Method: 提出Vision Concept Transformer (VCT)和Stable Vision Concept Transformer (SVCT)，后者基于ViT架构，融合概念特征与图像特征，并引入Denoised Diffusion Smoothing提升稳定性。

Result: 在四个医学数据集上的实验表明，VCT和SVCT在保持准确性的同时具有可解释性，SVCT在扰动下仍能提供稳定解释。

Conclusion: VCT和SVCT解决了医学AI的可解释性与稳定性问题，满足了医学领域的需求。
...
TL;DR: EOC-Bench是一个创新的基准测试，用于评估动态场景中多模态大语言模型（MLLMs）的物体认知能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注静态场景，忽略了用户交互带来的动态变化，EOC-Bench填补了这一空白。

Method: EOC-Bench包含3,277个标注QA对，分为过去、现在和未来三类，涵盖11个细粒度评估维度和3种视觉物体引用类型。采用混合格式的人工标注框架和多尺度时间准确性指标。

Result: 基于EOC-Bench，对多种MLLMs进行了全面评估。
...
TL;DR: 论文提出了一种名为AliTok的新型对齐分词器，通过因果解码器建立单向依赖关系，解决了现有图像分词器在自回归模型中建模效率低的问题，并在生成友好性和重建性能上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有图像分词器在压缩过程中编码的双向依赖关系阻碍了自回归模型的有效建模，因此需要一种新的分词器来对齐建模方式。

Method: 提出AliTok，利用因果解码器建立单向依赖关系，并通过前缀标记和两阶段分词器训练增强重建一致性。

Result: 在ImageNet-256基准测试中，AliTok在177M参数下gFID为1.50，IS为305.9；662M参数时gFID为1.35，超越当前最优扩散方法且采样速度快10倍。

Conclusion: AliTok通过单向依赖和两阶段训练，显著提升了自回归图像生成的性能，同时保持了高效的重建能力。
...
<details>
  <summary>Details</summary>
Motivation: 尽管基于扩散的视频修复方法在视觉质量上有显著提升，但其计算成本过高，且现有方法难以扩展到高分辨率视频修复。
...
Method: 集成SAM 2和LLMs，引入Semantic Perceiver，开发数据增强流程。

Result: 性能优于现有方法，速度快1.2-2.4倍，GPU内存消耗更低。
...
TL;DR: 提出一种基于3D感知伪标签的方法，改进语义匹配，减少对特定数据集标注的依赖，并在SPair-71k上取得显著性能提升。
...
Method: 通过3D感知链式伪标签、松弛循环一致性和3D球形原型映射约束训练适配器，优化现有特征。
...
Method: 通过找到去噪UNet中负责材料属性的模块，利用CLIP空间中的方向混合材料，并使用浅层网络预测细粒度属性变化方向。
...
TL;DR: ProJo4D提出了一种渐进式联合优化框架，解决了稀疏多视角视频中物理参数估计的挑战，显著提升了4D场景理解的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在稀疏多视角视频中因误差累积和优化困难而效果不佳，限制了物理准确数字孪生等应用的实际可行性。

Method: 采用渐进式联合优化策略，逐步增加联合优化的参数集，最终实现几何、外观、物理状态和材料属性的全面联合优化。

Result: 在PAC-NeRF和Spring-Gaus数据集上，ProJo4D在4D未来状态预测、未来状态的新视角渲染和材料参数估计方面优于现有方法。

Conclusion: ProJo4D为物理基础的4D场景理解提供了有效解决方案，具有实际应用潜力。
...
TL;DR: 论文探讨了3D视觉语言模型（VLMs）的性能差距，分析了3D场景中心方法的不足，并提出新数据集以促进真正的3D场景理解。


<details>
  <summary>Details</summary>
Motivation: 研究3D VLMs在任务中的表现差异，尤其是3D场景中心方法性能较低的原因。

Method: 分类3D VLMs为3D对象中心、2D图像基础和3D场景中心方法，并分析其性能差距。

Result: 发现3D场景中心方法对3D编码器依赖有限，预训练效果较差，且易过拟合语言线索。
...
TL;DR: 论文提出了一种名为PM-Loss的新型正则化损失函数，用于解决基于深度的3D高斯泼溅（3DGS）中深度不连续性导致的点云稀疏问题，从而提升渲染质量。


<details>
  <summary>Details</summary>
Motivation: 深度图在3D高斯泼溅（3DGS）中被广泛用于生成3D点云以进行新视角合成，但深度不连续性会导致点云稀疏或断裂，影响渲染质量。

Method: 通过预训练的Transformer预测点图（pointmap），并基于此设计PM-Loss正则化损失函数，强制几何平滑性，尤其是在物体边界处。

Result: 改进后的深度图显著提升了3DGS的渲染质量，适用于多种架构和场景。

Conclusion: PM-Loss通过几何平滑性优化深度图，有效解决了3DGS中的点云稀疏问题，提升了渲染效果。
...
TL;DR: 论文提出了CG-AV-Counting基准和AV-Reasoner模型，用于解决视频理解中的计数任务，并在多个基准上取得最优结果。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs在计数任务上表现不佳，现有基准存在视频短、查询封闭、缺乏线索标注和多模态覆盖不足等问题。
...
Method: MINT-CoT通过Interleave Token动态选择数学图形中的视觉区域，并构建了包含54K问题的数据集，采用三阶段训练策略（文本CoT SFT、插入CoT SFT、插入CoT RL）。
...
TL;DR: VideoMarathon数据集填补了长视频标注的空白，支持长达1小时的视频训练，并提出了Hour-LLaVA模型，在长视频理解任务中表现优异。
...
Method: 提出了VideoMarathon数据集（9,700小时长视频，3.3M QA对）和Hour-LLaVA模型，利用内存增强模块实现1-FPS采样训练和推理。

Result: Hour-LLaVA在多个长视频语言基准测试中表现最佳，验证了数据集和模型的有效性。

Conclusion: VideoMarathon和Hour-LLaVA为长视频多模态理解提供了高质量数据和高效模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频跟踪方法缺乏语言模型的推理能力，限制了上下文理解和泛化能力。

Method: 基于Molmo架构，引入时序模块和双向点传播的掩码融合管道，分两步生成指向坐标和分割掩码。

Result: 在多个真实场景和任务中显著提升了时空指向准确性和推理能力。
...
TL;DR: 提出了一种从纹理网格和多视角全景图像生成无家具室内场景的流程，通过简化网格、边缘提取和ControlNet修复技术实现高质量结果。
...
Method: 1. 分割并移除家具，生成简化网格（SDM）；2. 从SDM提取边缘信息；3. 使用ControlNet修复全景图像；4. 用修复图像重新纹理网格。

Result: 生成的资产质量高于依赖神经辐射场或RGB-D修复的方法，避免了模糊和幻觉问题。

Conclusion: 该方法通过结合几何引导和图像修复技术，显著提升了无家具场景生成的质量和准确性。
...
<details>
  <summary>Details</summary>
Motivation: 3D室内场景合成的布局生成因数据集有限而具有挑战性，现有方法要么过拟合数据集，要么依赖预定义约束，牺牲了灵活性。
...
Conclusion: DirectLayout通过直接生成和迭代优化，实现了开放词汇和用户指令对齐的高质量3D布局生成。
...
TL;DR: 论文提出了一种新的任务——全模态引用表达分割（ORES），并提出了一个框架RAS，通过增强分割模型的多模态交互能力来解决复杂查询问题。
...
Conclusion: ORES任务和RAS框架为多模态交互提供了新的解决方案，并展示了优越性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着视频生成技术的进步，训练成本急剧增加，需要更高效的训练方法。

Method: 1. 最小化架构，重用预训练图像生成模型；2. 多阶段训练策略，利用流匹配提高效率；3. 低成本强化学习框架，无需额外人工标注。
...
TL;DR: 研究发现多模态大语言模型（MLLMs）中仅有约5%的注意力头对视觉理解有贡献，称为视觉头。提出了一种无训练框架识别这些头，并基于此设计了KV-Cache优化策略SparseMM，显著提升了推理效率和内存利用率。
...
Conclusion: 视觉头的稀疏性为MLLMs的推理优化提供了新思路，SparseMM展示了高效且性能无损的加速潜力。
...
TL;DR: 首个基于物理的多视角视频神经逆向渲染系统，支持传播光的建模，结合神经辐射缓存技术，实现高效的光传输模拟。
...
Method: 采用时间分辨的神经辐射缓存技术，扩展为无限反弹辐射存储，支持直接和间接光传输的精确建模。

Result: 在闪光激光雷达系统捕获的数据上实现先进的3D重建，支持传播光的视图合成、直接与间接光分解及多视角时间分辨重光照。

Conclusion: 该系统为复杂光传输场景下的逆向渲染提供了高效且准确的解决方案，并展示了新颖的应用能力。
...
TL;DR: 论文提出FreeTimeGS，一种新型4D表示方法，用于解决动态3D场景重建中复杂运动的挑战，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂运动场景中优化变形场困难，导致动态视图合成效果不佳。

Method: 提出FreeTimeGS，允许高斯基元在任意时间和位置出现，并赋予其运动函数以减少时间冗余。

Result: 实验表明，该方法在多个数据集上的渲染质量显著优于现有方法。

Conclusion: FreeTimeGS通过灵活的高斯基元表示和运动函数，有效提升了动态3D场景建模能力。
...
TL;DR: VideoMathQA是一个评估模型在视频中进行跨模态数学推理能力的基准，涵盖10个数学领域，强调时间扩展和多模态整合。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的视频数学推理比静态图像或文本更具挑战性，需要整合视觉、音频和文本信息。

Method: 引入VideoMathQA基准，包含多样化的视频和问题类型，由专家标注，设计三种核心推理挑战。

Result: 通过基准测试，揭示了现有方法的局限性，并建立了系统性评估框架。

Conclusion: VideoMathQA为模型在多模态和时间扩展的数学问题中的推理能力提供了评估标准。
...
TL;DR: 论文提出了一种对比流匹配方法，用于解决条件设置下流匹配的唯一性问题，显著提升了训练速度和生成质量。
...
### [258] [Reviewriter: AI-Generated Instructions For Peer Review Writing](https://arxiv.org/abs/2506.04423)
*Xiaotian Su,Thiemo Wambsganss,Roman Rietsche,Seyed Parsa Neshaei,Tanja Käser*

Main category: cs.HC

TL;DR: 论文介绍了Reviewriter工具，利用生成式AI为德语写作提供指导，并通过学生实验验证其效果。


<details>
  <summary>Details</summary>
Motivation: 探索生成式AI在教育中的应用潜力，特别是针对德语写作的个性化学习需求。

Method: 设计并实现Reviewriter工具，基于德语语料库微调语言模型，并通过学生实验评估工具效果。

Result: 工具在学生实验中表现出积极的技术接受度，同时揭示了生成式AI在写作指导中的优缺点。

Conclusion: 生成式AI在教育中具有潜力，但仍需进一步研究其在真实场景中的应用效果。
...
benefits and limitations of generative AI in peer review writing.

</details>


### [259] [Seamless and Efficient Interactions within a Mixed-Dimensional Information Space](https://arxiv.org/abs/2506.04545)
*Chen Chen*

Main category: cs.HC

TL;DR: 论文探讨了混合维度信息空间的概念，提出了三种交互系统以实现无缝高效的用户体验。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决用户在混合维度信息空间中与2D和3D实体交互时的潜在问题，提升任务专注度。

Method: 提出了三种交互系统：MemoVis（生成AI辅助3D设计）、PaperToPlace（纸质文档MR化）、VRContour（VR轮廓绘制）。

Result: 通过新系统实现了更直观、沉浸式的交互体验，减少了维度转换的干扰。

Conclusion: 混合维度信息空间的交互技术可显著提升用户体验，未来需进一步优化无缝性。

Abstract: Mediated by today's visual displays, information space allows users to
...
### [260] [Multi-Tool Analysis of User Interface & Accessibility in Deployed Web-Based Chatbots](https://arxiv.org/abs/2506.04659)
*Mukesh Rajmohan,Smit Desai,Sanchari Das*

Main category: cs.HC

TL;DR: 对106个网络聊天机器人进行多工具评估，发现80%存在关键无障碍问题，45%存在语义结构缺失或ARIA角色误用。无障碍评分工具间相关性高，但性能评分工具间相关性低。


<details>
  <summary>Details</summary>
Motivation: 评估已部署的网络聊天机器人在无障碍和性能方面的表现，以支持开发更用户友好的对话界面。

Method: 使用自动化工具（如Google Lighthouse、PageSpeed Insights、SiteImprove Accessibility Checker）和手动审核（Microsoft Accessibility Insights）对106个聊天机器人进行评估。

Result: 80%的聊天机器人存在至少一个关键无障碍问题，45%存在语义结构缺失或ARIA角色误用。无障碍评分工具间相关性高（r=0.861），性能评分工具间相关性低（r=0.436）。

Conclusion: 多工具评估方法对识别无障碍问题更有效，提供了可复制的评估见解和可操作建议，以改进对话界面的开发。

Abstract: In this work, we present a multi-tool evaluation of 106 deployed web-based
...
### [261] [Beyond the Desktop: XR-Driven Segmentation with Meta Quest 3 and MX Ink](https://arxiv.org/abs/2506.04858)
*Lisle Faray de Paiva,Gijs Luijten,Ana Sofia Ferreira Santos,Moon Kim,Behrus Puladi,Jens Kleesiek,Jan Egger*

Main category: cs.HC

TL;DR: 本文提出了一种基于扩展现实（XR）的医学图像分割工具，利用Meta Quest 3头显和Logitech MX Ink触控笔，提供沉浸式交互界面，旨在减轻传统手动分割的工作负担。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割在临床中至关重要，但手动标注繁琐且耗时。本研究旨在通过XR技术改进这一过程。

Method: 开发了一个结合触控笔标注和即时3D渲染的沉浸式界面，使用户能实时交互2D和3D医学图像数据。

Result: 用户研究表明，该工具在系统可用性（SUS得分66）和直观性（ISONORM得分4.1/5）方面表现良好，但仍需改进任务精度和错误管理。

Conclusion: XR-触控笔范式为沉浸式分割工具提供了有前景的基础，未来可通过迭代优化进一步推动其在术前规划中的应用。
...
### [262] [Neuronal avalanches as a predictive biomarker of BCI performance: towards a tool to guide tailored training program](https://arxiv.org/abs/2506.04745)
*Camilla Mannino,Pierpaolo Sorrentino,Mario Chavez,Marie-Costance Corsi*

Main category: cs.HC

TL;DR: 研究提出利用神经元雪崩动态作为生物标志物，预测和优化基于运动想象的脑机接口（BCI）训练效果，显著提高成功率。


<details>
  <summary>Details</summary>
Motivation: 解决现有BCI训练中固定长度范式忽视个体学习差异的问题，减少BCI无效性现象。

Method: 通过EEG数据提取神经元雪崩特征（长度和激活），结合支持向量回归和分类模型预测BCI表现。

Result: 预测准确率高达91%，空间过滤后效果更佳，证实雪崩特征对个性化训练的有效性。

Conclusion: 神经元雪崩动态可作为BCI训练的可靠生物标志物，推动个性化协议开发以减少BCI无效性。

Abstract: Brain-Computer Interfaces (BCIs) based on motor imagery (MI) hold promise for
...
### [263] [Adapting Online Customer Reviews for Blind Users: A Case Study of Restaurant Reviews](https://arxiv.org/abs/2506.04865)
*Mohan Sunkara,Akshay Kolgar Nayak,Sandeep Kalari,Yash Prakash,Sampath Jayarathna,Hae-Na Lee,Vikas Ashok*

Main category: cs.HC

TL;DR: QuickQue工具通过主题分类和情感摘要帮助盲人用户更高效地浏览餐厅评论，显著提升了可用性和任务效率。


<details>
  <summary>Details</summary>
Motivation: 盲人用户依赖屏幕阅读器浏览在线餐厅评论时面临疲劳和效率低下的问题。

Method: 开发QuickQue工具，利用大语言模型进行主题分类和情感摘要，生成更易访问的评论摘要。

Result: 用户研究表明，QuickQue显著提高了可用性并降低了任务负担。

Conclusion: QuickQue为盲人用户提供了一种更高效的评论浏览方式，解决了现有屏幕阅读器的局限性。

Abstract: Online reviews have become an integral aspect of consumer decision-making on
...
### [264] [From Screen to Space: Evaluating Siemens' Cinematic Reality](https://arxiv.org/abs/2506.04972)
...
TL;DR: 研究团队评估了Siemens的Cinematic Reality在Apple Vision Pro上的可用性和临床潜力，通过医学专家反馈总结了其优势和需改进之处。


<details>
  <summary>Details</summary>
Motivation: 探索沉浸式电影渲染技术在医学影像中的实际应用潜力。

Method: 使用CHAOS和MRCP_DLRecon数据集，14位医学专家通过问卷和开放调查评估可用性和临床潜力。

Result: 反馈指出了可行性、关键优势及需改进的功能，以促进临床工作流程中的应用。

Conclusion: 沉浸式电影渲染在医学影像中具有潜力，但仍需进一步优化。
...
TL;DR: 论文提出了一种新的社会技术框架，旨在弥补人工智能在医学中的翻译差距，强调与医生认知活动的互补性。


<details>
  <summary>Details</summary>
Motivation: 当前以技术为中心的方法导致人工智能系统与临床实践（如诊断推理和决策）不兼容，限制了其实际影响。

Method: 提出了一种社会技术概念化方法，设计数据驱动的支持工具，以补充医生的认知和认知活动。

Result: 该方法强调实际影响，而非在无关基准上的超人类表现。

Conclusion: 通过社会技术框架，可以更好地将人工智能融入临床实践，提升其实际价值。
...
### [266] [Towards Effective Multidisciplinary Health and HCI Teams based on AI Framework](https://arxiv.org/abs/2506.05226)
*Mohammed Almutairi,Diego Gómez-Zará*

Main category: cs.HC

TL;DR: 研究探讨如何利用HCI和AI技术构建跨学科团队，以促进健康领域的多学科合作。


<details>
  <summary>Details</summary>
Motivation: 解决跨学科和多利益相关者团队项目中的挑战，特别是在教育、学术和健康领域。

Method: 开发基于HCI的社会技术系统，利用先进算法优先考虑团队成员的多样性和熟悉度。

Result: 目标是构建AI驱动的团队组合系统，以优化跨学科协作。

Conclusion: 通过研讨会与学者交流，进一步完善AI驱动的团队组合方法。

Abstract: As a Ph.D. student with a diverse background in both public and private
...
TL;DR: 该论文提出AI增强的团队优化框架，包括基于多臂老虎机的团队形成算法、AI反馈助手tAIfa和模拟框架PuppeteerLLM，以提升团队满意度、凝聚力和表现。


<details>
  <summary>Details</summary>
Motivation: 现有团队优化工具依赖静态数据或特定场景，无法动态适应成员个性、目标变化和偏好调整，导致团队表现不佳。

Method: 1. 多臂老虎机算法优化团队形成；2. AI助手tAIfa提供个性化反馈；3. PuppeteerLLM框架模拟团队动态。

Result: 提出的框架能动态优化团队，提升满意度、凝聚力和表现。
...
### [268] [A Comprehensive Survey on the Risks and Limitations of Concept-based Models](https://arxiv.org/abs/2506.04237)
*Sanchit Sinha,Aidong Zhang*

Main category: cs.LG

TL;DR: 本文综述了基于概念的模型的风险与局限性，包括概念泄漏、表示纠缠和对抗性脆弱性等问题，并探讨了改进其可靠性的方法。


<details>
  <summary>Details</summary>
Motivation: 基于概念的模型在关键领域（如医疗诊断和金融风险预测）中表现出色，但其结构、训练过程和假设存在显著局限性，影响了其可靠性和泛化能力。本文旨在全面调查这些问题。

Method: 通过综述现有研究，总结了基于概念的模型在监督和无监督范式中的常见挑战，并分析了缓解这些挑战的架构选择。

Result: 研究发现，概念泄漏、表示纠缠和对抗性脆弱性是主要问题，但已有方法在提高模型可靠性方面取得进展。

Conclusion: 尽管基于概念的模型存在局限性，但通过改进架构和训练方法，未来研究有望进一步提升其可靠性和实用性。
...
### [269] [Improving Out-of-Distribution Detection with Markov Logic Networks](https://arxiv.org/abs/2506.04241)
*Konstantin Kirchheim,Frank Ortmeier*

Main category: cs.LG

TL;DR: 该论文提出了一种基于马尔可夫逻辑网络（MLN）的方法，用于增强现有OOD检测器的性能，同时提高可解释性。


<details>
  <summary>Details</summary>
Motivation: 在开放世界场景中，确保深度学习模型的可靠性需要有效的OOD检测方法。当前方法主要依赖统计模型，缺乏可解释性。

Method: 利用MLN将一阶逻辑与概率推理结合，通过加权逻辑约束为输入分配概率，并提出了学习逻辑约束的算法。

Result: 实验表明，MLN能显著提升多种现有OOD检测器的性能，同时保持计算效率。

Conclusion: MLN为OOD检测提供了性能提升和可解释性增强的有效途径。
...
### [270] [Triple Attention Transformer Architecture for Time-Dependent Concrete Creep Prediction](https://arxiv.org/abs/2506.04243)
*Warayut Dokduea,Weerachart Tangchirapat,Sompote Youwai*

Main category: cs.LG

TL;DR: 本文提出了一种新型三重注意力Transformer架构，用于预测时间依赖性混凝土徐变，解决了现有方法仅将时间作为输入参数而未建模变形发展序列性的问题。


<details>
  <summary>Details</summary>
Motivation: 当前方法在混凝土徐变预测中未充分建模时间序列特性，限制了预测精度。本文旨在通过Transformer的自注意力机制捕捉长期依赖关系。

Method: 采用三重注意力框架（时间注意力、特征注意力和批次注意力），将混凝土徐变预测转化为自回归序列建模任务。

Result: 在160天的标准化数据集上，模型平均绝对百分比误差为1.63%，R2值为0.999，显著优于传统方法。SHAP分析显示弹性模量是最重要特征。

Conclusion: 该研究证明了Transformer架构在材料科学中的可行性，为数据驱动方法在工程设计和结构行为预测中的应用开辟了新途径。
...
### [271] [SafeSteer: Interpretable Safety Steering with Refusal-Evasion in LLMs](https://arxiv.org/abs/2506.04250)
*Shaona Ghosh,Amrita Bhattacharjee,Yftah Ziser,Christopher Parisien*

Main category: cs.LG

TL;DR: SafeSteer方法通过潜在激活导向实现LLM的安全调整，无需微调，保持文本质量与主题相关性。


<details>
  <summary>Details</summary>
Motivation: 适应LLM的安全策略成本高且不切实际，需要更灵活的方法。

Method: 利用类别特定导向向量和无监督梯度方法，避免对比数据需求。

Result: 在多种LLM和数据集上验证，能精确控制安全输出，避免生硬拒绝。

Conclusion: SafeSteer简单有效，为LLM安全调整提供实用解决方案。
...
### [272] [Localized Forest Fire Risk Prediction: A Department-Aware Approach for Operational Decision Support](https://arxiv.org/abs/2506.04254)
*Nicolas Caron,Christophe Guyeux,Hassan Noura,Benjamin Aynes*

Main category: cs.LG

TL;DR: 论文提出了一种针对法国各部门的森林火灾风险评估方法，解决了传统二分类任务的局限性，提供了更具区域针对性的预测。


<details>
  <summary>Details</summary>
Motivation: 气候变化加剧了火灾行为与频率，传统二分类方法过于简化问题，无法满足消防员等终端用户的需求。

Method: 提出了一种基于部门特定条件的火灾风险评估方法，利用先进AI模型在法国全国范围内进行预测。

Result: 首次在法国全国范围内建立了AI基准测试，提供了区域特异性的火灾风险评估结果。
...
### [273] [MUC-G4: Minimal Unsat Core-Guided Incremental Verification for Deep Neural Network Compression](https://arxiv.org/abs/2506.04268)
*Jingyang Li,Guoqiang Li*

Main category: cs.LG

TL;DR: MUC-G4是一种新颖的增量验证框架，用于压缩深度神经网络的验证，通过SMT公式编码和最小不可满足核（MUCs）指导，显著提高了验证效率。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在边缘设备上的部署面临内存和运行时复杂性的挑战，现有增量验证方法主要关注量化且难以处理结构变化。

Method: 将原始和压缩网络编码为SMT公式，分类变化，并利用原始网络的MUCs指导压缩网络的验证。

Result: 实验证明MUC-G4能有效处理量化和剪枝，验证时间显著缩短，且证明重用率高。
...
### [274] [Understanding the Impact of Sampling Quality in Direct Preference Optimization](https://arxiv.org/abs/2506.04272)
*Kyung Rok Kim,Yumo Bai,Chonghuan Wang,Guanting Chen*

Main category: cs.LG

TL;DR: 研究了采样分布在直接偏好优化（DPO）中的作用及其对训练动态的影响，发现高质量响应能增强梯度信号并优化学习效果。


<details>
  <summary>Details</summary>
Motivation: 探讨采样分布对DPO训练动态的影响，以理解其优化行为。

Method: 通过分析响应分布对梯度更新的影响，设计简化对齐模型，并进行实证实验。

Result: 高质量响应频率的增加能显著提升梯度信号和优化效果。

Conclusion: 研究结果为在线DPO框架提供了理论支持。
...
### [275] [SF$^2$Bench: Evaluating Data-Driven Models for Compound Flood Forecasting in South Florida](https://arxiv.org/abs/2506.04281)
...
<details>
  <summary>Details</summary>
Motivation: 复合洪水预测因气象、水文和海洋因素的复杂交互而具挑战性，传统物理方法效率低，机器学习虽有效但缺乏全面数据集。

Method: 引入SF2Bench数据集，评估多层感知机、卷积神经网络、循环神经网络、图神经网络、Transformer和大语言模型的性能。

Result: 实验验证了不同关键特征对洪水预测的影响，揭示了各方法在捕捉时空依赖性上的差异。

Conclusion: 不同方法在复合洪水预测中表现各异，SF2Bench为改进预测提供了数据支持。
...
### [276] [Conservative classifiers do consistently well with improving agents: characterizing statistical and online learning](https://arxiv.org/abs/2506.05252)
*Dravyansh Sharma,Alec Sun*

Main category: cs.LG

TL;DR: 本文研究了机器学习中分类代理的真实改进行为对学习能力的影响，提出了新的学习能力特征，并解决了Attias等人提出的开放性问题。


<details>
  <summary>Details</summary>
Motivation: 探讨分类代理在真实改进（而非欺骗）情况下如何影响学习算法的性能，填补现有文献的空白。

Method: 引入非对称的最小一致概念类，研究欧几里得球改进集下的学习能力，并在有界噪声模型和在线学习中验证方法。

Result: 在可实现设置中精确刻画了学习能力，证明了在更自然的改进集下学习是可能的，并在噪声模型和在线学习中取得更低泛化误差。

Conclusion: 本文扩展了对改进学习能力的理解，解决了开放性问题，并为实际应用提供了理论支持。
...
### [277] [DrSR: LLM based Scientific Equation Discovery with Dual Reasoning from Data and Experience](https://arxiv.org/abs/2506.04282)
*Runxiang Wang,Boxiao Wang,Kai Li,Yifan Zhang,Jian Cheng*

Main category: cs.LG

TL;DR: DrSR（双推理符号回归）通过结合数据驱动洞察与反思学习，提升符号回归的鲁棒性和发现能力，显著优于传统和基于LLM的方法。
...
Method: DrSR框架结合数据驱动的结构关系分析和反思学习，通过闭环反馈优化方程生成。

Result: 实验表明，DrSR在物理、化学、生物和材料科学等领域的数据集上显著提高了有效方程率，并在准确性、泛化能力和搜索效率上优于其他方法。

Conclusion: DrSR通过数据理解和生成反思的闭环整合，为科学方程发现提供了高效工具。
...
### [278] [Training-free AI for Earth Observation Change Detection using Physics Aware Neuromorphic Networks](https://arxiv.org/abs/2506.04285)
*Stephen Smith,Cormac Purcell,Zdenka Kuncic*

Main category: cs.LG

TL;DR: 论文提出了一种基于物理感知神经形态网络（PANN）的方法，用于从多光谱卫星图像序列中检测自然灾害引起的变化，并生成变化图，以优先传输相关数据。该方法无需训练，计算资源需求低，性能与先进AI模型相当。


<details>
  <summary>Details</summary>
Motivation: 解决卫星数据下行带宽限制问题，通过机载处理优先传输自然灾害相关数据，以支持应急决策。

Method: 使用基于忆阻器的物理感知神经形态网络（PANN），动态更新权重，生成物理约束的动态特征，通过距离度量检测变化。

Result: PANN在自然灾害检测任务中表现优异，与先进AI模型性能相当或更好。

Conclusion: PANN为资源受限的机载处理提供了一种高效解决方案，具有实际应用潜力。
...
### [279] [Backbone Augmented Training for Adaptations](https://arxiv.org/abs/2506.04288)
*Jae Wan Park,Junhyeok Kim,Youngjun Jun,Hyunah Ko,Seong Jae Hwang*

Main category: cs.LG

TL;DR: 论文提出Backbone Augmented Training (BAT)方法，利用主干数据增强适应数据集，解决有限适应数据的问题，并通过ALBAT算法高效提升训练效果。


<details>
  <summary>Details</summary>
Motivation: 适应技术虽能高效训练大型主干模型，但有限适应数据常导致训练困难。

Method: 提出BAT方法，利用主干数据增强适应数据集，并引入ALBAT算法实现数据选择和训练优化。

Result: ALBAT在数据稀缺的个人化和语言生成任务中显著提升适应训练效果。

Conclusion: BAT和ALBAT为有限适应数据场景提供了高效解决方案。
...
### [280] [Relational reasoning and inductive bias in transformers trained on a transitive inference task](https://arxiv.org/abs/2506.04289)
*Jesse Geerts,Stephanie Chan,Claudia Clopath,Kimberly Stachenfeld*

Main category: cs.LG

TL;DR: 论文研究了Transformer模型在关系推理任务中的表现，比较了两种学习机制（IWL和ICL）对传递性推理的影响。
...
Method: 通过比较IWL（参数学习）和ICL（上下文学习）在传递性推理任务中的表现，并进行机制分析。

Result: IWL模型能自然推广到传递性推理，而ICL模型仅能处理相邻项关系。预训练后，ICL模型也能实现传递性推理。

Conclusion: 预训练任务的结构性有助于模型发展支持上下文关系推理的表征。
...
### [281] [A Lyapunov Drift-Plus-Penalty Method Tailored for Reinforcement Learning with Queue Stability](https://arxiv.org/abs/2506.04291)
...
<details>
  <summary>Details</summary>
Motivation: 随着IoT设备的普及，解决复杂优化问题的需求增加，现有方法未能有效结合Lyapunov漂移加惩罚算法与强化学习的优势。

Method: 通过理论分析，提出LDPTRLQ算法，平衡Lyapunov漂移加惩罚的贪婪优化与强化学习的长期视角。

Result: 仿真结果显示LDPTRLQ在性能和稳定性上优于基线方法。
...
### [282] [AUTOCT: Automating Interpretable Clinical Trial Prediction with LLM Agents](https://arxiv.org/abs/2506.04293)
*Fengze Liu,Haoyu Wang,Joonhyuk Cho,Dan Roth,Andrew W. Lo*

Main category: cs.LG

TL;DR: AutoCT结合大型语言模型和经典机器学习，提出了一种可解释、高效的临床试验预测框架。


<details>
  <summary>Details</summary>
Motivation: 临床试验成本高、耗时长，现有深度学习模型缺乏可解释性且易受标签泄漏影响。
...
### [283] [Agentic AI for Intent-Based Industrial Automation](https://arxiv.org/abs/2506.04980)
*Marcos Lima Romero,Ricardo Suyama*

Main category: cs.LG

TL;DR: 论文提出了一种将Agentic AI与意图驱动范式结合的框架，简化人机交互，支持工业5.0原则，并通过实验验证了其可行性。


<details>
  <summary>Details</summary>
Motivation: 工业4.0的复杂性促使需要更简化的人机交互方式，同时支持工业5.0的人类中心、可持续和韧性原则。

Method: 提出一个结合Agentic AI和意图驱动范式的框架，将高层目标分解为可执行任务，并由子代理执行。

Result: 通过CMAPSS数据集和Google ADK验证了意图分解、代理协调和自主决策的可行性。

Conclusion: 该框架有望降低技术壁垒，实现可扩展的意图驱动自动化，但仍需解决数据质量和可解释性问题。
...
### [284] [Short-Term Power Demand Forecasting for Diverse Consumer Types to Enhance Grid Planning and Synchronisation](https://arxiv.org/abs/2506.04294)
...
<details>
  <summary>Details</summary>
Motivation: 随着可再生能源的普及，电网稳定性对电力需求预测的准确性提出了更高要求。传统预测方法未充分考虑不同消费者群体的独特用电模式，导致预测精度不足。

Method: 研究通过消费者聚类和特征选择（包括时间、社会经济和天气数据），比较了多种AI和机器学习算法，提出了针对不同群体的定制化预测模型。

Result: 提出的新预测方法在短期和超短期预测中表现优于简单模型，证明了定制化策略和详细天气数据的重要性。
...
### [285] [Deep learning for predicting hauling fleet production capacity under uncertainties in open pit mines using real and simulated data](https://arxiv.org/abs/2506.04296)
*N Guerin,M Nakhla,A Dehoux,J L Loyer*

Main category: cs.LG

TL;DR: 论文提出了一种结合真实数据和模拟故障场景的深度学习框架，用于预测露天矿运输车队短期容量，XGBoost和LSTM模型的预测误差分别为14.3%和15.1%。


<details>
  <summary>Details</summary>
Motivation: 露天矿运输车队容量受天气、机械故障和人员变动等因素影响，预测不确定性高，需更精准的短期预测方法。

Method: 结合高分辨率降雨数据、车队性能遥测和模拟机械故障场景，采用XGBoost和LSTM模型进行预测，并通过SHAP值分析关键因素。

Result: XGBoost的MedAE为14.3%，LSTM为15.1%；模拟故障数据和排班特征显著降低了预测波动性。

Conclusion: 混合建模方法为动态不确定条件下的车队管理提供了全面的决策支持工具，未来将整合更多运营约束以增强预测稳健性。
...
### [286] [Softlog-Softmax Layers and Divergences Contribute to a Computationally Dependable Ensemble Learning](https://arxiv.org/abs/2506.04297)
...
Method: 1. 解剖学过程：选择多样化的典型元素构建目标集成模型；2. 生理学过程：通过softlog-softmax层实现一致的中间决策；3. 信息理论过程：提出基于softlog的熵和散度构建信息度量；4. 性能评估过程：推导信息性能张量进行可靠评估。

Result: 研究表明，问题越复杂，集成多样性越有用；提出的信息度量在闭区间上提供一致值。

Conclusion: 通过4步方法，显著提升了集成学习系统的一致性和可靠性，特别是在复杂问题中。
...
### [287] [The Cost of Dynamic Reasoning: Demystifying AI Agents and Test-Time Scaling from an AI Infrastructure Perspective](https://arxiv.org/abs/2506.04301)
*Jiin Kim,Byeongjun Shin,Jinha Chung,Minsoo Rhu*

Main category: cs.LG

TL;DR: 本文对基于大语言模型（LLM）的AI代理进行了系统级分析，量化了其资源使用、延迟行为、能耗及数据中心功耗，揭示了其在高计算成本下的可持续性问题。
...
Method: 通过量化不同代理设计和测试扩展策略的资源使用、延迟、能耗等指标，分析设计选择对准确性与成本的影响。

Result: 发现代理虽能通过增加计算提升准确性，但存在收益递减、延迟方差扩大及基础设施成本不可持续的问题。

Conclusion: 呼吁转向计算高效推理的代理设计，以在性能与可部署性之间取得平衡。
...
### [288] [RedRFT: A Light-Weight Benchmark for Reinforcement Fine-Tuning-Based Red Teaming](https://arxiv.org/abs/2506.04302)
*Xiang Zheng,Xingjun Ma,Wei-Bin Lee,Cong Wang*

Main category: cs.LG

TL;DR: 论文提出了RedRFT，一个轻量级基准，用于简化和标准化基于RFT的红队测试的实现与评估。
...
### [289] [You Only Train Once](https://arxiv.org/abs/2506.04349)
*Christos Sakaridis*
...
<details>
  <summary>Details</summary>
Motivation: 传统模型训练中，损失权重的选择和优化需要多次训练，效率低下。YOTO旨在通过一次性训练解决这一问题。

Method: 将损失权重超参数视为网络参数，通过梯度优化自动学习，并引入正则化损失确保权重均匀性和有界性。
...
### [290] [Half-Layered Neural Networks](https://arxiv.org/abs/2506.04352)
*Ethem Alpaydin*

Main category: cs.LG

TL;DR: 提出了一种“半”隐藏层单元，部分权重随机固定，部分可训练，通过减少可调参数实现正则化效果。
...
Method: 设计“半”隐藏单元，固定部分权重为随机值，仅调整少量权重（每个单元两个），适用于任何层。

Result: 在MNIST和FashionMNIST数据集上验证，表明半层能减少参数数量并保持合理准确率。

Conclusion: 半隐藏层通过随机连接的正则化效果，为减少参数提供了有效方法。
...
### [291] [A Risk-Aware Reinforcement Learning Reward for Financial Trading](https://arxiv.org/abs/2506.04358)
...
Method: 通过网格搜索调整权重，针对特定风险收益目标，并推导闭式梯度以支持梯度训练。

Result: 框架提供了构建多目标奖励函数的通用蓝图，适用于复杂交易环境。

Conclusion: 该方法可扩展性强，支持添加更多风险度量或自适应权重。
...
### [292] [Even Faster Hyperbolic Random Forests: A Beltrami-Klein Wrapper Approach](https://arxiv.org/abs/2506.04360)
*Philippe Chlenski,Itsik Pe'er*

Main category: cs.LG

TL;DR: 论文提出了一种在Beltrami-Klein双曲空间模型中重新表达hyperDT算法的方法，通过简单的预处理和后处理步骤，将双曲决策树转化为欧几里得空间模型的包装器，从而提升灵活性、速度和准确性。
...
Method: 将hyperDT算法重新表达为Beltrami-Klein模型中的形式，利用预处理和后处理步骤包装现有欧几里得空间模型。

Result: 实现了更灵活、快速和准确的模型，同时简化了代码库。

Conclusion: 该方法为双曲空间中的决策树模型提供了一种高效且易于维护的实现方式。
...
### [293] [Replay Can Provably Increase Forgetting](https://arxiv.org/abs/2506.04377)
*Yasaman Mahdaviyeh,James Lucas,Mengye Ren,Andreas S. Tolias,Richard Zemel,Toniann Pitassi*

Main category: cs.LG

TL;DR: 论文分析了样本回放在持续线性回归中的理论效果，发现回放样本可能导致性能下降，甚至在某些情况下有害。


<details>
  <summary>Details</summary>
Motivation: 持续学习中遗忘问题是主要挑战，样本回放是常用方法，但其理论效果尚不明确。

Method: 在过参数化的持续线性回归设定中，分析样本回放的作用及任务子空间关系。

Result: 发现回放样本可能导致非单调遗忘，甚至在某些情况下有害，并通过神经网络实验验证。

Conclusion: 回放样本的选择和任务关系对性能至关重要，需谨慎设计回放策略。
...
### [294] [Bridging the Performance Gap Between Target-Free and Target-Based Reinforcement Learning With Iterated Q-Learning](https://arxiv.org/abs/2506.04398)
*Théo Vincent,Yogesh Tripathi,Tim Faust,Yaniv Oren,Jan Peters,Carlo D'Eramo*

Main category: cs.LG

TL;DR: 提出了一种名为iS-QL的新方法，通过共享在线网络的最后一层线性层作为目标网络，解决了去除目标网络导致的性能下降问题，同时提高了样本效率。


<details>
  <summary>Details</summary>
Motivation: 在基于价值的强化学习中，去除目标网络虽能节省内存并利用最新估计，但会引入不稳定性和性能下降。

Method: 使用在线网络的最后一层线性层副本作为目标网络，其余参数与在线网络共享，结合迭代Q学习的概念。

Result: iS-QL在多种设置下提高了目标无关方法的样本效率，且内存占用更小，训练时间与传统目标算法相当。

Conclusion: iS-QL为强化学习研究提供了一种可扩展的新方法，平衡了目标网络和无目标网络的优缺点。
...
### [295] [Unsupervised Meta-Testing with Conditional Neural Processes for Hybrid Meta-Reinforcement Learning](https://arxiv.org/abs/2506.04399)
*Suzan Ece Ada,Emre Ugur*

Main category: cs.LG

TL;DR: UMCNP是一种结合参数化策略梯度和任务推断的元强化学习方法，适用于元测试阶段缺少奖励信号的情况，显著提高样本效率。


<details>
  <summary>Details</summary>
Motivation: 解决元测试阶段奖励信号缺失的问题，同时减少元测试所需的在线交互次数。

Method: 利用条件神经过程（CNPs）高效复用元训练样本，通过单次测试任务推断潜在动态模型表示。

Result: 在2D-Point Agent和连续控制基准测试中，UMCNP比基线方法显著减少元测试样本需求。

Conclusion: UMCNP在样本效率和任务适应性上表现优异，适用于动态参数未知的任务。
...
### [296] [Self-Supervised Contrastive Learning is Approximately Supervised Contrastive Learning](https://arxiv.org/abs/2506.04411)
*Achleshwar Luthra,Tianbao Yang,Tomer Galanti*

Main category: cs.LG

TL;DR: 本文通过理论分析表明，自监督对比学习（CL）的目标隐式近似于一种称为“仅负样本监督对比损失（NSCL）”的变体，并证明了随着语义类别数量的增加，CL与NSCL之间的差距会消失。
...
Result: CL与NSCL的差距随类别数量增加而减小，且NSCL的全局最优解具有特定的几何结构。

Conclusion: 自监督对比学习隐式优化了NSCL，支持准确的少样本分类。
...
### [297] [Leveraging Coordinate Momentum in SignSGD and Muon: Memory-Optimized Zero-Order](https://arxiv.org/abs/2506.04430)
...
Method: 提出JAGUAR SignSGD（基于零阶动量）和JAGUAR Muon（利用参数矩阵结构），并提供了收敛性理论保证。

Result: 实验表明，这些方法在收敛质量上优于或持平传统一阶方法，同时显著降低内存需求。
...
### [298] [KOALA++: Efficient Kalman-Based Optimization of Neural Networks with Gradient-Covariance Products](https://arxiv.org/abs/2506.04432)
*Zixuan Xia,Aram Davtyan,Paolo Favaro*

Main category: cs.LG

TL;DR: KOALA++是一种基于卡尔曼滤波的优化算法，通过显式建模梯度不确定性来提升神经网络训练效率，避免昂贵的二阶梯度计算。
...
Conclusion: KOALA++在性能和效率之间取得了平衡，为神经网络训练提供了一种更优的优化方案。
...
### [299] [Grokking and Generalization Collapse: Insights from \texttt{HTSR} theory](https://arxiv.org/abs/2506.04434)
*Hari K. Prakash,Charles H. Martin*

Main category: cs.LG

TL;DR: 论文研究神经网络中的grokking现象，发现了一个新的第三阶段——anti-grokking，并通过HTSR指标α揭示了其特性。


<details>
  <summary>Details</summary>
Motivation: 探索神经网络训练中的grokking现象，特别是其潜在的第三阶段anti-grokking，以更好地理解模型的泛化和过拟合。
...
### [300] [RETRO SYNFLOW: Discrete Flow Matching for Accurate and Diverse Single-Step Retrosynthesis](https://arxiv.org/abs/2506.04439)
*Robin Yadav,Qi Yan,Guy Wolf,Avishek Joey Bose,Renjie Liao*

Main category: cs.LG

TL;DR: 论文提出了一种名为RETRO SYNFLOW（RSF）的新方法，用于解决单步逆合成反应预测问题，通过离散流匹配框架和反应中心识别步骤，显著提升了预测准确性和多样性。


<details>
  <summary>Details</summary>
Motivation: 有机化学中，预测目标产物分子的合成反应路径是一个基础但具有挑战性的问题，现有方法在准确性和多样性上仍有不足。

Method: RSF采用离散流匹配框架，结合反应中心识别步骤生成中间结构（合成子），并通过Feynman-Kac引导和顺序蒙特卡洛重采样提升生成样本的多样性和可行性。

Result: 实验表明，RSF在top-1准确率上达到60.0%，比现有最佳方法提升20%，且在top-5往返准确率上提升19%。

Conclusion: RSF通过创新的框架和引导策略，显著提升了单步逆合成预测的性能，为有机化学合成路径设计提供了更有效的工具。
...
### [301] [Selective Matching Losses -- Not All Scores Are Created Equal](https://arxiv.org/abs/2506.04446)
...
### [302] [Neurosymbolic Artificial Intelligence for Robust Network Intrusion Detection: From Scratch to Transfer Learning](https://arxiv.org/abs/2506.04454)
*Huynh T. T. Tran,Jacob Sander,Achraf Cohen,Brian Jalaian,Nathaniel D. Bastian*

Main category: cs.LG

TL;DR: 本文扩展了ODXU框架，结合神经符号AI和不确定性量化技术，提升网络入侵检测系统的性能，并在实验中优于传统模型。同时，提出了迁移学习策略，验证了其在小样本数据上的有效性。


<details>
  <summary>Details</summary>
Motivation: 网络入侵检测系统面临日益复杂的网络威胁，传统方法在鲁棒性、可解释性和泛化能力上存在不足。本文旨在通过神经符号AI和不确定性量化技术提升NIDS的性能。

Method: 扩展ODXU框架，结合深度嵌入聚类、XGBoost符号推理和不确定性量化（包括基于分数和基于元模型的方法）。提出迁移学习策略，重用预训练模型。

Result: 在CIC-IDS-2017数据集上，ODXU在六项评估指标上优于传统模型。迁移学习策略在ACI-IoT-2023数据集上仅需50%训练数据即可超越传统模型。基于元模型的不确定性量化方法表现更优。

Conclusion: ODXU框架显著提升了NIDS的性能，迁移学习策略为小样本场景提供了有效解决方案，基于元模型的不确定性量化方法更具优势。
...
### [303] [COSMOS: Predictable and Cost-Effective Adaptation of LLMs](https://arxiv.org/abs/2505.01449)
*Jiayu Wang,Aws Albarghouthi,Frederic Sala*

Main category: cs.LG

TL;DR: COSMOS框架通过轻量级代理模型和低样本扩展定律，高效预测LLM适应策略的性能和成本，显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 在资源受限情况下，如何无需昂贵实验即可准确预测LLM适应策略的性能和成本。

Method: 提出COSMOS框架，结合嵌入增强的轻量级代理模型和低样本扩展定律，预测微调性能和检索增强的上下文学习。

Result: 在八个基准测试中，COSMOS平均降低计算成本92.72%，最高达98.71%，同时保持高预测准确性。

Conclusion: 高效预测LLM适应策略可行，能显著减少计算开销并维持性能标准。
...
<details>
  <summary>Details</summary>
Motivation: 系统性是组合性的核心属性，对模型在新情境中的泛化能力至关重要。现有研究多关注行为系统性，而忽略了表征系统性。
...
Conclusion: 强调区分行为系统性和表征系统性的重要性，并呼吁在评估模型时更全面地考虑系统性。
...
TL;DR: 该研究利用机器学习模型对牙科服务提供者进行分类，通过特征排名和多种模型评估，发现神经网络在20个特征下准确率最高（94.1%），并强调了特征选择在医疗数据集中的重要性。
...
Method: 使用2018年的24,300条数据实例和20个特征，采用信息增益、基尼指数和ANOVA进行特征排名，评估了12种机器学习模型（如kNN、决策树、SVM等）。

Result: 神经网络表现最佳（94.1%准确率），梯度提升和随机森林紧随其后。特征选择显示治疗相关指标是关键预测因素。

Conclusion: 特征选择显著提升模型性能，建议结合特征排名和先进算法优化分类，支持资源精准分配。
...
Method: 使用2018年数据集（24,300个实例，20个特征），测试多种机器学习算法（如kNN、决策树、SVM、神经网络等），采用10折交叉验证。

Result: 神经网络表现最佳（AUC: 0.975, CA: 94.1%），随机森林次之（AUC: 0.948, CA: 93.0%）。

Conclusion: 高级机器学习技术（如集成学习和深度学习）显著提升了牙科提供者分类效果，有助于优化资源分配和服务不足人群的医疗支持。
...
### [307] [Orthogonal Gradient Descent Improves Neural Calibration](https://arxiv.org/abs/2506.04487)
*C. Evans Hedges*
...
### [308] [Multiscale guidance of AlphaFold3 with heterogeneous cryo-EM data](https://arxiv.org/abs/2506.04490)
*Rishwanth Raghu,Axel Levy,Gordon Wetzstein,Ellen D. Zhong*

Main category: cs.LG

TL;DR: CryoBoltz结合冷冻电镜密度图和蛋白质结构预测模型，通过全局和局部结构约束生成与实验数据一致的构象状态。


<details>
  <summary>Details</summary>
Motivation: 解决蛋白质结构预测模型无法捕捉动态生物分子复合物构象多样性的问题，同时简化冷冻电镜从原始数据到原子模型的流程。
...
### [309] [Perturbative Gradient Training: A novel training paradigm for bridging the gap between deep neural networks and physical reservoir computing](https://arxiv.org/abs/2506.04523)
...
<details>
  <summary>Details</summary>
Motivation: 物理储层计算的黑盒特性使其无法进行反向传播，PGT旨在解决这一问题。

Method: 利用参数空间的随机扰动，仅通过前向传播近似梯度更新。

Result: 在模拟和实验硬件中，PGT性能与标准反向传播相当。

Conclusion: PGT为物理储层与深度神经网络结合及提升AI训练能效提供了可能。
...
### [310] [Hierarchical Implicit Neural Emulators](https://arxiv.org/abs/2506.04528)
*Ruoxi Jiang,Xiao Zhang,Karan Jakhar,Peter Y. Lu,Pedram Hassanzadeh,Michael Maire,Rebecca Willett*

Main category: cs.LG

TL;DR: 提出了一种多尺度隐式神经模拟器，通过分层预测未来状态表示，提升长期预测准确性，显著优于自回归基线。


<details>
  <summary>Details</summary>
Motivation: 神经PDE求解器在长期时间范围内易受误差累积和稳定性问题困扰，需改进其物理一致性和预测稳定性。

Method: 采用多尺度隐式时间步进方法，结合分层压缩的未来状态表示，动态调整时间下采样率。

Result: 在湍流流体动力学实验中，该方法短期精度高，长期预测稳定，计算开销小。
...
### [311] [HALoS: Hierarchical Asynchronous Local SGD over Slow Networks for Geo-Distributed Large Language Model Training](https://arxiv.org/abs/2506.04531)
*Geon-Woo Kim,Junbo Li,Shashidhar Gandham,Omar Baldonado,Adithya Gangidi,Pavan Balaji,Zhangyang Wang,Aditya Akella*

Main category: cs.LG

TL;DR: HALoS是一种分层异步优化框架，通过本地和全局参数服务器减少跨区域通信成本，提升异构硬件利用率，显著加速LLM训练。
...
Method: 引入本地参数服务器（LPS）和全局参数服务器（GPS），分层设计减少通信延迟，利用分层动量优化异步训练。
...
### [312] [NOBLE -- Neural Operator with Biologically-informed Latent Embeddings to Capture Experimental Variability in Biological Neuron Models](https://arxiv.org/abs/2506.04536)
*Luca Ghafourpour,Valentin Duruisseaux,Bahareh Tolooshams,Philip H. Wong,Costas A. Anastassiou,Anima Anandkumar*

Main category: cs.LG

TL;DR: NOBLE是一种神经操作框架，通过学习神经元特征的连续频率调制嵌入到电压响应的映射，解决了传统神经元模型在生物物理复杂性和变异性上的不足，实现了高效生成合成神经元。


<details>
  <summary>Details</summary>
Motivation: 当前神经元建模受限于实验数据的有限性和变异性，传统方法难以扩展，深度学习未能完全捕捉生物物理复杂性。

Method: NOBLE框架通过训练生物物理真实神经元模型生成的数据，学习从神经元特征到电压响应的映射，预测动态分布。

Result: NOBLE实现了4200倍的速度提升，生成具有实验变异性的合成神经元，并在真实实验数据上验证。
...
### [313] [Neural MJD: Neural Non-Stationary Merton Jump Diffusion for Time Series Prediction](https://arxiv.org/abs/2506.04542)
*Yuanpei Gao,Qi Yan,Yan Leng,Renjie Liao*

Main category: cs.LG

TL;DR: Neural MJD模型结合神经网络与非平稳Merton跳跃扩散模型，通过显式建模随机过程提升非平稳时间序列预测性能。


<details>
  <summary>Details</summary>
Motivation: 深度学习在黑盒性质和无法显式建模随机过程方面存在局限，尤其在非平稳数据中表现不佳。

Method: 提出Neural MJD模型，将预测问题建模为SDE模拟，结合时间非齐次Itô扩散和复合泊松过程，并引入截断机制和优化求解器。

Result: 在合成和真实数据集上，Neural MJD优于现有深度学习和统计学习方法。

Conclusion: Neural MJD通过显式建模随机过程，显著提升了非平稳时间序列预测的泛化能力。
...
### [314] [Communication Efficient Adaptive Model-Driven Quantum Federated Learning](https://arxiv.org/abs/2506.04548)
...
Method: 提出mdQFL算法，结合训练和更新个性化，测试泛化能力，适用于多种设备数量和非独立同分布数据。

Result: 实验表明，mdQFL在Qiskit环境中降低了50%通信成本，同时保持或提升模型准确性，并优化本地模型训练。

Conclusion: mdQFL在量子联邦学习中表现出高效性和适应性，为其他联邦学习场景提供了新思路。
...
### [315] [Unsupervised Machine Learning for Scientific Discovery: Workflow and Best Practices](https://arxiv.org/abs/2506.04553)
...
Method: 提出结构化工作流程，包括问题定义、数据准备、建模、验证及结果文档化，并以天文学案例验证。

Result: 案例研究表明，精心设计的工作流程能显著提升无监督学习在科学发现中的效果。

Conclusion: 标准化工作流程有助于提高无监督学习的科学价值，推动可重复性研究。
...
### [316] [Clustering and Median Aggregation Improve Differentially Private Inference](https://arxiv.org/abs/2506.04566)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法通过均匀采样敏感输入生成合成文本，但在异构主题下效果不佳，因此需要改进。

Method: 聚类输入数据后选择推理批次，并通过私有计算中位数（而非平均值）聚合下一个标记的统计信息。

Result: 新方法在代表性和下游任务性能上均有提升，且隐私成本显著降低。

Conclusion: 该方法能高效生成高质量合成数据，同时提供更强的隐私保护。
...
### [317] [StatsMerging: Statistics-Guided Model Merging via Task-Specific Teacher Distillation](https://arxiv.org/abs/2506.04567)
*Ranjith Merugu,Bryan Bo Cao,Shubham Jain*

Main category: cs.LG

TL;DR: StatsMerging是一种基于统计的轻量级模型合并方法，利用SVD和任务特定权重分布指导合并，无需真实标签或测试样本，在多个任务中表现优异。
...
Method: 利用SVD的奇异值捕捉任务特定权重分布，通过轻量级学习器StatsMergeLearner建模权重分布，并引入任务特定教师蒸馏处理异构架构模型。

Result: 在八项任务中，StatsMerging在整体准确性、泛化能力和鲁棒性上优于现有技术。

Conclusion: StatsMerging为模型合并提供了一种高效且无需标签的解决方案，具有广泛的应用潜力。
...
### [318] [Scaling Laws for Robust Comparison of Open Foundation Language-Vision Models and Datasets](https://arxiv.org/abs/2506.04598)
*Marianna Nezhurina,Tomer Porian,Giovanni Pucceti,Tommie Kerssies,Romain Beaumont,Mehdi Cherti,Jenia Jitsev*

Main category: cs.LG

TL;DR: 论文通过推导缩放定律，比较了CLIP和MaMMUT两种语言-视觉学习方法的性能，发现MaMMUT在扩展性和样本效率上优于CLIP，并验证了其在多种下游任务和数据集中的一致性。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索如何利用缩放定律进行模型和数据集的比较，以优化预训练方法的选择。

Method: 方法包括密集测量不同模型和样本规模的缩放定律，并比较CLIP和MaMMUT的性能。

Result: 结果显示MaMMUT在扩展性和样本效率上优于CLIP，并在多种任务和数据集中表现一致。

Conclusion: 结论是准确推导缩放定律可避免单点比较的误导，为系统优化开源基础模型和数据集提供了方法。
...
### [319] [Ignoring Directionality Leads to Compromised Graph Neural Network Explanations](https://arxiv.org/abs/2506.04608)
*Changsheng Sun,Xinke Li,Jin Song Dong*

Main category: cs.LG

TL;DR: 论文指出图神经网络（GNNs）中常见的图对称化操作会丢失方向信息，影响解释的可靠性。通过理论和实证研究，作者证明保留方向语义能显著提升解释质量。


<details>
  <summary>Details</summary>
Motivation: 在关键领域应用GNNs时，可靠的解释对支持人类决策至关重要。然而，图对称化操作丢弃了方向信息，导致信息损失和误导性解释。

Method: 通过理论和实证研究，分析图对称化对解释保真度的影响，并探讨保留方向语义的方法。

Result: 研究发现保留方向语义能显著提升GNNs的解释质量，为人类决策提供更可靠的洞察。

Conclusion: 研究强调了在安全关键应用中采用方向感知的GNN解释方法的必要性。
...
### [320] [Exploring bidirectional bounds for minimax-training of Energy-based models](https://arxiv.org/abs/2506.04609)
...
<details>
  <summary>Details</summary>
Motivation: 能量模型（EBM）在密度估计中具有优雅的框架，但训练困难。传统方法通过极小化下界训练可能导致不稳定。

Method: 研究了四种基于不同视角的对数似然边界，包括生成器雅可比矩阵的奇异值和互信息的下界，以及梯度惩罚和扩散过程的上界。
...
### [321] [Composing Agents to Minimize Worst-case Risk](https://arxiv.org/abs/2506.04632)
*Guruprerana Shabadi,Rajeev Alur*

Main category: cs.LG

TL;DR: 本文提出了一种基于有向无环图（agent graph）的AI代理组合优化方法，旨在最大化任务成功率的同时最小化风险（如安全、公平、隐私）。通过近似尾部风险（value-at-risk）和动态规划，算法高效地找到近似最优代理组合。


<details>
  <summary>Details</summary>
Motivation: 现代AI系统需要将复杂任务分解为子任务并由多个代理完成，但需同时考虑任务成功率和风险（如安全、公平、隐私）。本文旨在解决代理组合的优化问题，尤其是尾部风险的最小化。

Method: 将代理工作流建模为有向无环图（agent graph），定义尾部风险为损失分布的分位数（value-at-risk），提出基于联合边界和动态规划的高效近似算法。

Result: 算法在视频游戏控制基准测试中表现优异，能有效近似尾部风险并找到最优代理组合。理论证明该近似对广泛损失函数渐近最优。

Conclusion: 本文提出的方法能高效优化代理组合，平衡任务成功率和风险，适用于实际部署中的复杂AI系统。
...
### [322] [Inference economics of language models](https://arxiv.org/abs/2506.04645)
*Ege Erdil*

Main category: cs.LG

TL;DR: 论文提出了一种理论模型，用于权衡大规模部署LLM推理时的每个令牌成本与串行令牌生成速度之间的经济关系。


<details>
  <summary>Details</summary>
Motivation: 研究动机是优化大规模语言模型推理的经济性和效率，特别是在算术、内存带宽、网络带宽和延迟等约束条件下。

Method: 方法包括建立一个理论模型，考虑多种约束条件，并通过优化并行设置和批量大小来找到在给定令牌成本下最大化串行推理速度的方案。

Result: 结果计算了流行语言模型的串行速度与每个令牌成本的帕累托前沿。

Conclusion: 结论是提出的模型能够有效权衡成本与速度，为大规模LLM部署提供优化方案。
...
### [323] [Neural Network Reprogrammability: A Unified Theme on Model Reprogramming, Prompt Tuning, and Prompt Instruction](https://arxiv.org/abs/2506.04650)
*Zesheng Ye,Chengyi Cai,Ruijiang Dong,Jianzhong Qi,Lei Feng,Pin-Yu Chen,Feng Liu*

Main category: cs.LG

TL;DR: 该论文提出了神经网络可重编程性作为统一框架，将模型重编程、提示调整和提示指令等主流模型适应技术联系起来，通过信息操作实现预训练模型的适配。


<details>
  <summary>Details</summary>
Motivation: 随着大规模预训练基础模型的规模和能力不断扩大，如何高效地将其适配到特定下游任务变得至关重要。现有方法缺乏对其相互关系的清晰理解。

Method: 引入神经网络可重编程性框架，通过操作模型接口信息（如输入扰动、中间层插入令牌或上下文任务示例）来重定向模型行为。提出了基于信息操作的分类法，涵盖格式、位置、操作符和输出对齐四个维度。

Result: 该框架适用于多种数据模态和模型架构，揭示了现有技术（如上下文学习和思维链提示）的理论联系和实际区别。

Conclusion: 神经网络可重编程性是高效模型适配的基本范式，论文还指出了未来的研究方向和技术挑战。
...
### [324] [The Oversmoothing Fallacy: A Misguided Narrative in GNN Research](https://arxiv.org/abs/2506.04653)
*MoonJeong Park,Sunghyun Choi,Jaeseung Heo,Eunhyeok Park,Dongwoo Kim*

Main category: cs.LG

TL;DR: 本文认为图神经网络（GNN）中的过平滑问题被夸大，并提倡进一步探索深层GNN架构。研究表明，过平滑与梯度消失混淆，且经典方法如跳跃连接和归一化可解决性能下降问题。


<details>
  <summary>Details</summary>
Motivation: 探讨过平滑问题对深层GNN性能的影响是否被高估，并澄清其与梯度消失的区别。

Method: 分析GNN的三种核心操作（聚合、线性变换和非线性激活），指出过平滑与梯度消失的混淆，并验证经典方法（如跳跃连接和归一化）的有效性。

Result: 研究表明过平滑并非GNN独有，且经典方法能有效支持深层GNN的构建。

Conclusion: 过平滑问题被误解，深层GNN具有潜力，经典方法可解决性能问题。
...
### [325] [Noise-Resistant Label Reconstruction Feature Selection for Partial Multi-Label Learning](https://arxiv.org/abs/2506.04669)
*Wanfu Gao,Hanlin Pan,Qingqi Han,Kunpeng Liu*

Main category: cs.LG

TL;DR: 提出了一种针对部分多标签学习（PML）的特征选择方法，通过标签关系的抗噪性和标签连通性解决维度诅咒问题，提升模型分类性能。
...
### [326] [FedAPM: Federated Learning via ADMM with Partial Model Personalization](https://arxiv.org/abs/2506.04672)
*Shengkun Zhu,Feiteng Nie,Jinshan Zeng,Sheng Wang,Yuan Sun,Yuan Yao,Shangfeng Chen,Quanqing Xu,Chuanhui Yang*

Main category: cs.LG

TL;DR: 论文提出了一种基于ADMM的联邦学习框架FedAPM，通过引入一阶和二阶近端项来缓解客户端漂移，实验表明其在收敛速度和准确性上优于现有方法。
...
Method: 使用ADMM框架，构造包含一阶和二阶近端项的增广拉格朗日函数，通过显式估计拉格朗日乘子实现稳定收敛。

Result: FedAPM在四种异构数据集上表现优异，测试准确率、F1分数和AUC分别平均提升12.3%、16.4%和18.0%，且通信轮次更少。

Conclusion: FedAPM通过有效缓解客户端漂移，显著提升了联邦学习的效率和性能。
...
### [327] [The cost of ensembling: is it always worth combining?](https://arxiv.org/abs/2506.04677)
*Marco Zanotti*

Main category: cs.LG

TL;DR: 论文研究了时间序列预测中集成学习的准确性与计算成本之间的权衡，通过评估多种模型和配置，发现小规模集成在保持高精度的同时显著降低成本。


<details>
  <summary>Details</summary>
Motivation: 随着数据集规模和预测模型复杂度的增加，准确性与计算成本之间的权衡成为重要课题，尤其是在时间序列预测的集成学习中。

Method: 评估了十个基础模型和八种集成配置，在两个大型零售数据集（M5和VN1）上测试了点预测和概率预测的准确性，同时考虑了不同的重新训练频率。

Result: 集成方法显著提高了预测性能，尤其是概率预测，但计算成本较高。减少重新训练频率可降低成本且对精度影响较小，小型集成（2-3个模型）通常能达到接近最优的结果。

Conclusion: 通过精心设计集成和选择重新训练策略，可以实现准确、稳健且成本效益高的预测，适用于实际应用，支持可持续AI的目标。
...
### [328] [Urania: Differentially Private Insights into AI Use](https://arxiv.org/abs/2506.04681)
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是为LLM聊天机器人交互提供隐私保护的洞察生成方法，确保用户数据隐私不被泄露。

Method: 方法包括私有聚类机制、多种关键词提取方法（频率、TF-IDF、LLM引导），以及DP工具（聚类、分区选择、直方图摘要）。

Result: 评估表明，Urania在保留语义内容和隐私的同时，能够提取有意义的对话洞察，且DP管道具有更强的鲁棒性。

Conclusion: 结论是Urania框架成功平衡了数据效用和隐私保护，适用于需要严格隐私的场景。
...
### [329] [Towards Better Generalization via Distributional Input Projection Network](https://arxiv.org/abs/2506.04690)
...
<details>
  <summary>Details</summary>
Motivation: 过参数化模型的训练损失无法充分反映泛化性能，而平滑性虽与泛化相关，但直接在神经网络中实现平滑性具有挑战性。

Method: 提出DIPNet框架，通过在每层将输入投影到可学习的分布中，诱导更平滑的损失景观。
...
### [330] [Influence Functions for Edge Edits in Non-Convex Graph Neural Networks](https://arxiv.org/abs/2506.04694)
...
Result: 实验证明该方法能准确预测GNN中边修改的影响，并适用于图重连和对抗攻击。
...
### [331] [On the Mechanism of Reasoning Pattern Selection in Reinforcement Learning for Language Models](https://arxiv.org/abs/2506.04695)
*Xingwu Chen,Tianle Li,Difan Zou*

Main category: cs.LG

TL;DR: 论文研究了强化学习在提升模型推理能力中的作用，提出RLVR方法，发现其通过优化推理模式选择实现性能提升，并分析了其收敛性和训练动态。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习在提升模型能力方面表现出色，但其如何改善推理能力的机制尚不明确，因此需要系统性研究。

Method: 提出RLVR方法，通过实验和理论分析研究其优化推理模式选择的效果，并探讨收敛性和训练动态。

Result: RLVR能优先选择高成功率的推理模式，且理论分析揭示了两种收敛模式：快速收敛和较慢优化。

Conclusion: RLVR的理论分析深化了对强化学习在模型微调中作用的理解，为提升推理能力提供了新思路。
...
### [332] [Enhanced Drought Analysis in Bangladesh: A Machine Learning Approach for Severity Classification Using Satellite Data](https://arxiv.org/abs/2506.04696)
*Tonmoy Paul,Mrittika Devi Mati,Md. Mahmudul Islam*

Main category: cs.LG

TL;DR: 提出了一种基于卫星数据的机器学习框架，用于分类孟加拉国38个地区的干旱严重程度，填补了传统指数和现有模型的不足。


<details>
  <summary>Details</summary>
Motivation: 孟加拉国因地理和人为因素易受干旱影响，传统干旱指数（如SPI和PDSI）忽略关键因素，且现有机器学习模型在该地区应用不足。

Method: 结合K-means和贝叶斯高斯混合聚类算法，以及KNN、随机森林、决策树和朴素贝叶斯分类模型，整合2012-2024年的气象数据。

Result: 成功将干旱严重程度分类为不同等级，但显示出区域间干旱脆弱性的显著差异。
...
### [333] [Explicit Density Approximation for Neural Implicit Samplers Using a Bernstein-Based Convex Divergence](https://arxiv.org/abs/2506.04700)
*José Manuel de Frutos,Manuel A. Vázquez,Pablo M. Olmos,Joaquín Míguez*

Main category: cs.LG

TL;DR: 论文提出了一种名为dual-ISL的新目标函数，用于训练隐式生成模型，通过交换目标分布和模型分布的角色，解决了传统ISL框架中的非凸优化问题。


<details>
  <summary>Details</summary>
Motivation: 传统ISL框架在训练隐式生成模型时存在非凸优化问题，且经典差异度量（如KL或Wasserstein距离）缺乏连续性和凸性。dual-ISL旨在解决这些问题。

Method: 提出dual-ISL目标函数，将其视为密度比的L2投影到Bernstein多项式基上，并扩展到多元设置。

Result: dual-ISL具有连续性和凸性，收敛更快，训练更稳定，能有效防止模式崩溃，并提供显式密度近似。

Conclusion: dual-ISL在理论和实践中均优于传统ISL和其他隐式生成方法，为生成模型训练提供了更优的工具。
...
### [334] [UNO: Unlearning via Orthogonalization in Generative models](https://arxiv.org/abs/2506.04712)
...
<details>
  <summary>Details</summary>
Motivation: 随着生成模型的广泛应用，如何高效移除特定数据的影响（如隐私、法律或有害内容）变得至关重要，而传统方法需要昂贵的重新训练。

Method: 基于损失梯度正交化的快速遗忘算法，旨在选择性移除数据影响而不需从头训练。

Result: 在MNIST和CelebA数据上，算法比梯度手术等方法快几个数量级，同时保持模型性能。

Conclusion: 该算法在高效遗忘数据的同时，显著提升了速度和模型保真度。
...
### [335] [Multi-Layer GRPO: Enhancing Reasoning and Self-Correction in Large Language Models](https://arxiv.org/abs/2506.04746)
*Fei Ding,Baiqiao Wang,Zijian Zeng,Youwei Wang*

Main category: cs.LG

TL;DR: MGRPO（多层GRPO）通过两层GRPO机制提升LLMs的推理能力，第一层生成初始响应，第二层纠正错误，显著优于标准GRPO。


<details>
  <summary>Details</summary>
Motivation: 标准GRPO缺乏中间监督，导致探索效率低，单点错误可能破坏整个推理链，影响训练稳定性。

Method: MGRPO采用两层GRPO：第一层生成初始响应，第二层纠正错误，形成自校正循环。

Result: 实验表明，MGRPO在数学推理任务中显著优于标准GRPO，提升了推理和自校正能力。

Conclusion: MGRPO通过自校正机制提供隐式监督，无需密集标注，显著提升模型性能。
...
### [336] [Log-Linear Attention](https://arxiv.org/abs/2506.04761)
...
<details>
  <summary>Details</summary>
Motivation: 传统Transformer注意力机制的二次计算和线性内存复杂度是瓶颈，而线性注意力和状态空间模型虽然高效，但受限于固定大小的隐藏状态。

Method: 开发了log-linear attention，用对数增长的隐藏状态集替代固定大小的隐藏状态，并通过特定增长函数实现并行化，计算成本为对数线性。

Result: 在Mamba-2和Gated DeltaNet等架构上应用log-linear attention，表现优于线性时间变体。

Conclusion: log-linear attention是一种通用框架，能够高效且灵活地应用于现有线性注意力变体。
...
### [337] [OpenGT: A Comprehensive Benchmark For Graph Transformers](https://arxiv.org/abs/2506.04765)
*Jiachen Tang,Zhonghao Wang,Sirui Chen,Sheng Zhou,Jiawei Chen,Jiajun Bu*

Main category: cs.LG

TL;DR: 论文介绍了OpenGT，一个用于Graph Transformers（GTs）的综合基准测试库，旨在解决GTs设计选择和应用场景的未解问题。


<details>
  <summary>Details</summary>
Motivation: 当前GTs的设计空间和应用场景尚不明确，缺乏统一的基准测试库，阻碍了其进一步发展和理解。

Method: 通过OpenGT库，标准化实验设置，纳入多种GNNs和GTs，从多角度评估GTs的性能。
...
### [338] [Improved Regret Bounds for Linear Bandits with Heavy-Tailed Rewards](https://arxiv.org/abs/2506.04775)
*Artin Tajdini,Jonathan Scarlett,Kevin Jamieson*

Main category: cs.LG

TL;DR: 论文研究了具有重尾奖励的随机线性老虎机问题，改进了上下界，提出了一种新的基于实验设计的算法，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 研究重尾奖励下的线性老虎机问题，改进现有上下界，填补理论与实际应用的差距。

Method: 提出了一种基于实验设计的消除算法，优化了维度依赖性和奖励分布特性。

Result: 新算法在维度依赖性和奖励分布上显著优于现有方法，同时建立了更紧的下界。

Conclusion: 论文在理论和实际应用上均取得了重要进展，为处理重尾奖励问题提供了新思路。
...
### [339] [Kernel $k$-Medoids as General Vector Quantization](https://arxiv.org/abs/2506.04786)
...
<details>
  <summary>Details</summary>
Motivation: 探索k-medoids和KDE这两种看似无关的VQ方法之间的联系，以理解其几何解释和权重参数。

Method: 通过QUBO框架比较k-medoids和KDE的启发式与理论优化方法。

Result: 发现KDE-QUBO在特定条件下是k-medoids-QUBO的特例，揭示了两者的结构关系。

Conclusion: 研究为VQ方法的权重参数提供了新的几何解释，并统一了两种方法的理论视角。
...
### [340] [Adaptive Preconditioners Trigger Loss Spikes in Adam](https://arxiv.org/abs/2506.04805)
*Zhiwei Bai,Zhangchen Zhou,Jiajie Zhao,Xiaolong Li,Zhiyu Li,Feiyu Xiong,Hongkang Yang,Yaoyu Zhang,Zhi-Qin John Xu*

Main category: cs.LG

TL;DR: 研究发现Adam优化器中的自适应预条件器可能导致训练中的损失突增，而非传统认为的损失景观特性。


<details>
  <summary>Details</summary>
Motivation: 探讨Adam优化器在训练神经网络时出现损失突增的根本原因。

Method: 分析Adam优化器的预条件器机制，识别关键状态并验证其对损失突增的影响。

Result: 发现预条件器的最大特征值超过稳定性阈值时会导致不稳定性和损失突增。

Conclusion: Adam的自适应预条件器机制是损失突增的主要原因，这一发现通过多种网络架构实验得到验证。
...
### [341] [LogicPuzzleRL: Cultivating Robust Mathematical Reasoning in LLMs via Reinforcement Learning](https://arxiv.org/abs/2506.04821)
...
Method: 采用“玩中学”框架，通过强化学习在七种定制逻辑谜题上微调LLMs，每种谜题设计用于培养特定推理技能（如约束传播、空间一致性等）。

Result: 该方法显著提升了LLMs在数学基准测试中的分布外性能，尤其是在需要多步推理的中等难度问题上。

Conclusion: 研究表明，逻辑谜题的强化学习训练重塑了LLMs的内部推理机制，使其能够实现更鲁棒和组合式的泛化，而无需依赖任务特定的符号工具。
...
### [342] [From EHRs to Patient Pathways: Scalable Modeling of Longitudinal Health Trajectories with LLMs](https://arxiv.org/abs/2506.04831)
*Chantal Pellegrini,Ege Özsoy,David Bani-Harouni,Matthias Keicher,Nassir Navab*

Main category: cs.LG

TL;DR: 提出了一种名为EHR2Path的新方法，通过将多样化的电子健康记录数据转化为结构化表示，并设计了一个全面的路径预测模型，以预测未来的健康轨迹。该方法在预测性能和效率上优于现有基线。
...
Method: 将电子健康记录数据转化为结构化表示，设计EHR2Path模型，并引入一种新颖的摘要机制，将长期时间上下文嵌入到特定主题的摘要标记中。

Result: EHR2Path在下一步预测和纵向模拟中表现出色，优于现有基线，并能详细模拟患者轨迹。

Conclusion: EHR2Path为预测性和个性化医疗提供了一条新路径，能够支持多种评估任务。
...
### [343] [Sparse Autoencoders, Again?](https://arxiv.org/abs/2506.04859)
...
<details>
  <summary>Details</summary>
Motivation: 尽管稀疏自编码器（SAEs）和变分自编码器（VAEs）在建模低维潜在结构方面具有广泛应用，但其方法多年来变化不大，且存在未被充分认识的弱点。

Method: 提出了一种混合替代模型，结合了SAEs和VAEs的优点，并提供了理论支持，证明其能恢复分布在多个流形上的结构化数据。

Result: 实验证明，该模型在合成和真实数据集上能更准确地估计流形维度，生成更稀疏的潜在表示，且不牺牲重建误差，性能优于SAEs、VAEs和扩散模型。

Conclusion: 该混合模型在图像和语言模型激活模式等领域表现出色，克服了传统方法的局限性。
...
### [344] [Aligning Multimodal Representations through an Information Bottleneck](https://arxiv.org/abs/2506.04870)
...
Method: 通过信息瓶颈原理理论分析，并提出一种基于变分近似的正则化损失函数。

Result: 实验验证了正则化项在控制实验和实际应用中的有效性。

Conclusion: 提出的正则化方法能显著提高多模态表示的对齐性。
...
### [345] [There Was Never a Bottleneck in Concept Bottleneck Models](https://arxiv.org/abs/2506.04877)
*Antonio Almudévar,José Miguel Hernández-Lobato,Alfonso Ortega*

Main category: cs.LG

TL;DR: 论文提出最小概念瓶颈模型（MCBMs），通过信息瓶颈目标改进概念瓶颈模型（CBMs），确保每个表示组件仅保留与其对应概念相关的信息。


<details>
  <summary>Details</summary>
Motivation: CBMs未能真正限制表示组件仅编码特定概念信息，影响可解释性和干预有效性。

Method: 引入信息瓶颈（IB）目标，通过变分正则化项约束表示组件。

Result: MCBMs支持概念级干预并提供理论保证，同时符合贝叶斯原则。

Conclusion: MCBMs在可解释性和设计灵活性上优于CBMs。
...
### [346] [Gaussian Process Diffeomorphic Statistical Shape Modelling Outperforms Angle-Based Methods for Assessment of Hip Dysplasia](https://arxiv.org/abs/2506.04886)
*Allen Paul,George Grammatopoulos,Adwaye Rambojun,Neill D. F. Campbell,Harinderjit S. Gill,Tony Shardlow*

Main category: cs.LG

TL;DR: 论文提出了一种基于CT扫描的半自动化髋关节发育不良分类方法GPDSSM，显著提高了分类准确性并节省临床时间。


<details>
  <summary>Details</summary>
Motivation: 髋关节发育不良是骨关节炎的风险因素，早期诊断对手术干预至关重要，但传统方法依赖手动测量，效率低。

Method: 结合高斯过程隐变量模型和微分同胚，构建GPDSSM统计形状模型，利用192例CT扫描数据（100训练，92测试）。

Result: GPDSSM能有效区分发育不良样本（AUC 96.2%），优于传统角度测量方法（AUC 91.2%），并识别异常区域。

Conclusion: GPDSSM提高了分类准确性，减少了临床手动操作需求，具有潜在应用价值。
...
### [347] [Dissecting Long Reasoning Models: An Empirical Study](https://arxiv.org/abs/2506.04913)
*Yongyu Mu,Jiali Zeng,Bei Li,Xinyan Guan,Fandong Meng,Jie Zhou,Tong Xiao,Jingbo Zhu*

Main category: cs.LG

TL;DR: 本文研究了强化学习中正负样本的作用、数据效率问题以及模型性能不稳定性，提出了改进策略。


<details>
  <summary>Details</summary>
Motivation: 探索强化学习中正负样本的不同作用、数据效率低下的原因以及模型性能不稳定的根源，并提出解决方案。

Method: 系统分析正负样本的作用，提出相对长度奖励和离线样本注入策略，并通过多次评估解决性能不稳定问题。

Result: 发现负样本显著提升泛化能力，数据效率问题可通过简单策略改善，多次评估能缓解性能不稳定。

Conclusion: 正负样本的作用不同，数据效率和性能不稳定问题可通过策略优化解决，为强化学习模型训练提供了新思路。
...
### [348] [Predicting ICU In-Hospital Mortality Using Adaptive Transformer Layer Fusion](https://arxiv.org/abs/2506.04924)
...
TL;DR: ALFIA是一种基于注意力机制的模块化架构，用于ICU高风险患者的早期识别，通过融合多层语义特征和自适应权重机制，显著提升了性能。
...
### [349] [FPTQuant: Function-Preserving Transforms for LLM Quantization](https://arxiv.org/abs/2506.04985)
...
<details>
  <summary>Details</summary>
Motivation: LLMs在推理时计算和能源消耗大，传统量化方法因异常值问题导致性能显著下降。

Method: 引入四种FPTs：预RoPE变换、值变换、MLP块内缩放变换和动态缩放变换，通过局部和端到端训练减少异常值。

Result: FPTQuant实现静态INT4量化，速度提升3.9倍，精度与FP相当或优于多数现有方法。

Conclusion: FPTQuant在精度和速度间取得优秀平衡，无需定制内核，推理时几乎无额外开销。
...
### [350] [Cautious Optimism: A Meta-Algorithm for Near-Constant Regret in General Games](https://arxiv.org/abs/2506.05005)
...
Method: 方法是将任何Follow-the-Regularized-Leader (FTRL)实例输入到Cautious Optimism框架中，通过最小计算开销调整学习速度，生成加速的无遗憾学习算法。

Result: 结果展示了在自对弈中实现近乎最优的O_T(log T)遗憾，同时在对抗场景中保持最优的O(√T)遗憾。

Conclusion: 结论表明，无需依赖单调步长，Cautious Optimistic FTRL为一般游戏中的快速学习提供了一条新路径。
...
### [351] [Towards Reasonable Concept Bottleneck Models](https://arxiv.org/abs/2506.05014)
...
<details>
  <summary>Details</summary>
Motivation: 现有概念瓶颈模型在任务表现和概念重要性之间难以平衡，CREAM旨在通过结构化关系和正则化侧通道解决这一问题。

Method: CREAM通过嵌入专家定义的双向概念关系和概念到任务关系，同时切断不必要的信息流，并引入正则化侧通道以增强性能。

Result: 实验表明，CREAM主要依赖概念，任务表现与黑盒模型相当，同时嵌入的关系简化了模型干预并减少概念泄漏。

Conclusion: CREAM在保持概念重要性的同时实现了高性能任务表现，并通过结构化关系提升了模型的可解释性和干预能力。
...
### [352] [Multi-Point Proximity Encoding For Vector-Mode Geospatial Machine Learning](https://arxiv.org/abs/2506.05016)
*John Collins*

Main category: cs.LG

TL;DR: 本文提出了一种基于参考点距离的多点邻近（MPP）编码方法，用于将矢量地理空间数据编码为适合机器学习模型的向量形式，性能优于栅格化方法。


<details>
  <summary>Details</summary>
Motivation: 矢量地理空间数据（点、线、面）需要编码为适合传统机器学习和人工智能模型的形式，现有方法（如栅格化）存在局限性。

Method: MPP编码通过计算形状与区域内一组参考点的缩放距离，捕捉几何特征和空间关系，适用于任何形状类型。

Result: MPP编码具有形状中心性和连续性，能区分空间对象的几何特征，并高精度捕捉空间关系，性能优于栅格化方法。

Conclusion: MPP编码是一种高效且通用的矢量地理空间数据编码方法，适用于机器学习模型参数化。
...
### [353] [Tuning the Right Foundation Models is What you Need for Partial Label Learning](https://arxiv.org/abs/2506.05027)
...
<details>
  <summary>Details</summary>
Motivation: 现实应用中部分标签学习的挑战，以及现有方法对基础模型关注不足的问题。

Method: 提出了PartialCLIP框架，评估了11种基础模型在13种PLL方法和8个数据集上的表现。

Result: 发现基础模型能显著提升性能，但性能相似且受模型选择和适应策略影响。PartialCLIP展示了文本嵌入初始化和零样本CLIP的有效性。
...
### [354] [Identifying and Understanding Cross-Class Features in Adversarial Training](https://arxiv.org/abs/2506.05032)
*Zeming Wei,Yiwen Guo,Yisen Wang*

Main category: cs.LG

TL;DR: 本文通过类间特征归因的视角研究对抗训练（AT），发现跨类特征对AT的关键影响，并揭示了AT训练动态中的新机制。


<details>
  <summary>Details</summary>
Motivation: 对抗训练（AT）是提升深度神经网络对抗攻击鲁棒性的有效方法，但其训练机制和动态仍待深入研究。本文旨在通过类间特征归因的视角揭示AT的新机制。

Method: 通过合成数据模型和系统实验，研究跨类特征在AT中的作用及其动态变化。

Result: 研究发现，AT初期模型倾向于学习更多跨类特征，而随着训练深入，模型更依赖类特定特征，导致鲁棒过拟合。

Conclusion: 本文为AT机制提供了新视角，统一解释了软标签训练的优势和鲁棒过拟合现象。
...
### [355] [TIMING: Temporality-Aware Integrated Gradients for Time Series Explanation](https://arxiv.org/abs/2506.05035)
*Hyeongwon Jang,Changhun Kim,Eunho Yang*

Main category: cs.LG

TL;DR: 论文提出TIMING方法，通过增强集成梯度（IG）的时间感知能力，改进时间序列可解释AI（XAI）中对正负关键点的识别，并引入新评估指标CPD和CPP。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列XAI方法仅关注点状归因量，忽略方向性影响，且评估指标未能有效区分正负贡献。

Method: 提出TIMING方法，改进IG以融入时间关系，避免分布外样本，并引入CPD和CPP评估指标。

Result: TIMING在合成和真实时间序列基准测试中优于现有XAI基线方法。

Conclusion: TIMING通过时间感知增强IG，显著提升时间序列XAI的性能和解释能力。
...
### [356] [iN2V: Bringing Transductive Node Embeddings to Inductive Graphs](https://arxiv.org/abs/2506.05039)
*Nicolas Lell,Ansgar Scherp*

Main category: cs.LG

TL;DR: iN2V是一种将转导式节点嵌入方法（如node2vec）扩展到归纳式设置的方法，通过后处理计算新节点的嵌入，并改进训练过程。实验表明，iN2V能有效提升节点分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有节点嵌入方法（如node2vec）仅适用于转导式设置，无法处理训练时未见的新节点。iN2V旨在解决这一问题，使其适用于归纳式场景。

Method: iN2V结合后处理计算新节点嵌入，并改进原始N2V训练过程以适配后处理。

Result: 实验显示，iN2V平均提升节点分类1个百分点，最高可达6个百分点。

Conclusion: iN2V是一种灵活的方法，可与其他嵌入方法结合，适用于归纳式节点表示学习。
...
### [357] [Reliably detecting model failures in deployment without labels](https://arxiv.org/abs/2506.05047)
*Viet Nguyen Changjian Shui,Vijay Giri,Siddarth Arya,Amol Verma,Fahad Razak,Rahul G. Krishnan*

Main category: cs.LG

TL;DR: 该论文提出了一种名为D3M的算法，用于监测模型部署后的性能退化（PDD），通过预测模型的分歧来实现高效监控。
...
Method: 提出基于预测模型分歧的D3M算法，能够在非退化变化下保持低误报率，并在退化变化下实现高真报率。

Result: 在标准基准和真实世界大规模内科数据集上的实验证明了该框架的有效性。

Conclusion: D3M是一种实用的监控算法，适合作为高风险机器学习管道的预警机制。
...
### [358] [NIMO: a Nonlinear Interpretable MOdel](https://arxiv.org/abs/2506.05059)
*Shijian Xu,Marcello Massimo Negri,Volker Roth*

Main category: cs.LG

TL;DR: NIMO（非线性可解释模型）结合神经网络和线性模型的优势，通过神经网络学习线性预测的非线性修正，同时保持线性系数的可解释性。


<details>
  <summary>Details</summary>
Motivation: 神经网络难以解释，而线性模型虽可解释但表达能力有限。NIMO旨在结合两者的优点。

Method: 使用基于轮廓似然的优化算法，同时优化神经网络参数并解析更新线性系数，结合自适应岭回归引入稀疏性约束。

Result: 实验表明，NIMO能恢复线性系数并显著提升预测准确性，同时保持线性模型的可解释性。

Conclusion: NIMO在保持线性系数可解释性的同时，优于其他混合方法，适用于多种回归和分类任务。
...
### [359] [UnHiPPO: Uncertainty-aware Initialization for State Space Models](https://arxiv.org/abs/2506.05065)
...
Conclusion: 提出的不确定性感知初始化方法有效提升了模型在噪声环境中的性能。
...
### [360] [Semi-Implicit Variational Inference via Kernelized Path Gradient Descent](https://arxiv.org/abs/2506.05088)
*Tobias Pielok,Bernd Bischl,David Rügamer*

Main category: cs.LG

TL;DR: 提出了一种基于核化的KL散度估计器，通过非参数平滑稳定训练，并引入重要性采样校正以减少偏差。该方法在性能和训练效率上优于或匹配现有SIVI方法。


<details>
  <summary>Details</summary>
Motivation: 解决半隐式变分推断（SIVI）在高维设置中因KL散度训练导致的高方差和偏差问题，以及现有方法训练成本较高的问题。

Method: 提出核化KL散度估计器，结合非参数平滑和重要性采样校正，理论联系Stein变分梯度下降。

Result: 方法在性能和训练效率上优于或匹配现有SIVI方法，梯度方差更低，优化更稳定高效。

Conclusion: 该方法通过核化KL散度估计器和重要性采样校正，显著提升了SIVI在高维问题中的训练稳定性和效率。
...
### [361] [Privacy Amplification Through Synthetic Data: Insights from Linear Regression](https://arxiv.org/abs/2506.05101)
*Clément Pierquin,Aurélien Bellet,Marc Tommasi,Matthieu Boussard*

Main category: cs.LG

TL;DR: 论文研究了合成数据在隐私保护中的隐私放大现象，通过线性回归框架揭示了种子控制和随机输入对隐私泄露的影响。
...
Result: 发现种子控制下合成数据可能泄露大量信息，而随机输入下合成数据能放大隐私保护效果。
...
### [362] [Federated Isolation Forest for Efficient Anomaly Detection on Edge IoT Systems](https://arxiv.org/abs/2506.05138)
*Pavle Vasiljevic,Milica Matic,Miroslav Popovic*

Main category: cs.LG

TL;DR: 本文提出了一种基于隔离森林的联邦学习温度异常检测方法，适用于资源受限的边缘设备和IoT系统，实验显示其高准确性和低内存占用。


<details>
  <summary>Details</summary>
Motivation: 解决边缘设备和IoT系统中的隐私保护和资源效率问题，同时实现高效的异常检测。

Method: 采用基于隔离森林的FLiForest算法，结合Python和MicroPython联邦学习框架，进行温度异常检测。

Result: 实验结果显示96%的异常检测准确率和78%的精确率，内存占用低于160 KB。

Conclusion: 该方法适合资源受限环境，同时满足联邦学习的隐私保护和协作学习原则。
...
### [363] [Associative Memory and Generative Diffusion in the Zero-noise Limit](https://arxiv.org/abs/2506.05178)
*Joshua Hess,Quaid Morris*

Main category: cs.LG

TL;DR: 研究生成扩散与连续状态联想记忆模型的联系，强调Morse-Smale动力系统作为梯度联想记忆模型的通用逼近器，并探讨噪声水平降低时从生成到记忆的通用过渡。


<details>
  <summary>Details</summary>
Motivation: 探讨生成扩散模型与联想记忆模型之间的理论联系，揭示噪声水平变化对系统稳定性和行为的影响。

Method: 利用Morse-Smale动力系统作为理论框架，分析梯度联想记忆模型和扩散模型的结构稳定性，并通过分岔理论描述系统行为的变化。

Result: 在零噪声极限下，系统表现出稳定性，仅在某些孤立点出现分岔；通过分岔序列可以描述学习与生成景观的定性变化。

Conclusion: 研究为理解生成扩散模型与联想记忆模型的共同特性提供了理论支持，并揭示了噪声水平对系统行为的关键作用。
...
### [364] [TreeRPO: Tree Relative Policy Optimization](https://arxiv.org/abs/2506.05183)
*Zhicheng Yang,Zhijiang Guo,Yinya Huang,Xiaodan Liang,Yiwei Wang,Jing Tang*

Main category: cs.LG

TL;DR: 论文提出了一种名为TreeRPO的新方法，通过树采样估计推理步骤的奖励期望，显著提升LLMs的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在推理过程中对中间步骤的奖励指导不足，限制了LLMs的优化效果。

Method: 利用树采样估计推理步骤的奖励期望，结合GRPO的组相对奖励机制，生成细粒度奖励信号。

Result: TreeRPO将Qwen-2.5-Math的Pass@1准确率从19.0%提升至35.5%，并减少响应长度18.1%。
...
### [365] [Locality Preserving Markovian Transition for Instance Retrieval](https://arxiv.org/abs/2506.05196)
...
<details>
  <summary>Details</summary>
Motivation: 扩散重排序方法在数据流形建模中有效，但信号随距离衰减，导致局部区域外判别力下降。

Method: 提出LPMT框架，结合双向协作扩散（BCD）和局部状态嵌入（LSE），通过热力学马尔可夫转移（TMT）过程实现全局检索。

Result: 实验证明LPMT在多样化任务中提升了实例检索效果。

Conclusion: LPMT通过保留局部一致性和全局检索能力，有效解决了信号衰减问题。
...
### [366] [Transformers Meet In-Context Learning: A Universal Approximation Theory](https://arxiv.org/abs/2506.05200)
*Gen Li,Yuchen Jiao,Yu Huang,Yuting Wei,Yuxin Chen*

Main category: cs.LG

TL;DR: 该论文提出了一种基于通用函数逼近理论的方法，解释了大语言模型如何通过上下文学习完成任务，无需微调或参数更新。


<details>
  <summary>Details</summary>
Motivation: 理解Transformer如何通过上下文学习实现通用任务预测，而不依赖优化算法近似。

Method: 构建一个Transformer模型，仅通过少量上下文示例即可可靠预测，基于通用函数逼近理论。

Result: 证明了Transformer可以在不更新权重的情况下，通过上下文示例完成多种任务的预测。

Conclusion: 该方法揭示了Transformer如何同时学习通用表示并动态适应上下文示例，超越了传统优化算法的限制。
...
### [367] [Mitigating Degree Bias Adaptively with Hard-to-Learn Nodes in Graph Contrastive Learning](https://arxiv.org/abs/2506.05214)
*Jingyu Hu,Hongbo Bo,Jun Hong,Xiaowei Liu,Weiru Liu*

Main category: cs.LG

TL;DR: 论文提出HAR对比损失和SHARP框架，通过增加正样本对和自适应权重缓解GNN中的度偏差问题，实验验证其有效性。
...
### [368] [Learning Theory of Decentralized Robust Kernel-Based Learning Algorithm](https://arxiv.org/abs/2506.05215)
*Zhan Yu*

Main category: cs.LG

TL;DR: 提出了一种新的基于核的分散式鲁棒学习算法，利用图网络结构，通过鲁棒损失函数统一框架，实现鲁棒回归，并分析了其收敛性和学习理论。
...
Result: 算法在均方距离、RKHS范数和泛化误差上均达到最优学习率，参数σ对鲁棒性和收敛性至关重要。

Conclusion: 该算法为分散式鲁棒学习提供了统一框架，参数σ优化了鲁棒性和收敛性。
...
### [369] [A Unified Framework for Provably Efficient Algorithms to Estimate Shapley Values](https://arxiv.org/abs/2506.05216)
...
TL;DR: 论文提出了一个统一框架，涵盖KernelSHAP及相关估计器，并首次为KernelSHAP提供了理论保证，验证了其在小到中维度数据集上的有效性。


<details>
  <summary>Details</summary>
Motivation: Shapley值在解释机器学习模型决策时至关重要，但精确计算困难。现有随机估计器缺乏理论保证，尤其是KernelSHAP。

Method: 提出一个统一框架，涵盖KernelSHAP及相关估计器，采用有/无替换采样策略，并证明其非渐近理论保证。

Result: 在中小维度数据集上验证了方法的有效性，实现了低均方误差，并改进了实现以支持高维度数据集。

Conclusion: 该框架首次为KernelSHAP提供了理论保证，揭示了现有估计器间的权衡，并在实验中表现优于KernelSHAP库。
...
### [370] [Diagonal Batching Unlocks Parallelism in Recurrent Memory Transformers for Long Contexts](https://arxiv.org/abs/2506.05229)
...
Method: 提出了Diagonal Batching调度方案，通过运行时计算重排序实现并行化，无需重新训练现有模型。
...
### [371] [Progressive Tempering Sampler with Diffusion](https://arxiv.org/abs/2506.05231)
*Severi Rissanen,RuiKang OuYang,Jiajun He,Wenlin Chen,Markus Heinonen,Arno Solin,José Miguel Hernández-Lobato*

Main category: cs.LG

TL;DR: 论文提出了一种结合并行退火（PT）和扩散模型的渐进退火采样器（PTSD），通过跨温度序列训练扩散模型，提升神经采样器的效率和样本质量。


<details>
  <summary>Details</summary>
Motivation: 现有神经采样器在目标评估效率上仍不及并行退火（PT），而PT生成依赖样本且计算成本高。PTSD旨在结合两者优势，解决这些问题。

Method: PTSD通过温度序列训练扩散模型，利用PT提升神经采样器训练，并提出新方法结合高温扩散模型生成近似低温样本，再用MCMC微调。
...
### [372] [MesaNet: Sequence Modeling by Locally Optimal Test-Time Training](https://arxiv.org/abs/2506.05233)
...
TL;DR: 论文提出了一种基于Mesa层的改进方法，通过在线学习规则优化回归目标，实现了语言模型性能的提升，尤其是在长上下文任务中。


<details>
  <summary>Details</summary>
Motivation: 当前序列建模主要依赖Transformer，但其推理时内存和计算成本线性增长。近期研究通过线性化softmax操作，提出了高效的RNN模型（如DeltaNet、Mamba等），但仍需进一步优化。

Method: 改进Mesa层，采用快速共轭梯度求解器，在每一步最小化上下文损失，实现数值稳定性和分块并行化。

Result: 实验表明，该方法在语言建模困惑度和下游任务性能上优于先前RNN模型，尤其在长上下文任务中表现突出。

Conclusion: 通过增加推理时的计算成本（解决序列优化问题），显著提升了模型性能，与当前增加测试时计算以提升性能的趋势一致。
...
### [373] [Evaluating Sparse Autoencoders: From Shallow Design to Matching Pursuit](https://arxiv.org/abs/2506.05239)
*Valérie Costa,Thomas Fel,Ekdeep Singh Lubana,Bahareh Tolooshams,Demba Ba*

Main category: cs.LG

TL;DR: 论文评估稀疏自编码器（SAE）在MNIST数据集上的表现，发现当前浅层架构依赖准正交性假设，限制了提取相关特征的能力。作者提出多迭代SAE（MP-SAE），通过展开匹配追踪方法提取相关特征，并在手写数字生成等分层场景中实现重建效果的单调提升。


<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器（SAE）是解释神经网络表示的重要工具，但当前浅层架构依赖准正交性假设，无法有效提取相关特征。

Method: 提出多迭代SAE（MP-SAE），通过展开匹配追踪方法（Matching Pursuit）提取相关特征，并利用残差引导实现重建效果的单调提升。

Result: MP-SAE能够提取相关特征，并在手写数字生成等分层场景中实现更好的重建效果。

Conclusion: MP-SAE突破了浅层SAE的局限性，为提取相关特征提供了新方法，适用于分层场景。
...
### [374] [Aligning Latent Spaces with Flow Priors](https://arxiv.org/abs/2506.05240)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在潜在空间对齐中计算成本高，且需要解决ODE问题，本文旨在提出一种更高效的方法。

Method: 预训练一个流模型捕捉目标分布，然后通过对齐损失正则化潜在空间，避免昂贵的似然评估和ODE求解。

Result: 理论证明对齐损失是目标分布对数似然变分下界的可计算替代目标，实验验证了方法的有效性。

Conclusion: 该框架为潜在空间对齐提供了一种新的高效途径，兼具理论和实证支持。
...
### [375] [On the Convergence of Gradient Descent on Learning Transformers with Residual Connections](https://arxiv.org/abs/2506.05249)
*Zhen Qin,Jinxin Zhou,Zhihui Zhu*

Main category: cs.LG

TL;DR: 本文分析了单层Transformer的收敛行为，包括自注意力、前馈网络和残差连接，证明了梯度下降在适当初始化下具有线性收敛速率，并揭示了残差连接对优化稳定性的积极作用。


<details>
  <summary>Details</summary>
Motivation: 尽管Transformer模型在应用中表现出色，但其理论基础仍不完善，尤其是训练动态的理解不足。现有研究多关注孤立组件，而忽略了组件间的相互依赖关系。

Method: 研究采用结构完整的单层Transformer，分析其收敛行为，并扩展到多层架构。理论分析基于梯度下降的线性收敛速率和残差连接的作用。

Result: 在适当初始化下，梯度下降具有线性收敛速率，残差连接改善了输出矩阵的病态问题，提升了优化稳定性。实证结果支持理论发现。

Conclusion: 残差连接在Transformer中起到关键作用，通过改善输出矩阵的病态问题促进收敛稳定性，理论分析为理解Transformer训练动态提供了新视角。
...
### [376] [Learning long range dependencies through time reversal symmetry breaking](https://arxiv.org/abs/2506.05259)
*Guillaume Pourcel,Maxence Ernoult*

Main category: cs.LG

TL;DR: 论文提出了一种名为RHEL的算法，用于训练哈密顿系统，无需显式计算雅可比矩阵，且梯度估计无方差。RHEL在连续和离散时间下均有效，并展示了与BPTT相当的性能。
...
Method: 提出了RHEL算法，通过有限差分计算损失梯度，适用于连续和离散时间系统，并引入哈密顿循环单元（HRUs）和哈密顿状态空间模型（HSSMs）。

Result: RHEL在多种时间序列任务中表现与BPTT相当，适用于长序列（长度达50k）。

Conclusion: RHEL为设计可扩展、高效的自学习物理系统提供了新途径。
...
### [377] [Tight analyses of first-order methods with error feedback](https://arxiv.org/abs/2506.05271)
*Daniel Berg Thomsen,Adrien Taylor,Aymeric Dieuleveut*

Main category: cs.LG

TL;DR: 论文分析了分布式学习中通信压缩的两种误差反馈方法（EF和EF²¹），通过Lyapunov函数提供了最优收敛率，并进行了公平比较。
...
Result: 提供了两种方法的最优收敛率，并验证了其性能保证。
...
### [378] [How to Unlock Time Series Editing? Diffusion-Driven Approach with Multi-Grained Control](https://arxiv.org/abs/2506.05276)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以同时满足点级和段级的精确控制需求，CocktailEdit旨在填补这一空白。

Method: 结合置信加权锚点控制（点级约束）和基于分类器的控制（段级统计属性管理），在去噪推理阶段实现精确控制。

Result: 实验证明该框架在多种数据集和模型上有效，保持了时间一致性并灵活适应不同约束。
...
### [379] [Fast-DataShapley: Neural Modeling for Training Data Valuation](https://arxiv.org/abs/2506.05281)
*Haifeng Sun,Yu Xiong,Runze Wu,Xinyu Cai,Changjie Fan,Lan Zhang,Xiang-Yang Li*

Main category: cs.LG

TL;DR: 论文提出Fast-DataShapley方法，通过加权最小二乘法和一次性训练，高效计算训练数据的Shapley值，避免重复训练，显著提升性能和效率。


<details>
  <summary>Details</summary>
Motivation: 在AI行业中，训练数据的价值和版权保护至关重要，但目前基于Shapley值的方法计算复杂度高且需重复训练，成本难以承受。

Method: 提出Fast-DataShapley，利用加权最小二乘法训练可复用的解释模型，支持实时推理，并引入三种方法降低训练开销。

Result: 实验表明，性能提升2.5倍以上，训练速度提高两个数量级。

Conclusion: Fast-DataShapley在理论和实验上均优于基线方法，为数据贡献评估提供了高效解决方案。
...
### [380] [Learning Beyond Experience: Generalizing to Unseen State Space with Reservoir Computing](https://arxiv.org/abs/2506.05292)
*Declan A. Norton,Yuanzhao Zhang,Michelle Girvan*

Main category: cs.LG

TL;DR: 论文探讨了储层计算在无显式结构先验的情况下，如何通过多轨迹训练方案实现状态空间的泛化。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于解决传统机器学习方法在训练数据不足或分布不均时难以泛化的问题。

Method: 提出了一种多轨迹训练方案，用于储层计算，支持在多个不连续时间序列上进行训练。

Result: 实验表明，该方法能在未观测到的吸引盆中捕捉系统行为，实现域外泛化。

Conclusion: 储层计算通过多轨迹训练方案，无需显式结构先验即可实现状态空间的泛化。
...
### [381] [A Smooth Sea Never Made a Skilled $\texttt{SAILOR}$: Robust Imitation via Learning to Search](https://arxiv.org/abs/2506.05294)
*Arnav Kumar Jain,Vibhakar Mohta,Subin Kim,Atiksh Bhardwaj,Juntao Ren,Yunhai Feng,Sanjiban Choudhury,Gokul Swamy*

Main category: cs.LG

TL;DR: 论文提出了一种名为SAILOR的学习方法，通过结合世界模型和奖励模型，解决了行为克隆（BC）在模仿学习中无法处理未知状态的问题。


<details>
  <summary>Details</summary>
Motivation: 行为克隆（BC）方法在模仿学习中只能学习专家访问过的状态，无法处理未知状态下的错误恢复。

Method: 提出学习搜索（L2S）方法，结合世界模型和奖励模型，实现高效学习恢复行为。

Result: 在多个视觉操作任务中，SAILOR表现优于基于BC的Diffusion Policies，且能识别细微错误并避免奖励滥用。

Conclusion: SAILOR通过L2S方法显著提升了模仿学习的鲁棒性和性能。
...
### [382] [Sample Complexity and Representation Ability of Test-time Scaling Paradigms](https://arxiv.org/abs/2506.05295)
*Baihe Huang,Shanda Li,Tianhao Wu,Yiming Yang,Ameet Talwalkar,Kannan Ramchandran,Michael I. Jordan,Jiantao Jiao*

Main category: cs.LG

TL;DR: 论文研究了测试时扩展策略（如自一致性、最佳-n和自校正）的样本效率，理论分析了不同策略的样本需求，并展示了自校正方法在多任务中的表达能力。


<details>
  <summary>Details</summary>
Motivation: 尽管测试时扩展策略在实践中取得了成功，但其样本效率的理论理解仍有限。本文旨在填补这一空白。

Method: 通过理论分析比较自一致性和最佳-n策略的样本需求，并研究自校正方法在多任务中的表达能力。

Result: 自一致性需要Θ(1/Δ²)样本，而最佳-n仅需Θ(1/Δ)；自校正方法使Transformer能模拟在线学习，支持多任务。
...
### [383] [Power Law Guided Dynamic Sifting for Efficient Attention](https://arxiv.org/abs/2506.05300)
...
<details>
  <summary>Details</summary>
Motivation: GPU上大型语言模型的高效推理因内存带宽限制（尤其是HBM和SRAM之间的数据传输）而具有挑战性。现有近似注意力方法依赖昂贵的top-$k$操作，在GPU上性能不佳。

Method: 提出SiftAttention，用基于阈值的元素级过滤操作替代top-$k$步骤，动态估计每个生成步骤的阈值，仅加载和使用高于阈值的注意力分数及其对应值向量。

Result: SiftAttention在减少内存带宽使用的同时，比现有近似注意力方法更好地保持了模型质量。

Conclusion: SiftAttention通过动态阈值过滤提供了一种高效且质量保持的近似注意力解决方案。
...
### [384] [Learning normalized image densities via dual score matching](https://arxiv.org/abs/2506.05310)
*Florentin Guth,Zahra Kadkhodaie,Eero P Simoncelli*

Main category: cs.LG

TL;DR: 提出了一种基于扩散生成模型的新框架，用于学习归一化能量模型，通过双重评分匹配目标优化能量网络，并在ImageNet64数据集上取得与现有技术相当的交叉熵值。


<details>
  <summary>Details</summary>
Motivation: 高维数据中学习概率模型具有挑战性，现有方法难以处理维度灾难问题。

Method: 修改评分网络架构以计算能量，同时保持其归纳偏差，并通过双重评分匹配目标优化能量网络。

Result: 在ImageNet64数据集上取得与现有技术相当的交叉熵值，且能量模型表现出强泛化能力。

Conclusion: 该方法不仅验证了能量模型的有效性，还揭示了图像概率和局部邻域维度随内容变化的特性。
...
### [385] [Improving Data Efficiency for LLM Reinforcement Fine-tuning Through Difficulty-targeted Online Data Selection and Rollout Replay](https://arxiv.org/abs/2506.05316)
*Yifan Sun,Jingyan Shen,Yibin Wang,Tianyu Chen,Zhendong Wang,Mingyuan Zhou,Huan Zhang*

Main category: cs.LG

TL;DR: 本文提出两种技术（难度目标在线数据选择和回放重放）以提高LLM强化学习微调的数据效率，显著减少训练时间。


<details>
  <summary>Details</summary>
Motivation: 现有RL微调方法资源消耗大且忽视数据效率问题，需改进以提升效率。

Method: 引入自适应难度概念指导在线数据选择，开发注意力框架估计难度，并采用回放机制减少计算成本。

Result: 实验表明，方法在6种LLM-数据集组合下将微调时间减少25%至65%。
...
### [386] [LSM-2: Learning from Incomplete Wearable Sensor Data](https://arxiv.org/abs/2506.05321)
...
TL;DR: LSM-2 with AIM是一种自监督学习方法，通过自适应和继承掩码处理不完整的可穿戴传感器数据，无需显式插补即可学习鲁棒表示。


<details>
  <summary>Details</summary>
Motivation: 可穿戴传感器数据通常存在大量缺失，而传统自监督学习模型假设数据完整，因此需要一种能直接从不完整数据中学习的方法。

Method: 提出AIM方法，使用可学习掩码令牌建模现有和人为引入的缺失，预训练于40M小时的多模态传感器数据。

Result: LSM-2 with AIM在分类、回归和生成建模任务中表现最佳，且在目标缺失场景下仍保持高性能。

Conclusion: AIM为可穿戴数据应用提供了更可靠的选择，尤其在临床相关模式中表现优异。
...
### [387] [Seeing the Invisible: Machine learning-Based QPI Kernel Extraction via Latent Alignment](https://arxiv.org/abs/2506.05325)
...
Method: 采用两步学习策略：1) 变分自编码器学习核的潜在表示；2) 专用编码器将观测与核对齐。
...
### [388] [Kinetics: Rethinking Test-Time Scaling Laws](https://arxiv.org/abs/2506.05333)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究基于计算最优性，忽略了测试时内存访问瓶颈，导致小模型效率被高估。

Method: 通过分析0.6B到32B参数的模型，提出Kinetics Scaling Law，并基于稀疏注意力设计新范式。
...
### [389] [Exploring Diffusion Transformer Designs via Grafting](https://arxiv.org/abs/2506.05340)
...
TL;DR: 论文提出了一种名为“嫁接”的方法，通过编辑预训练的扩散变换器（DiTs）来探索新架构设计，显著降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统模型架构设计需要昂贵的预训练来评估决策效果，限制了架构研究的广度。受软件开发的启发，研究能否利用预训练模型研究新架构。

Method: 提出“嫁接”方法，通过替换预训练DiTs中的操作符（如注意力机制、卷积）和配置（如MLP扩展率），构建混合设计。

Result: 混合设计在少量计算成本下（<2%预训练计算）实现了高质量（FID: 2.38-2.64 vs. 2.27）。文本到图像模型速度提升1.43倍，性能损失<2%。

Conclusion: 嫁接方法为扩散模型设计提供了高效探索途径，从操作符替换到架构重组均可实现。
...
### [390] [Inference-Time Hyper-Scaling with KV Cache Compression](https://arxiv.org/abs/2506.05345)
*Adrian Łańcucki,Konrad Staniszewski,Piotr Nawrot,Edoardo M. Ponti*

Main category: cs.LG

TL;DR: 论文提出了一种名为动态内存稀疏化（DMS）的方法，通过压缩Transformer LLMs中的键值（KV）缓存，在相同计算预算下生成更多标记，从而提升推理精度。


<details>
  <summary>Details</summary>
Motivation: Transformer LLMs的生成成本受限于KV缓存的大小，而非生成的标记数量。通过压缩KV缓存，可以在不增加计算开销的情况下提升推理精度。

Method: 引入动态内存稀疏化（DMS），一种稀疏化KV缓存的新方法，仅需1K训练步骤即可实现8倍压缩，同时保持比无训练稀疏注意力更高的精度。

Result: 在多个LLM家族中验证了DMS的有效性，例如在Qwen-R1 32B上，AIME 24、GPQA和LiveCodeBench的精度分别平均提升了9.1、7.6和9.6分。

Conclusion: DMS通过延迟标记淘汰和隐式合并表示，显著提升了推理时的超缩放效果，为高效推理提供了新思路。
...
### [391] [Spore in the Wild: Case Study on Spore.fun, a Real-World Experiment of Sovereign Agent Open-ended Evolution on Blockchain with TEEs](https://arxiv.org/abs/2506.04236)
*Botao Amber Hu,Helena Rong*

Main category: cs.MA

TL;DR: 论文探讨了通过开放系统（如区块链和LLM代理）实现开放式演化（OEE）的可能性，并以Spore.fun为例研究了代理行为和演化轨迹。


<details>
  <summary>Details</summary>
Motivation: 传统封闭系统模拟（如Tierra和Avida）难以实现持续的OEE，因此研究开放系统（如区块链和LLM代理）是否能实现这一目标。

Method: 通过Spore.fun实验，研究基于区块链和TEE的自主代理的演化行为，采用数字行为学方法分析其轨迹。

Result: 初步结果表明，开放系统可能为OEE提供新途径，但需进一步验证。
...
### [392] [HASHIRU: Hierarchical Agent System for Hybrid Intelligent Resource Utilization](https://arxiv.org/abs/2506.04255)
*Kunal Pai,Parth Shah,Harshil Patel*

Main category: cs.MA

TL;DR: HASHIRU是一个新型多智能体系统框架，通过动态分层控制、资源感知混合智能和自主功能扩展，提升灵活性、资源效率和适应性。
...
Method: HASHIRU采用分层结构，由动态管理的“CEO”代理和任务驱动的“员工”代理组成，结合本地小模型和外部API，并通过经济模型优化资源分配。

Result: 在学术论文评审、安全评估和复杂推理任务中表现优异，部分任务超越现有模型（如Gemini 2.0 Flash）。

Conclusion: HASHIRU为多智能体系统提供了更强大、高效和自适应的解决方案，支持动态资源管理和自主功能扩展。
...
### [393] [CORA: Coalitional Rational Advantage Decomposition for Multi-Agent Policy Gradients](https://arxiv.org/abs/2506.04265)
...
<details>
  <summary>Details</summary>
Motivation: 共享全局优势在多智能体强化学习中常导致次优策略更新，因为未考虑智能体的独特贡献。现有方法缺乏对联盟层面的详细分析。

Method: 提出CORA方法，通过联盟边际贡献评估优势，并利用合作博弈论的核心解分解优势，确保联盟合理性。采用随机联盟采样降低计算开销。
...
### [394] [CPU-Based Layout Design for Picker-to-Parts Pallet Warehouses](https://arxiv.org/abs/2506.04266)
*Timo Looms,Lin Xie*

Main category: cs.MA

TL;DR: 研究提出了一种基于CPU架构的新型仓库布局设计，显著提高了吞吐效率并降低了劳动力需求。
...
Method: 采用离散事件模拟，比较新型CPU分区布局（P、E、S区）与传统矩形和Flying-V布局。

Result: 新型布局显著缩短吞吐时间并减少劳动力需求。
...
### [395] [Autonomous Collaborative Scheduling of Time-dependent UAVs, Workers and Vehicles for Crowdsensing in Disaster Response](https://arxiv.org/abs/2506.04276)
*Lei Han,Yitong Guo,Pengfei Yang,Zhiyong Yu,Liang Wang,Quan Wang,Zhiwen Yu*

Main category: cs.MA

TL;DR: HoAs-PALN算法通过自适应降维和局部纳什均衡博弈，实现了无人机、工人和车辆的高效协作调度，显著提升了灾后环境信息收集的效率。


<details>
  <summary>Details</summary>
Motivation: 自然灾害造成巨大损失，现有传感技术因环境复杂性和适应性不足，难以高效获取灾后信息。

Method: 提出HoAs-PALN算法，包括自适应降维匹配和局部纳什均衡博弈，优化多智能体协作调度。

Result: 实验表明，HoAs-PALN任务完成率平均提升64.12%，调度决策时间少于10秒。
...
### [396] [From Standalone LLMs to Integrated Intelligence: A Survey of Compound Al Systems](https://arxiv.org/abs/2506.04565)
*Jiayi Chen,Junyi Ye,Guiling Wang*

Main category: cs.MA

TL;DR: 本文综述了复合人工智能系统（CAIS），提出了一种多维分类法，并分析了四种基础范式，旨在为下一代系统级人工智能提供研究基础。


<details>
  <summary>Details</summary>
Motivation: 解决独立模型在记忆、推理、实时基础和多模态理解任务中的局限性，同时填补CAIS领域缺乏统一分析框架的空白。

Method: 定义了CAIS概念，提出了基于组件角色和编排策略的多维分类法，并分析了RAG、LLM代理、MLLM和编排中心架构四种范式。

Result: 总结了代表性系统、设计权衡和评估方法，并指出了可扩展性、互操作性、基准测试和协调等关键挑战。

Conclusion: 为研究人员和从业者提供了理解和推进下一代系统级人工智能的全面基础。
...
### [397] [Towards Language-Augmented Multi-Agent Deep Reinforcement Learning](https://arxiv.org/abs/2506.05236)
*Maxime Toquebiau,Jae-Yun Jun,Faïz Benamar,Nicolas Bredeche*

Main category: cs.MA

TL;DR: 论文提出了一种基于人类定义语言的框架，通过自然语言描述增强多智能体强化学习中的通信与协调能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注自发形成的通信协议，但效率低且难以解释。受自然语言在智能中的启发，研究如何通过人类定义语言提升多智能体的学习与协调能力。

Method: 提出一种框架，智能体不仅学习行动，还生成和解释自然语言描述。语言增强学习既支持显式通信，又指导表征学习。

Result: 实验表明，该方法优于传统自发通信基线，语言基础带来更丰富内部表征、更好的泛化能力和人机交互能力。

Conclusion: 结构化语言整合能有效提升多智能体系统的性能和可解释性，为未来研究开辟新方向。
...
### [398] [Time to Talk: LLM Agents for Asynchronous Group Communication in Mafia Games](https://arxiv.org/abs/2506.05309)
...
<details>
  <summary>Details</summary>
Motivation: 现实场景多为异步通信，而现有LLM主要用于同步通信，因此需要开发适应异步环境的代理。

Method: 开发自适应异步LLM代理，并收集在线Mafia游戏数据集进行评估。

Result: 代理在游戏表现和融入人类玩家方面与人类相当，发言时机行为接近人类，但内容有差异。

Conclusion: 该研究为LLM融入现实人类群体场景（如团队讨论、教育等）奠定了基础。
...
### [399] [CMIE: Combining MLLM Insights with External Evidence for Explainable Out-of-Context Misinformation Detection](https://arxiv.org/abs/2505.23449)
...
Conclusion: CMIE有效提升虚假信息检测的准确性，解决了MLLM的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂3D场景理解和动态空间推理方面表现不足，需要更强大的模型支持机器人交互。

Method: 提出RoboRefer，结合SFT和RFT训练，并引入RefSpatial数据集和RefSpatial-Bench基准。

Result: SFT训练的RoboRefer空间理解成功率89.6%，RFT训练的RoboRefer在基准测试中超越其他基线17.4%。
...
TL;DR: cuVSLAM是一种先进的视觉SLAM解决方案，支持多种视觉-惯性传感器组合，适用于从单一RGB相机到32个相机的配置，并通过CUDA优化实现实时边缘计算。


<details>
  <summary>Details</summary>
Motivation: 精确且鲁棒的姿态估计是自主机器人的关键需求，cuVSLAM旨在提供一种高性能的视觉SLAM解决方案，适应多种传感器配置。

Method: cuVSLAM支持多种视觉-惯性传感器组合，包括RGB和深度相机及IMU，并通过CUDA优化实现实时计算。
...
### [402] [Learning Smooth State-Dependent Traversability from Dense Point Clouds](https://arxiv.org/abs/2506.04362)
*Zihao Dong,Alan Papalia,Leonard Jung,Alenna Spiro,Philip R. Osteen,Christa S. Robison,Michael Everett*

Main category: cs.RO

TL;DR: SPARTA是一种基于点云的方法，通过输出平滑的解析函数来预测不同接近角度下的地形可穿越性，解决了传统方法需要大量数据和计算效率低的问题。


<details>
  <summary>Details</summary>
Motivation: 解决越野自主性中地形可穿越性与车辆状态（如接近角度）相关的问题，避免传统方法对大数据集和高计算成本的需求。

Method: 利用傅里叶基函数构建平滑解析函数，预测任意接近角度下的风险分布，实现高效和可重用的模型推断。

Result: 在高保真仿真中，SPARTA在穿越40米巨石场时的成功率为91%，优于基线的73%，并在硬件测试中展示了泛化能力。

Conclusion: SPARTA通过几何结构化的网络设计，显著提升了地形可穿越性预测的效率和准确性，适用于实际越野场景。
...
### [403] [Online Adaptation of Terrain-Aware Dynamics for Planning in Unstructured Environments](https://arxiv.org/abs/2506.04484)
*William Ward,Sarah Etter,Tyler Ingebrand,Christian Ellis,Adam J. Thorpe,Ufuk Topcu*

Main category: cs.RO

TL;DR: 论文提出了一种基于函数编码器的在线自适应方法，用于动态建模和规划，使机器人在未知地形中快速适应。


<details>
  <summary>Details</summary>
Motivation: 远程非结构化环境中，机器人需适应快速变化的地形，动态估计是实现可靠导航的关键挑战。

Method: 通过神经网络基函数学习多样地形上的机器人动态，利用最小二乘法实现快速在线适应。

Result: 在Unity模拟器中验证，下游控制器因模型精度提升表现更优，碰撞减少。

Conclusion: 该方法无需重新训练即可高效适应新地形，优于基线方法。
...
### [404] [SGN-CIRL: Scene Graph-based Navigation with Curriculum, Imitation, and Reinforcement Learning](https://arxiv.org/abs/2506.04505)
...
<details>
  <summary>Details</summary>
Motivation: 解决部分可观测环境中机器人导航效率低的问题，通过3D场景图建模物体间空间关系。

Method: 结合强化学习、模仿学习和课程学习，利用3D场景图表示环境，逐步增加任务复杂度。

Result: 在Isaac Sim环境中验证，3D场景图显著提高了困难导航场景的成功率。
...
### [405] [Olfactory Inertial Odometry: Sensor Calibration and Drift Compensation](https://arxiv.org/abs/2506.04539)
...
<details>
  <summary>Details</summary>
Motivation: 嗅觉导航任务中气体动力学和环境因素会引入干扰，使OIO难以实现。本文旨在解决这一问题。

Method: 定义了一种适用于多种嗅觉传感器的OIO校准流程，并在真实机器人手臂上进行了验证。

Result: 校准后的OIO在冷启动嗅觉导航任务中表现优于未校准状态。

Conclusion: 该方法为机器人手术和无接触安全筛查等应用提供了可行的OIO校准方案。
...
### [406] [Chronoamperometry with Room-Temperature Ionic Liquids: Sub-Second Inference Techniques](https://arxiv.org/abs/2506.04540)
*Kordel K. France*

Main category: cs.RO

TL;DR: 本文提出了一种新的数学回归方法，显著缩短了室温离子液体中的计时安培测量时间至1秒以下，提高了测量效率。


<details>
  <summary>Details</summary>
Motivation: 在室温离子液体（RTILs）中，高粘度和缓慢的质量传输导致测量时间延长，限制了计时安培法的应用。

Method: 通过应用推理算法对初始瞬态电流响应进行分析，预测稳态电化学参数，无需硬件修改。

Result: 该方法在保持合理准确性的同时，将数据采集时间显著缩短至1秒以下，优于传统方法（1-4秒或更长）。

Conclusion: 该技术在分析化学、传感器技术和电池科学等领域具有广泛应用前景，尤其适用于快速电化学定量分析。
...
### [407] [Multimodal Limbless Crawling Soft Robot with a Kirigami Skin](https://arxiv.org/abs/2506.04547)
*Jonathan Tirado,Aida Parvaresh,Burcu Seyidoğlu,Darryl A. Bedford,Jonas Jørgensen,Ahmad Rafsanjani*

Main category: cs.RO

TL;DR: 该论文提出了一种受无肢生物启发的软体机器人，通过结合直线运动和不对称步态，能够在复杂地形中导航。


<details>
  <summary>Details</summary>
Motivation: 受无肢生物在平坦表面上爬行的启发，设计一种高效的无肢机器人，适用于复杂环境。

Method: 使用一对对抗性充气软体执行器和具有不对称摩擦特性的柔性剪纸皮肤，通过精确相位偏移的循环充气实现直线运动，利用不对称步态实现转向。

Result: 机器人在粗糙基质和多障碍物的环境中表现出良好的机动性，并通过实时反馈和自适应控制避免碰撞。

Conclusion: 这项研究展示了仿生软体机器人在受限或非结构化环境中的潜在应用价值。
...
### [408] [A Novel Transformer-Based Method for Full Lower-Limb Joint Angles and Moments Prediction in Gait Using sEMG and IMU data](https://arxiv.org/abs/2506.04577)
*Farshad Haghgoo Daryakenari,Tara Farizeh*

Main category: cs.RO

TL;DR: 该研究提出了一种基于Transformer的深度学习框架，用于通过表面肌电信号（sEMG）和惯性测量单元（IMU）信号预测下肢关节角度和力矩。模型在短长期预测中均表现优异，准确性高。


<details>
  <summary>Details</summary>
Motivation: 旨在开发一种适用于实验室外使用的实时预测模型，结合sEMG和IMU信号，以捕捉下肢运动的生物力学信息。

Method: 设计了两个独立的Transformer神经网络（TNN），分别用于运动学预测和动力学预测，使用可穿戴传感器数据。

Result: 模型在所有关节上的预测准确性高（Spearman相关系数>0.96，R平方>0.92），在关节角度预测中显著优于基准方法。

Conclusion: 研究表明Transformer模型在实时生物力学预测中具有潜力，未来可通过输入最小化和模态特定权重策略进一步提升效率与准确性。
...
### [409] [Real-Time LPV-Based Non-Linear Model Predictive Control for Robust Trajectory Tracking in Autonomous Vehicles](https://arxiv.org/abs/2506.04684)
*Nitish Kumar,Rajalakshmi Pachamuthu*

Main category: cs.RO

TL;DR: 论文提出了一种用于自动驾驶车辆轨迹跟踪的模型预测控制（MPC）框架，通过模块化架构整合状态估计、车辆动力学建模和优化，实现实时性能。


<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶车辆在多样化驾驶条件下的轨迹跟踪问题，确保实时性和鲁棒性。

Method: 采用线性参数变化（LPV）形式的状态空间方程，引入基于曲率的权重矩阵优化方法，并通过ROS实现并行执行。

Result: 仿真和实时实验表明，系统在高速度和激进操作下仍能保持高精度和低误差。

Conclusion: 该框架为动态权重调整和协同自动驾驶系统集成奠定了基础，提升了自动驾驶的安全性和效率。
...
### [410] [Enhancing Efficiency and Propulsion in Bio-mimetic Robotic Fish through End-to-End Deep Reinforcement Learning](https://arxiv.org/abs/2506.04627)
*Xinyu Cui,Boai Sun,Yi Zhu,Ning Yang,Haifeng Zhang,Weicheng Cui,Dixia Fan,Jun Wang*

Main category: cs.RO

TL;DR: 通过深度强化学习优化仿生机器鱼运动，提高推进效率并降低能耗。


<details>
  <summary>Details</summary>
Motivation: 现有研究常忽视控制策略在提升水下机器人效率中的关键作用。

Method: 采用深度强化学习（DRL），结合扩展压力感知、Transformer模型和策略迁移方案。

Result: 实验显示DRL训练的策略显著提升效率和推进力，并能灵活应对流体环境。
...
### [411] [ActivePusher: Active Learning and Planning with Residual Physics for Nonprehensile Manipulation](https://arxiv.org/abs/2506.04646)
*Zhuoyun Zhong,Seyedali Golestaneh,Constantinos Chamzas*

Main category: cs.RO

TL;DR: ActivePusher结合残差物理建模和基于核的不确定性驱动主动学习，提高数据效率和规划成功率。


<details>
  <summary>Details</summary>
Motivation: 在非抓取操作（如推动或滚动）中，精确的解析模型难以获取，而随机采样的训练数据成本高且效率低。

Method: 提出ActivePusher框架，结合残差物理建模和基于核的不确定性驱动主动学习，优化数据采集和规划。

Result: 在仿真和真实环境中验证，ActivePusher相比基线方法提高了数据效率和规划成功率。

Conclusion: ActivePusher为长时程非抓取操作提供了一种高效的数据驱动规划方法。
...
### [412] [Application of SDRE to Achieve Gait Control in a Bipedal Robot for Knee-Type Exoskeleton Testing](https://arxiv.org/abs/2506.04680)
*Ping-Kong Huang,Chien-Wu Lan,Chin-Tien Wu*

Main category: cs.RO

TL;DR: 研究提出了一种基于双足机器人平台的参数化控制方法，用于安全、可重复地评估外骨骼性能，通过SDRE优化控制和分段线性速度模型实现精确步态复制。
...
Method: 采用双足机器人模拟人类步态，基于SDRE的扭矩控制和参数化方法优化运动轨迹，并结合分段线性速度模型适应电机动态。

Result: 实验验证了参数化控制方法的可行性，双足机器人平台为外骨骼测试提供了可靠且可重复的机制。

Conclusion: 该方法有效降低了实时计算负担，为外骨骼性能评估提供了新思路。
...
### [413] [Efficient Path Planning and Task Allocation Algorithm for Boolean Specifications](https://arxiv.org/abs/2506.04881)
*Ioana Hustiu,Roozbeh Abolpour,Cristian Mahulea,Marius Kloetzer*

Main category: cs.RO

TL;DR: 提出了一种基于整数线性规划（ILP）和Petri网的多机器人路径规划与任务分配算法，通过松弛为线性规划（LP）提高计算效率，适用于大规模系统（如500机器人团队）。


<details>
  <summary>Details</summary>
Motivation: 解决多机器人系统在满足全局布尔规范时的路径规划和任务分配问题，同时确保计算可扩展性和效率。

Method: 结合ILP和Petri网的结构化方法，通过证明约束矩阵的完全单模性（TU），将ILP松弛为LP，减少计算复杂度。

Result: 算法能高效处理大规模系统（500机器人），生成无碰撞路径，适用于仓库物流和搜救等实际场景。

Conclusion: 该方法在理论和实践上为多机器人路径规划和任务分配提供了高效、可扩展的框架。
...
### [414] [Tire Wear Aware Trajectory Tracking Control for Multi-axle Swerve-drive Autonomous Mobile Robots](https://arxiv.org/abs/2506.04752)
*Tianxin Hu,Xinhang Xu,Thien-Minh Nguyen,Fen Liu,Shenghai Yuan,Lihua Xie*

Main category: cs.RO

TL;DR: 提出了一种新型模型预测控制（MPC）方法，用于多轴转向驱动自主移动机器人（MS-AGV）的轨迹跟踪，同时考虑轮胎磨损最小化。


<details>
  <summary>Details</summary>
Motivation: 高负载运输中，MS-AGV的轮胎磨损问题显著，需优化控制以减少磨损。

Method: 采用分层控制器设计，结合魔术公式轮胎模型和简化轮胎磨损模型，加速问题求解。

Result: 实验表明，轮胎磨损减少19.19%，在更复杂场景下减少65.20%，同时保持轨迹跟踪精度。

Conclusion: 该方法在减少轮胎磨损的同时，实现了高效的实时控制。
...
### [415] [MineInsight: A Multi-sensor Dataset for Humanitarian Demining Robotics in Off-Road Environments](https://arxiv.org/abs/2506.04842)
...
Method: 创建多传感器、多光谱数据集，包含35个目标，集成双视角传感器扫描和多种光谱范围图像。

Result: 数据集包含约38,000 RGB帧、53,000 VIS-SWIR帧和108,000 LWIR帧，提供目标位置估计。

Conclusion: MineInsight为排雷算法开发与评估提供了基准，数据集已公开。
...
### [416] [PulseRide: A Robotic Wheelchair for Personalized Exertion Control with Human-in-the-Loop Reinforcement Learning](https://arxiv.org/abs/2506.05056)
*Azizul Zahid,Bibek Poudel,Danny Scott,Jason Scott,Scott Crouter,Weizi Li,Sai Swaminathan*

Main category: cs.RO

TL;DR: PulseRide是一种新型轮椅系统，通过实时生理数据和强化学习提供个性化辅助，帮助用户保持适度活动水平。


<details>
  <summary>Details</summary>
Motivation: 轮椅用户面临活动不足或重复运动导致的健康问题，需要一种既能促进活动又能减少受伤风险的移动系统。

Method: PulseRide整合实时生理数据（如心率和ECG）与轮椅速度，采用基于DQN的强化学习算法，动态调整辅助力度。

Result: 初步测试显示，PulseRide能使用户71.7%的时间保持适度心率，并减少41.86%的肌肉收缩，延缓疲劳。

Conclusion: PulseRide提供了一种健康、自适应的移动解决方案，填补了被动与过度费力移动方式之间的空白。
...
### [417] [ArtVIP: Articulated Digital Assets of Visual Realism, Modular Interaction, and Physical Fidelity for Robot Learning](https://arxiv.org/abs/2506.04941)
...
TL;DR: ArtVIP是一个高质量的开源数据集，用于机器人学习，提供视觉逼真和物理保真度高的数字孪生铰接物体和室内场景资产。


<details>
  <summary>Details</summary>
Motivation: 现有开源铰接物体数据集在视觉逼真和物理保真度上不足，限制了其在真实世界机器人任务中的应用。

Method: ArtVIP由专业3D建模师制作，采用精确几何网格、高分辨率纹理和调优动态参数，并嵌入模块化交互行为和像素级功能标注。

Result: 通过特征图可视化和光学运动捕捉验证了ArtVIP的视觉和物理保真度，并在模仿学习和强化学习实验中验证了其适用性。

Conclusion: ArtVIP作为完全开源的数据集，推动了机器人学习研究，并为研究社区提供了高质量资源。
...
### [418] [A Pillbug-Inspired Morphing Mechanism Covered with Sliding Shells](https://arxiv.org/abs/2506.04942)
*Jieyu Wang,Yingzhong Tian,Fengfeng Xi,Damien Chablat,Jianing Lin,Gaoke Ren,Yinjun Zhao*

Main category: cs.RO

TL;DR: 提出一种受潮虫运动启发的变形结构，利用滑块曲柄机构实现卷曲与展开动作，并通过剪刀机构减少自由度，最终开发出具有两种运动模式的机器人。


<details>
  <summary>Details</summary>
Motivation: 受潮虫卷曲与展开运动的启发，设计一种新型变形结构，以实现类似的多功能运动能力。

Method: 采用滑块曲柄机构模拟潮虫的三种曲线变形，并添加剪刀机构将自由度降至1。通过类型和尺寸综合，开发出包含外壳和变形机构的完整系统。

Result: 成功开发出3D模型，并测试了其变形能力。最终设计出具有卷曲滚动和展开直线移动两种模式的机器人。

Conclusion: 该机制有效模拟了潮虫的运动特性，为多功能变形结构的设计提供了新思路。
...
### [419] [GEX: Democratizing Dexterity with Fully-Actuated Dexterous Hand and Exoskeleton Glove](https://arxiv.org/abs/2506.04982)
*Yunlong Dong,Xing Liu,Jun Wan,Zelin Deng*

Main category: cs.RO

TL;DR: GEX系统结合GX11三指仿人机械手和EX12三指外骨骼手套，通过运动学重定向实现高保真闭环遥操作，低成本且全驱动。


<details>
  <summary>Details</summary>
Motivation: 解决灵巧操作研究中成本与性能的差距，为AI和机器人技能学习提供高质量演示数据平台。

Method: 采用模块化3D打印设计，独立关节电机驱动23自由度，实现精确运动学建模和双向计算。

Result: 系统显著提升运动学重定向保真度，低成本且功能完整。

Conclusion: GEX为灵巧操作研究和技能学习提供了经济高效的解决方案。
...
### [420] [A Unified Framework for Simulating Strongly-Coupled Fluid-Robot Multiphysics](https://arxiv.org/abs/2506.05012)
*Jeong Hun Lee,Junzhe Hu,Sofia Kwok,Carmel Majidi,Zachary Manchester*

Main category: cs.RO

TL;DR: 提出了一种统一优化框架，用于模拟流体-机器人多物理场耦合问题，结合离散变分力学实现稳定隐式时间积分，并通过改进的浸没边界方法处理边界条件。


<details>
  <summary>Details</summary>
Motivation: 解决流体与机器人动力学耦合模拟中的数值不稳定和物理准确性挑战，为机器人任务提供高保真仿真工具。

Method: 基于最小作用量原理，统一推导流体和机器人动力学方程，采用离散变分力学设计隐式时间积分方案，改进浸没边界方法处理边界条件。

Result: 在基准流体动力学问题（如泊肃叶流和自由流中的圆盘）中验证了物理准确性，并通过仿真到现实的游泳机器人实验展示了框架的实用性。

Conclusion: 该框架为流体-机器人耦合系统提供了高精度仿真方法，支持机器人任务的仿真到现实应用。
...
### [421] [Hierarchical Language Models for Semantic Navigation and Manipulation in an Aerial-Ground Robotic System](https://arxiv.org/abs/2506.05020)
*Haokun Liu,Zhaoqi Ma,Yunong Li,Junichiro Sugihara,Yicheng Chen,Jinjie Li,Moju Zhao*

Main category: cs.RO

TL;DR: 提出了一种结合LLM和VLM的层次框架，用于异构多机器人系统的任务分解与动态环境适应。
...
Conclusion: 首次展示了结合VLM感知与LLM任务推理的空中-地面异构系统。
...
### [422] [DemoSpeedup: Accelerating Visuomotor Policies via Entropy-Guided Demonstration Acceleration](https://arxiv.org/abs/2506.05064)
*Lingxiao Guo,Zhengrong Xue,Zijing Xu,Huazhe Xu*

Main category: cs.RO

TL;DR: DemoSpeedup是一种自监督方法，通过熵引导的演示加速来提升视觉运动策略的执行速度，最高可提速3倍，同时保持任务完成性能。


<details>
  <summary>Details</summary>
Motivation: 模仿学习中人类操作者收集的演示通常较慢，导致策略执行速度不理想。

Method: 利用熵估计分段加速演示，低熵帧保持原速，高熵帧加速。

Result: 策略执行速度提升3倍，任务完成性能不变，甚至成功率更高。

Conclusion: DemoSpeedup有效加速策略执行，同时可能提升任务成功率。
...
### [423] [Synthetic Dataset Generation for Autonomous Mobile Robots Using 3D Gaussian Splatting for Vision Training](https://arxiv.org/abs/2506.05092)
*Aneesh Deogan,Wout Beks,Peter Teurlings,Koen de Vos,Mark van den Brand,Rene van de Molengraft*

Main category: cs.RO

TL;DR: 提出了一种基于Unreal Engine的自动生成合成数据方法，用于解决手动标注数据集的耗时、易错和多样性不足问题，尤其在机器人足球领域表现优异。


<details>
  <summary>Details</summary>
Motivation: 手动创建标注数据集耗时、易错且多样性有限，机器人领域的动态场景进一步增加了数据集创建的难度。

Method: 利用Unreal Engine和3D高斯样条快速生成逼真合成数据，结合真实数据提升检测性能。

Result: 合成数据训练的目标检测算法在机器人足球场景中表现与真实数据相当，且结合两者可显著提升性能。
...
### [424] [EDEN: Efficient Dual-Layer Exploration Planning for Fast UAV Autonomous Exploration in Large 3-D Environments](https://arxiv.org/abs/2506.05106)
*Qianli Dong,Xuebo Zhang,Shiyong Zhang,Ziyu Wang,Zhe Ma,Haobo Xi*

Main category: cs.RO

TL;DR: 提出了一种快速且计算高效的双层探索规划方法，通过近似算法实现大规模环境中的实时规划，并通过低曲率惩罚成本选择目标点，提升探索效率。


<details>
  <summary>Details</summary>
Motivation: 解决大规模环境中自主探索的高计算成本和低速机动问题。

Method: 采用双层探索规划方法，包括近似算法的长期区域路由和贪婪探索目标点选择。

Result: 在仿真和实际环境中验证，该方法在探索效率、计算成本和轨迹速度上优于现有方法。

Conclusion: 该方法有效提升了大规模环境中的自主探索效率，代码将开源。
...
### [425] [Whole-Body Constrained Learning for Legged Locomotion via Hierarchical Optimization](https://arxiv.org/abs/2506.05115)
*Haoyu Wang,Ruyi Zhou,Liang Ding,Tie Liu,Zhelin Zhang,Peng Xu,Haibo Gao,Zongquan Deng*

Main category: cs.RO

TL;DR: 论文提出了一种基于分层优化的全身跟随器，将硬约束和软约束集成到强化学习框架中，以提高机器人的安全性。


<details>
  <summary>Details</summary>
Motivation: 由于模拟与现实的差距和缺乏可解释性，无约束的强化学习策略在现实世界中存在安全问题，限制了其在严格安全要求任务中的应用。

Method: 设计了一种分层优化框架，结合模型控制优势，定义硬约束和软约束，以优化策略并减轻模拟到现实的转移挑战。

Result: 在六足机器人上部署了带有约束的策略，并在多种户外环境中测试，展示了出色的通过性和安全性。

Conclusion: 该方法在复杂非结构化环境中保持了强化学习的鲁棒性，同时提高了安全性。
...
### [426] [Realizing Text-Driven Motion Generation on NAO Robot: A Reinforcement Learning-Optimized Control Pipeline](https://arxiv.org/abs/2506.05117)
*Zihan Xu,Mengxian Hu,Kaiyan Xiao,Qin Fang,Chengju Liu,Qijun Chen*

Main category: cs.RO

TL;DR: 提出了一种基于文本驱动的方法，将人类运动映射到人形机器人，并通过NPR Loss和强化学习策略实现稳定运动跟踪。


<details>
  <summary>Details</summary>
Motivation: 人形机器人模仿人类运动具有挑战性，但潜力巨大，传统方法依赖姿态估计或动作捕捉系统。

Method: 采用基于NPR Loss的角度信号网络生成关节角度，结合强化学习策略控制全身关节运动。

Result: 实验成功将文本驱动的人类运动转移到真实人形机器人NAO上。
...
### [427] [LiPo: A Lightweight Post-optimization Framework for Smoothing Action Chunks Generated by Learned Policies](https://arxiv.org/abs/2506.05165)
*Dongwoo Son,Suhan Park*

Main category: cs.RO

TL;DR: 提出了一种轻量级后优化框架，用于平滑离散动作分块序列，提升机器人动态任务的运动质量。


<details>
  <summary>Details</summary>
Motivation: 离散动作分块在边界处引入不连续性，影响运动质量，尤其在需要平滑轨迹的动态任务中。

Method: 结合推理感知的分块调度、线性混合和抖动最小化轨迹优化，以减少振动和运动抖动。

Result: 实验验证了该方法能显著减少振动和抖动，提升运动平滑性和机械鲁棒性。

Conclusion: 该方法有效解决了离散动作分块的不连续性问题，适用于动态任务。
...
### [428] [Fabrica: Dual-Arm Assembly of General Multi-Part Objects via Integrated Planning and Learning](https://arxiv.org/abs/2506.05168)
...
TL;DR: Fabrica是一个双臂机器人系统，能够实现多部件物体的端到端自主装配，结合了高效全局规划和鲁棒局部控制。


<details>
  <summary>Details</summary>
Motivation: 解决机器人在复杂几何形状下执行长时程、接触丰富的多部件装配任务的挑战。

Method: 开发了层次化的规划框架（包括优先级、序列、抓取和运动规划）和轻量级强化学习框架，用于接触丰富的装配步骤。

Result: 系统在真实世界中实现了80%的成功率，无需领域知识或人工演示。

Conclusion: Fabrica是首个实现完全通用化多部件装配的系统，展示了高效规划与鲁棒控制的结合潜力。
...
### [429] [Domain Adaptation Method and Modality Gap Impact in Audio-Text Models for Prototypical Sound Classification](https://arxiv.org/abs/2506.04376)
*Emiliano Acevedo,Martín Rocamora,Magdalena Fuentes*

Main category: cs.SD

TL;DR: 论文提出了一种新方法，通过量化背景声源的影响并整合到分类过程中，提升了音频-文本模型在背景噪声下的性能，同时分析了音频与文本嵌入之间的模态差距。


<details>
  <summary>Details</summary>
Motivation: 现有音频-文本模型在背景噪声下性能显著下降，作者希望通过改进模型性能以应对这一问题。

Method: 提出了一种量化背景声源贡献的方法，并整合到分类过程中，同时分析了音频与文本嵌入的模态差距。

Result: 方法在不需重新训练模型的情况下提升了性能，并在不同背景和信噪比条件下表现出色。

Conclusion: 该方法具有可扩展性和鲁棒性，适用于多样化的环境。
...
### [430] [Benchmarking Time-localized Explanations for Audio Classification Models](https://arxiv.org/abs/2506.04391)
*Cecilia Bolaños,Leonardo Pepino,Martin Meza,Luciana Ferrer*

Main category: cs.SD

TL;DR: 提出了一种用于音频分类模型的时间局部化解释的基准，利用目标事件的时间注释作为真实解释的代理，并展示了其在优化和比较模型无关后解释方法中的实用性。


<details>
  <summary>Details</summary>
Motivation: 现代音频处理方法通常缺乏解释性，导致难以评估解释质量，因此需要一种基准来优化和比较解释方法。

Method: 使用时间注释作为代理真实解释，构建基准，并系统优化和比较多种模型无关的后解释方法。

Result: 在某些情况下获得了接近完美的解释，并揭示了虚假相关性。

Conclusion: 该基准为音频分类模型的解释提供了实用工具，有助于提高模型的可信度和洞察力。

Abstract: Most modern approaches for audio processing are opaque, in the sense that
...
explanations for uncovering spurious correlations.

</details>


### [431] [Grapheme-Coherent Phonemic and Prosodic Annotation of Speech by Implicit and Explicit Grapheme Conditioning](https://arxiv.org/abs/2506.04527)
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在音素和韵律标签生成中与字形一致性不足的问题，为文本到语音和口音估计等任务提供更可靠的数据。

Method: 1）通过预训练BERT特征的提示编码器实现隐式字形条件；2）在推理阶段显式剪枝与字形不一致的标签假设。

Result: 显著提升了字形与预测标签的一致性，并在口音估计任务中提高了准确性。

Conclusion: 所提方法有效解决了标签与字形的一致性问题，为下游任务提供了高质量的并行数据。
...
### [432] [LLM-based phoneme-to-grapheme for phoneme-based speech recognition](https://arxiv.org/abs/2506.04711)
*Te Ma,Min Bi,Saierdaer Yusuyin,Hao Huang,Zhijian Ou*

Main category: cs.SD

TL;DR: 论文提出了一种基于大型语言模型（LLM）的音素到字素解码方法（LLM-P2G），用于音素基础的自动语音识别（ASR），通过数据增强和随机化训练策略，显著优于传统WFST方法。


<details>
  <summary>Details</summary>
Motivation: 传统WFST解码方法在音素基础ASR中存在流程复杂且无法利用大型语言模型的局限性，因此需要一种更高效的方法。

Method: 提出LLM-P2G解码框架，包含语音到音素（S2P）和音素到字素（P2G）两个阶段，并引入数据增强（DANP）和随机化训练（TKM）策略。
...
### [433] [Improving AI-generated music with user-guided training](https://arxiv.org/abs/2506.04852)
*Vishwa Mohan Singh,Sai Anirudh Aryasomayajula,Ahan Chatterjee,Beste Aydemir,Rifat Mehreen Amin*

Main category: cs.SD

TL;DR: 论文提出了一种结合人类计算的方法，通过用户反馈优化AI音乐生成模型，逐步提升其性能。
...
Method: 采用人类计算方法，聚合用户评分作为损失函数，结合遗传算法逐步优化模型。

Result: 初步测试显示，第一轮迭代评分平均提升0.2，第二轮进一步提升0.39。

Conclusion: 该方法有效提升了AI音乐生成的个性化性能，证明了用户反馈在模型优化中的价值。
...
### [434] [Survey on the Evaluation of Generative Models in Music](https://arxiv.org/abs/2506.05104)
*Alexander Lerch,Claire Arthur,Nick Bryan-Kinns,Corey Ford,Qianyi Sun,Ashvala Vinay*

Main category: cs.SD

TL;DR: 本文综述了音乐生成系统的评估方法，包括主观与客观、定性与定量、实证与计算等多种方法，并从音乐学、工程学和HCI视角探讨其优缺点。
...
Method: 通过跨学科视角，综述了常见的评估目标、方法和指标，涵盖多种评估途径。

Result: 总结了不同评估方法的优势和挑战，为未来研究提供了参考。

Conclusion: 音乐生成系统的评估需要多学科协作，结合多种方法以实现更全面的评价。
...
### [435] [Through the Stealth Lens: Rethinking Attacks and Defenses in RAG](https://arxiv.org/abs/2506.04390)
*Sarthak Choudhary,Nils Palumbo,Ashish Hooda,Krishnamurthy Dj Dvijotham,Somesh Jha*

Main category: cs.CR

TL;DR: RAG系统易受低污染率的毒化段落攻击，现有攻击缺乏隐蔽性。论文提出基于注意力模式的防御方法，但适应性攻击仍能突破。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统的攻击容易被检测，缺乏隐蔽性，需研究更隐蔽的攻击和防御方法。

Method: 提出基于注意力模式的Normalized Passage Attention Score和Attention-Variance Filter算法，用于检测和过滤毒化段落。
...
### [436] [Learning to Diagnose Privately: DP-Powered LLMs for Radiology Report Classification](https://arxiv.org/abs/2506.04450)
...
Method: 使用50,232份放射学报告，通过差分隐私低秩适应（DP-LoRA）微调LLM，评估了BERT-medium、BERT-small和ALBERT-base三种模型在不同隐私预算下的性能。

Result: 在中等隐私保证下，DP微调模型在MIMIC-CXR和CT-RATE数据集上的加权F1分数分别为0.88和0.59，接近非隐私LoRA基线的0.90和0.78。
...
### [437] [BESA: Boosting Encoder Stealing Attack with Perturbation Recovery](https://arxiv.org/abs/2506.04556)
*Xuhao Ren,Haotian Liang,Yajie Wang,Chuan Zhang,Zehui Xiong,Liehuang Zhu*

Main category: cs.CR

TL;DR: 提出了一种名为BESA的增强编码器窃取攻击方法，通过扰动检测和恢复模块提升攻击性能，对抗基于扰动的防御。
...
Method: BESA包含扰动检测和恢复模块，前者推断防御机制，后者通过生成模型恢复干净特征向量。

Result: 实验表明，BESA在面对先进防御时，可将替代编码器准确率提升高达24.63%。

Conclusion: BESA有效增强了编码器窃取攻击的性能，对抗多种防御机制。
...
### [438] [On Automating Security Policies with Contemporary LLMs](https://arxiv.org/abs/2506.04838)
*Pablo Fernández Saura,K. R. Jayaram,Vatche Isahagian,Jorge Bernal Bernabé,Antonio Skarmeta*

Main category: cs.CR

TL;DR: 本文提出了一种利用大语言模型（LLM）结合上下文学习和检索增强生成（RAG）的框架，用于自动化攻击缓解策略合规性，显著提升了性能指标。
...
Method: 通过收集和管理工具及API规范，存储在向量数据库中，构建架构管道将高级缓解策略分解为离散任务，并转化为可操作的API调用。

Result: 使用公开的CTI策略和Windows API文档进行实证评估，RAG方法在精确率、召回率和F1分数上显著优于非RAG基线。
...
### [439] [Why LLM Safety Guardrails Collapse After Fine-tuning: A Similarity Analysis Between Alignment and Fine-tuning Datasets](https://arxiv.org/abs/2506.05346)
*Lei Hsiung,Tianyu Pang,Yung-Chen Tang,Linyue Song,Tsung-Yi Ho,Pin-Yu Chen,Yaoqing Yang*

Main category: cs.CR

TL;DR: 论文探讨了上游安全对齐数据与下游微调任务之间的表示相似性对安全护栏的影响，发现高相似性会削弱安全性，而低相似性则能显著提升模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有缓解策略主要关注事后处理或微调过程中的安全强化，而忽视了上游安全对齐数据的关键作用。
...
### [440] [EMBER2024 -- A Benchmark Dataset for Holistic Evaluation of Malware Classifiers](https://arxiv.org/abs/2506.05074)
...
<details>
  <summary>Details</summary>
Motivation: 现有公共数据集范围狭窄，无法全面评估恶意软件分类器，且缺乏对规避性恶意文件的覆盖。

Method: 通过合作创建EMBER2024数据集，包含320万文件的哈希、元数据、特征向量和标签，支持七种分类任务，并引入挑战集和EMBER特征版本3。

Result: EMBER2024数据集支持多种分类任务，并首次包含挑战集，提升了对规避性恶意软件的评估能力。

Conclusion: EMBER2024的发布旨在促进可重复性，并为恶意软件研究提供新工具。
...
### [441] [Membership Inference Attacks on Sequence Models](https://arxiv.org/abs/2506.05126)
*Lorenzo Rossi,Michael Aerni,Jie Zhang,Florian Tramèr*

Main category: cs.CR

TL;DR: 论文提出了一种改进序列模型隐私泄露审计的方法，通过利用序列生成的固有相关性，改进了现有的成员推理攻击方法。


<details>
  <summary>Details</summary>
Motivation: 现有工具在审计序列模型（如LLM和自回归图像生成器）的隐私泄露风险时存在不足，主要由于假设不匹配。
...
### [442] [An Expansion-Based Approach for Quantified Integer Programming](https://arxiv.org/abs/2506.04452)
*Michael Hartisch,Leroy Chew*

Main category: cs.DM

TL;DR: 论文提出了一种基于扩展的QIP求解方法，利用CEGAR技术，性能优于现有搜索方法。


<details>
  <summary>Details</summary>
Motivation: QIP结合了QBF和整数规划的优势，但现有方法仅探索了搜索范式，扩展范式未被研究。

Method: 采用CEGAR技术，将QBF的扩展方法应用于QIP，并嵌入优化框架。

Result: 实验显示该方法在特定实例中优于现有搜索求解器，且在QBF扩展求解器上也有性能提升。

Conclusion: 扩展方法为QIP提供了新的求解范式，尤其在多阶段鲁棒优化中表现优异。
...
### [443] [BridgeNet: A Hybrid, Physics-Informed Machine Learning Framework for Solving High-Dimensional Fokker-Planck Equations](https://arxiv.org/abs/2506.04354)
...
<details>
  <summary>Details</summary>
Motivation: 传统PINN在处理复杂空间层次和边界条件时表现不佳，需改进。

Method: 采用自适应CNN层提取局部特征，动态加权损失函数强化物理约束。

Result: 在多种测试中，BridgeNet误差更低、收敛更快，且在高维环境下稳定。

Conclusion: BridgeNet为计算物理提供可扩展且精确的解决方案，应用前景广阔。
...
### [444] [Benchmark for Antibody Binding Affinity Maturation and Design](https://arxiv.org/abs/2506.04235)
...
TL;DR: AbBiBench是一个抗体结合亲和力成熟与设计的基准框架，通过评估抗体-抗原复合物的功能单元，系统比较了14种蛋白质模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有抗体评估方法仅关注抗体本身或其与天然抗体的相似性，缺乏对功能单元（抗体-抗原复合物）的评估。

Method: AbBiBench通过测量蛋白质模型在抗体-抗原复合物上的似然性来评估抗体设计潜力，并使用9个数据集和155,853个突变抗体进行系统比较。
...
### [445] [The Latent Space Hypothesis: Toward Universal Medical Representation Learning](https://arxiv.org/abs/2506.04515)
*Salil Patel*

Main category: q-bio.QM

TL;DR: 论文提出了一种基于潜在空间假设的框架，将多模态医学数据映射到统一的高维流形中，以支持个性化医疗。


<details>
  <summary>Details</summary>
Motivation: 医学数据多模态且异构，但可能编码了相同的生理状态信息。潜在空间假设旨在将这些数据统一表示，以更好地理解疾病和个体健康状态。

Method: 通过潜在空间假设，将多模态医学数据投影到一个统一的、层次化组织的流形中，利用几何表示（如点、轨迹和向量）建模健康状态、疾病进展和治疗干预。

Result: 该框架揭示了疾病的子轨迹和患者特异性变化方向，为个性化诊断、监测和治疗提供了量化依据。

Conclusion: 潜在空间假设为临床实践提供了新视角，但仍需解决数据偏差、稀缺性、隐私和因果推断等挑战。
...
### [446] [Observations on robust diffusive stability and common Lyapunov functions](https://arxiv.org/abs/2506.04863)
*Blake McGrane-Corrigan,Rafael de Andrade Moral,Oliver Mason*

Main category: math.DS

TL;DR: 论文证明了对于一对Schur稳定的非负矩阵，存在共同对角Lyapunov函数是鲁棒扩散稳定性（RDS）的充分条件，并指出其与基于线性共正Lyapunov函数的结果不同。同时，论文还针对种群动力学中的扩展Leslie矩阵提出了两个RDS相关结果。


<details>
  <summary>Details</summary>
Motivation: 研究非负矩阵对的鲁棒扩散稳定性（RDS），特别是探讨共同对角Lyapunov函数在RDS中的作用，并与现有基于线性共正Lyapunov函数的结果进行对比。

Method: 通过分析Schur稳定的非负矩阵对，证明共同对角Lyapunov函数的存在性是RDS的充分条件，并针对扩展Leslie矩阵提出相关结果。

Result: 共同对角Lyapunov函数的存在确保了RDS，且这一条件与线性共正Lyapunov函数的结果不同。此外，论文还针对扩展Leslie矩阵给出了两个RDS相关结论。

Conclusion: 共同对角Lyapunov函数是RDS的有效工具，其适用性与现有方法不同，且在种群动力学中的应用展示了其潜力。

Abstract: We consider the problem of robust diffusive stability (RDS) for a pair of
...
### [447] [Computationally Intensive Research: Advancing a Role for Secondary Analysis of Qualitative Data](https://arxiv.org/abs/2506.04230)
*Kaveh Mohajeri,Amir Karami*

Main category: cs.DB

TL;DR: 本文探讨了计算技术在重新利用过去定性研究数据中的潜力，提出了计算密集型二次分析的方法，并讨论了其优势与挑战。


<details>
  <summary>Details</summary>
Motivation: 定性研究数据通常丰富但未被充分利用，计算技术可以挖掘其潜在价值，支持跨上下文和纵向研究。

Method: 提出了一种计算密集型二次分析的方案，并探讨如何通过这种方法创新研究设计。

Result: 强调了计算密集型二次分析在跨上下文和纵向研究中的潜力，并列举了相关挑战。

Conclusion: 计算密集型二次分析为定性数据的再利用提供了新机会，但需解决数据共享和重用中的关键问题。
...
### [448] [kTULA: A Langevin sampling algorithm with improved KL bounds under super-linear log-gradients](https://arxiv.org/abs/2506.04878)
*Iosif Lytras,Sotirios Sabanis,Ying Zhang*

Main category: math.ST

TL;DR: 论文提出了一种名为kTULA的新算法，用于解决深度学习中超线性增长对数梯度分布的采样问题，并提供了理论性能保证。
...
Method: 提出基于tamed Langevin dynamics的kTULA算法，并建立其非渐近收敛性理论。

Result: 在KL散度中取得最佳收敛速率$2-\overline{\epsilon}$，显著优于现有文献，并进一步改进了Wasserstein-2距离误差界。
...
### [449] [Estimating properties of a homogeneous bounded soil using machine learning models](https://arxiv.org/abs/2506.04256)
*Konstantinos Kalimeris,Leonidas Mindrinos,Nikolaos Pallikarakis*

Main category: physics.geo-ph

TL;DR: 利用机器学习模型从水分测量数据估计土壤性质，支持向量机和神经网络表现最佳。


<details>
  <summary>Details</summary>
Motivation: 解决从水分测量数据中准确估计土壤性质的问题，尤其是扩散率D和导水率K。

Method: 使用Fokas方法模拟数据，并比较多种机器学习模型在完整、噪声和有限数据条件下的表现。

Result: 扩散率D的预测比导水率K更准确，支持向量机和神经网络在鲁棒性和准确性上表现最优。

Conclusion: 支持向量机和神经网络是解决此类参数识别问题的有效工具。
...
### [450] [hdl2v: A Code Translation Dataset for Enhanced LLM Verilog Generation](https://arxiv.org/abs/2506.04544)
...
<details>
  <summary>Details</summary>
Motivation: 公开可用的Verilog代码远少于软件语言（如Python），限制了LLM在硬件代码生成领域的潜力。

Method: 通过翻译或编译VHDL、Chisel和PyMTL3生成Verilog代码，构建hdl2v数据集。

Result: hdl2v将32B参数模型的Verilog生成性能提升23%（pass@10），数据增强微调方法性能提升63%。

Conclusion: hdl2v数据集有效提升LLM性能，未来可进一步优化HDL-to-Verilog数据集特性。
...
### [451] [QiMeng: Fully Automated Hardware and Software Design for Processor Chip](https://arxiv.org/abs/2506.05007)
...
TL;DR: QiMeng是一个用于全自动处理器芯片硬件和软件设计的新系统，通过分层结构解决设计挑战。


<details>
  <summary>Details</summary>
Motivation: 传统处理器芯片设计面临物理限制、资源需求和生态系统多样化的挑战，需要自动化解决方案。

Method: QiMeng分为三层：底层构建领域特定的大规模处理器芯片模型（LPCM），中层开发硬件和软件设计代理，顶层应用验证系统。

Result: 部分组件已完成并成功应用于顶层应用，展示了显著优势。
...
### [452] [Laterally Excited Bulk Acoustic Wave (LBAW) X-Cut Lithium Niobate Resonators](https://arxiv.org/abs/2506.04433)
*Walter Gubinelli,Ryan Tetro,Pietro Simeoni,Luca Colombo,Matteo Rinaldi*

Main category: eess.SY

TL;DR: 论文展示了在X切铌酸锂上实现的横向激发体声波（LBAW）谐振器及其高阶谐波（LOBAW），通过嵌入压电薄膜的叉指电极，利用S0和SH0振动模式，实现了频率可调和多频扩展。


<details>
  <summary>Details</summary>
Motivation: 研究旨在开发一种新型谐振器架构，以支持5G/6G射频前端模块的多频扩展和性能提升。

Method: 采用嵌入压电薄膜的叉指电极结构，解耦色散关系与薄膜厚度，实现频率的微调。

Result: SH0模式在673 MHz和1.05 GHz下分别实现了437和53的品质因数（FoM）。

Conclusion: 该架构为未来5G/6G射频前端模块提供了多频扩展和性能优化的潜力。

Abstract: In this work, Laterally excited Bulk Acoustic Wave (LBAW) resonators on X-cut
...
modules, enabling on-chip multi-frequency scaling and improved performance.

</details>


### [453] [Distribution System State and Impedance Estimation Augmented with Carson's Equations](https://arxiv.org/abs/2506.04949)
*Marta Vanin,Frederik Geth,Rahmat Heidari,Dirk Van Hertem*

Main category: eess.SY

TL;DR: 本文提出了一种结合统计学习和领域知识（Carson方程）的方法，通过非线性数学优化从智能电表数据中学习电缆和线路的阻抗矩阵，显著提高了模型精度和计算效率。


<details>
  <summary>Details</summary>
Motivation: 由于配电网络中电缆和线路的阻抗通常未知或近似，导致基于物理的电力系统计算结果不准确，因此需要从时间序列数据中学习参数以改进阻抗模型。

Method: 结合统计学习概念和领域知识（Carson方程），通过非线性数学优化从智能电表数据中推导四线及以下系统的阻抗矩阵。

Result: 尽管缺乏相量测量且智能电表数据信噪比低，该方法仍能生成高质量的阻抗模型，显著提高了精度和计算时间。

Conclusion: 该方法为电力系统计算提供了更准确的阻抗模型，优于先前工作。

Abstract: The impedances of cables and lines used in (multi-conductor) distribution
...
### [454] [En Route Path-planning for Partially Occupied Vehicles in Ride-pooling Systems](https://arxiv.org/abs/2506.04968)
*Pengbo Zhu,Giancarlo Ferrari-Trecate,Nikolas Geroliminis*

Main category: eess.SY

TL;DR: 提出了一种新型路径规划算法，用于提升拼车服务的效率，动态响应乘客需求并优化旅行时间。


<details>
  <summary>Details</summary>
Motivation: 传统拼车服务路径规划方法基于预定义时间窗口，无法实时响应乘客需求，因此需要一种更灵活高效的算法。

Method: 采用整数线性规划（ILP）方法，动态考虑乘客需求并与其他车辆协调，实现实时路径规划。

Result: 仿真结果表明，该算法显著提升了服务质量。

Conclusion: 该算法在实时应用中具有高效性和实用性，能够显著改善拼车服务性能。

Abstract: Ride-pooling services, such as UberPool and Lyft Shared Saver, enable a
...
### [455] [Cloud-Based Interoperability in Residential Energy Systems](https://arxiv.org/abs/2506.05076)
*Darren Leniston,David Ryan,Ammar Malik,Jack Jackman,Terence O'Donnell*

Main category: eess.SY

TL;DR: 本文提出了一种可扩展的智能电网网关架构，实现了Modbus逆变器与IEEE 2030.5云控制系统的互操作性，利用Azure云服务和边缘计算设备支持动态配置和远程控制。


<details>
  <summary>Details</summary>
Motivation: 随着分布式能源资源（DERs）的普及，维持电网稳定性需要先进的监控和控制机制。

Method: 采用基于微服务的架构，结合Azure云服务和边缘计算网关设备，支持动态配置、遥测数据采集和远程控制。

Result: 成功将Fronius Primo逆变器的Modbus寄存器映射到IEEE 2030.5兼容的遥测和控制功能，并展示了实时VVC更新的效果。

Conclusion: 基于标准的虚拟化控制基础设施具有支持DER集成和主动电网参与的潜力，同时适应不断发展的智能电网架构。

Abstract: As distributed energy resources (DERs) such as solar PV, batteries and
...
### [456] [Towards provable probabilistic safety for scalable embodied AI systems](https://arxiv.org/abs/2506.05171)
...
TL;DR: 论文提出了一种可证明的概率安全方法，以解决具身AI系统在大规模部署中的安全性挑战，通过统计方法确保残余风险低于预设阈值。


<details>
  <summary>Details</summary>
Motivation: 具身AI系统在复杂环境中的安全性难以通过确定性方法全面验证，阻碍了其在安全关键领域的大规模应用。

Method: 引入可证明的概率安全概念，利用统计方法建立概率安全边界，而非穷举所有极端情况。

Result: 该方法为具身AI系统提供了可扩展的安全保障，允许持续优化安全性能。

Conclusion: 通过理论安全保证与实际部署的桥梁，为安全关键领域的大规模应用提供了可行路径。
...
### [457] [Diffusion Transformer-based Universal Dose Denoising for Pencil Beam Scanning Proton Therapy](https://arxiv.org/abs/2506.04467)
...
TL;DR: 提出了一种基于扩散变换器的去噪框架，用于快速生成高质量的质子治疗剂量分布图，解决了低统计量蒙特卡罗模拟的精度问题。


<details>
  <summary>Details</summary>
Motivation: 强度调制质子治疗（IMPT）对解剖结构变化敏感，需要频繁调整，而快速准确的剂量计算是关键。低统计量蒙特卡罗模拟虽快但精度不足，因此需要去噪技术。

Method: 开发了基于扩散变换器的去噪模型，使用80例头颈部患者的IMPT计划和CT图像生成噪声和高统计量剂量图，通过标准化和归一化处理数据，并采用多种损失函数训练模型。

Result: 模型在不同部位的剂量分布上表现出色，平均绝对误差低，3D Gamma通过率高，剂量体积直方图指标接近真实值。

Conclusion: 该去噪框架虽仅基于头颈部数据训练，但在多个疾病部位表现良好，具有广泛适用性。
...
### [458] [Deep learning image burst stacking to reconstruct high-resolution ground-based solar observations](https://arxiv.org/abs/2506.04781)
...
Method: 采用无配对图像到图像转换的深度学习模型，将100张短曝光图像实时重建为高质量图像，训练数据为退化的图像爆发和斑点重建参考。

Result: 该方法在感知质量上表现出更强的鲁棒性，尤其在斑点重建存在伪影时效果显著，且能充分利用图像爆发信息。

Conclusion: 该方法在提供完整图像爆发时实现了最佳重建效果，为太阳观测提供了高效解决方案。
...
### [459] [Exp4Fuse: A Rank Fusion Framework for Enhanced Sparse Retrieval using Large Language Model-based Query Expansion](https://arxiv.org/abs/2506.04760)
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM生成文档质量依赖复杂提示和高计算成本的问题，提升稀疏检索效率。

Method: 引入Exp4Fuse框架，结合原始查询和LLM增强查询的双重检索路径，采用改进的互惠排名融合方法。

Result: 在多个数据集上超越现有LLM查询扩展方法，结合高级稀疏检索器实现SOTA性能。

Conclusion: Exp4Fuse显著提升了稀疏检索的查询扩展效果，具有高效和优越性能。
...
### [460] [GOLFer: Smaller LM-Generated Documents Hallucination Filter & Combiner for Query Expansion in Information Retrieval](https://arxiv.org/abs/2506.04762)
*Lingyuan Liu,Mengxiang Zhang*
...
Conclusion: GOLFer证明了小型语言模型在查询扩展中的有效性，提供了一种成本更低且高效的选择。
...
### [461] [Towards Storage-Efficient Visual Document Retrieval: An Empirical Study on Reducing Patch-Level Embeddings](https://arxiv.org/abs/2506.04997)
*Yubo Ma,Jinsong Li,Yuhang Zang,Xiaobao Wu,Xiaoyi Dong,Pan Zhang,Yuhang Cao,Haodong Duan,Jiaqi Wang,Yixin Cao,Aixin Sun*

Main category: cs.IR

TL;DR: 论文研究了在视觉文档检索（VDR）中减少每页的补丁嵌入数量的方法，提出了两种策略：令牌修剪和令牌合并。研究发现随机修剪策略优于其他复杂方法，但修剪本身不适合VDR。令牌合并更有效，最终开发的Light-ColPali/ColQwen2在保持98.2%检索性能的同时，仅占用11.8%的内存。


<details>
  <summary>Details</summary>
Motivation: ColPali/ColQwen2在VDR中性能强大，但每页编码为多个补丁级嵌入导致内存占用过高。研究旨在减少嵌入数量，同时最小化性能损失。

Method: 评估了两种令牌减少策略：令牌修剪和令牌合并。修剪采用随机策略，合并则通过优化三个维度的组合策略。

Result: 随机修剪效果不佳，令牌合并更有效。Light-ColPali/ColQwen2在98.2%性能下仅用11.8%内存，2.8%内存时仍保持94.6%性能。

Conclusion: 令牌合并更适合VDR，Light-ColPali/ColQwen2为高效VDR研究提供了有价值的基准。
...
### [462] [Reason-to-Recommend: Using Interaction-of-Thought Reasoning to Enhance LLM Recommendation](https://arxiv.org/abs/2506.05069)
*Keyu Zhao,Fengli Xu,Yong Li*

Main category: cs.IR

TL;DR: R2Rec是一个基于LLM的推荐框架，通过交互链和渐进式提示策略模拟逐步推理，提升推荐性能和可解释性。
...
Method: 提出R2Rec框架，通过交互链和渐进式提示策略生成结构化推理，结合监督微调和强化学习训练。

Result: 在三个真实数据集上，R2Rec在HitRatio@1上平均提升10.48%，性能超过原始LLM 131.81%。

Conclusion: R2Rec通过显式推理链提升推荐性能和可解释性，解决了隐式反馈的监督问题。
...
### [463] [Energy-Optimized Scheduling for AIoT Workloads Using TOPSIS](https://arxiv.org/abs/2506.04902)
*Preethika Pradeep,Eyhab Al-Masri*

Main category: cs.DC

TL;DR: GreenPod是一个基于TOPSIS的调度器，优化了Kubernetes集群中的Pod放置，显著提升了能源效率。


<details>
  <summary>Details</summary>
Motivation: AIoT工作负载需要跨云边基础设施的高效能源调度，而Kubernetes默认调度器缺乏对异构环境的多标准优化。

Method: GreenPod采用TOPSIS方法，综合考虑执行时间、能耗、处理核心、内存可用性和资源平衡来优化Pod放置。

Result: 在异构Google Kubernetes集群测试中，GreenPod比默认调度器提升能源效率高达39.1%，尤其在能源权重方案下表现突出。

Conclusion: GreenPod在AIoT应用中有效平衡了可持续性和性能，中等复杂度工作负载的节能效果最佳。
...
### [464] [FlashDMoE: Fast Distributed MoE in a Single Kernel](https://arxiv.org/abs/2506.04667)
*Osayamen Jonathan Aimuyo,Byungsoo Oh,Rachee Singh*

Main category: cs.DC

TL;DR: FlashDMoE通过GPU内核融合和设备启动通信协议，显著提升了MoE模型的性能，降低了延迟并提高了GPU利用率。
...
Method: 开发了FlashDMoE，一种完全GPU驻留的MoE算子，将专家计算和GPU间通信融合为单个持久GPU内核，实现细粒度流水线。

Result: 在8-H100 GPU节点上，FlashDMoE实现了6倍延迟降低、5.7倍吞吐量提升、4倍弱扩展效率提升和9倍GPU利用率提升。

Conclusion: FlashDMoE证明了GPU内核与硬件协同设计是解锁大规模分布式机器学习性能上限的关键。
...
### [465] [The GAIN Model: A Nature-Inspired Neural Network Framework Based on an Adaptation of the Izhikevich Model](https://arxiv.org/abs/2506.04247)
*Gage K. R. Hooper*
...
<details>
  <summary>Details</summary>
Motivation: 改进神经网络的生物合理性和动态性，以更接近生物神经元的行为。

Method: 使用网格结构促进神经元与邻近神经元交互，结合Izhikevich模型进行模拟。

Result: 实现了计算高效且生物准确的神经模拟，适用于神经网络开发和大规模仿真。

Conclusion: GAIN模型通过网格结构和Izhikevich模型的结合，提升了动态性和准确性，适用于神经科学领域的专业应用。
...
### [466] [Generalizable, real-time neural decoding with hybrid state-space models](https://arxiv.org/abs/2506.05320)
*Avery Hee-Woon Ryoo,Nanda H. Krishna,Ximeng Mao,Mehdi Azabou,Eva L. Dyer,Matthew G. Perich,Guillaume Lajoie*

Main category: q-bio.NC

TL;DR: POSSM是一种混合架构，结合了交叉注意力模块和递归状态空间模型，实现了快速、实时的神经活动解码，并在跨物种任务中表现出色。
...
Method: 采用交叉注意力模块进行单脉冲标记化，结合递归状态空间模型（SSM）进行多数据集预训练。

Result: 在猴子和人类任务中，POSSM的准确性与最先进的Transformer相当，但推理速度快9倍。
...
### [467] [Knowledge-guided Contextual Gene Set Analysis Using Large Language Models](https://arxiv.org/abs/2506.04303)
...
TL;DR: cGSA是一种新型AI驱动框架，通过结合上下文感知通路优先化改进传统基因集分析（GSA），解决了传统方法忽略临床背景、结果冗余的问题。
...
### [468] [Prosthetics of the Indian State: The e-Shram Portal for Unorganized Workers in India](https://arxiv.org/abs/2503.05714)
*Rozin Hasin*

Main category: cs.CY

TL;DR: 论文批评了印度非正规经济中工人数字门户e-Shram的运作问题，包括可访问性和认知问题。


<details>
  <summary>Details</summary>
Motivation: 研究旨在揭示e-Shram数据库对非正规工人的实际效用及其局限性。

Method: 采用affordance理论分析e-Shram的运作方式。

Result: 发现e-Shram在可访问性和工人认知方面存在显著问题。

Conclusion: e-Shram需改进以更好地服务非正规工人。
...
### [469] [Turning to Online Forums for Legal Information: A Case Study of GDPR's Legitimate Interests](https://arxiv.org/abs/2506.04260)
*Lin Kyi,Cristiana Santos,Sushil Ammanaghatta Shivakumar,Franziska Roesner,Asia Biega*

Main category: cs.CY

TL;DR: 论文研究了从业者如何利用在线论坛获取GDPR合规法律指导，重点关注“合法利益”法律依据的应用，发现论坛信息虽法律上可靠但不完整，并提出改进建议。


<details>
  <summary>Details</summary>
Motivation: 在线论坛为从业者提供GDPR合规指导，但其信息的准确性和质量直接影响隐私与数据保护权利的实现，需深入研究从业者的决策过程和法律理解。

Method: 以GDPR的“合法利益”法律依据为案例，分析从业者在论坛中的讨论，评估论坛回答的法律可靠性。

Result: 研究发现“合法利益”应用复杂，论坛回答法律上可靠但不完整，影响GDPR实践。

Conclusion: 建议提升论坛回答的法律准确性和全面性，帮助从业者有效应用“合法利益”并遵守GDPR。
...
### [470] [Understanding and Meeting Practitioner Needs When Measuring Representational Harms Caused by LLM-Based Systems](https://arxiv.org/abs/2506.04482)
*Emma Harvey,Emily Sheng,Su Lin Blodgett,Alexandra Chouldechova,Jean Garcia-Gathright,Alexandra Olteanu,Hanna Wallach*

Main category: cs.CY

TL;DR: 研究探讨了现有NLP工具在测量大型语言模型（LLM）表征性危害时与从业者需求的脱节问题。


<details>
  <summary>Details</summary>
Motivation: 当前NLP社区提供了大量测量LLM表征性危害的工具，但这些工具是否真正满足从业者需求尚不明确。

Method: 通过半结构化访谈12位从业者，分析现有工具的可用性和实用性。

Result: 发现工具因测量不准确或与需求不匹配而无法使用，或由于实际和制度障碍未被采用。

Conclusion: 基于测量理论和实用测量，提出了改进建议以更好地满足从业者需求。
...
### [471] [Oversight Structures for Agentic AI in Public-Sector Organizations](https://arxiv.org/abs/2506.04836)
*Chris Schmitz,Jonathan Rystrøm,Jan Batzner*

Main category: cs.CY

TL;DR: 论文发现，引入代理性AI系统加剧了传统公共部门监督机制的挑战，提出五个关键治理维度，并通过混合方法评估现有监督结构的适应性。


<details>
  <summary>Details</summary>
Motivation: 研究代理性AI系统对公共部门监督机制的挑战，以改进现有治理框架。
...
### [472] [Intentionally Unintentional: GenAI Exceptionalism and the First Amendment](https://arxiv.org/abs/2506.05211)
...
Method: 通过法律先例和言论自由的核心目的分析AI输出的性质。

Result: AI输出不构成法律意义上的言论，扩展保护会阻碍政府监管，增加社会风险。
...
### [473] [On the Wasserstein Geodesic Principal Component Analysis of probability measures](https://arxiv.org/abs/2506.04480)
*Nina Vesseron,Elsa Cazelles,Alice Le Brigant,Thierry Klein*

Main category: stat.ML

TL;DR: 论文提出了一种基于Otto-Wasserstein几何的概率分布集合的测地主成分分析方法（GPCA），旨在识别最佳捕捉数据集变化模式的地测曲线。


<details>
  <summary>Details</summary>
Motivation: 研究目的是在概率测度空间中识别能够有效描述数据集变化模式的地测曲线。

Method: 首先针对高斯分布集合，展示了如何在可逆线性映射空间中计算；对于更一般的绝对连续概率测度，利用神经网络参数化Wasserstein空间中的地测曲线。

Result: 通过多种示例与经典切空间PCA进行比较，并在真实数据集上进行了验证。

Conclusion: 该方法在概率测度空间中有效捕捉了数据集的变化模式，并通过实验验证了其优越性。
...
### [474] [Regret-Optimal Q-Learning with Low Cost for Single-Agent and Federated Reinforcement Learning](https://arxiv.org/abs/2506.04626)
*Haochen Zhang,Zhong Zheng,Lingzhou Xue*

Main category: stat.ML

TL;DR: 论文提出两种新型无模型强化学习算法，旨在降低样本和通信成本，同时保持最优遗憾性能。
...
Method: 提出Q-EarlySettled-LowCost和FedQ-EarlySettled-LowCost算法，实现低样本成本和通信成本。

Result: 算法在样本成本、通信成本和遗憾性能上均优于现有方法。

Conclusion: 新算法在降低成本和保持性能方面具有显著优势，适用于实际应用。
...
### [475] [Distributional encoding for Gaussian process regression with qualitative inputs](https://arxiv.org/abs/2506.04813)
*Sébastien Da Veiga*
...
<details>
  <summary>Details</summary>
Motivation: 高斯过程回归在处理分类变量时面临预测和计算效率的挑战，传统目标编码方法仅使用目标变量的均值，信息利用不足。

Method: 提出分布编码（DE）方法，利用目标变量的全部分布信息，结合特征核（如最大均值差异和Wasserstein距离）改进GP。

Result: 实验验证表明，DE方法在合成和真实数据集上实现了最先进的预测性能。

Conclusion: DE方法为处理分类变量的GP回归提供了高效解决方案，并与离散和混合空间贝叶斯优化（BO）的最新进展互补。
...
### [476] [Learning Joint Interventional Effects from Single-Variable Interventions in Additive Models](https://arxiv.org/abs/2506.04945)
*Armin Kekić,Sergio Hernan Garrido Mejia,Bernhard Schölkopf*

Main category: stat.ML

TL;DR: 研究提出了一种仅使用观察数据和单变量干预数据来估计联合干预效果的方法，通过非线性加性机制实现联合效应的可识别性。


<details>
  <summary>Details</summary>
Motivation: 在多个领域中，联合干预的效果估计很重要，但获取联合干预数据困难，因此需要一种替代方法。

Method: 提出了一种可识别性结果，将因果效应分解为每个干预变量的混杂和非混杂贡献，并设计了实用估计器。

Result: 在合成数据上的实验表明，该方法性能接近直接使用联合干预数据训练的模型，优于纯观察估计器。

Conclusion: 仅使用观察和单变量干预数据即可有效估计联合干预效果，为实际应用提供了可行方案。
...
### [477] [Nonlinear Causal Discovery for Grouped Data](https://arxiv.org/abs/2506.05120)
...
<details>
  <summary>Details</summary>
Motivation: 在神经科学、心理学、社会科学和工业制造等领域，因果关系的单位通常是变量组而非单个标量变量，现有方法对此支持不足。

Method: 采用两步法：首先推断随机向量间的因果顺序，然后进行模型选择以确定最佳因果图。

Result: 在模拟实验中表现优异，并成功应用于真实世界的装配线数据。
...
### [478] [Causal Effect Identification in lvLiNGAM from Higher-Order Cumulants](https://arxiv.org/abs/2506.05202)
*Daniele Tramontano,Yaroslav Kivva,Saber Salehkaleybar Mathias Drton,Negar Kiyavash*

Main category: stat.ML

TL;DR: 论文研究了在潜在变量线性非高斯无环模型（lvLiNGAM）中利用高阶累积量识别因果效应，解决了存在潜在混杂时的两种挑战性场景：单代理变量和欠定工具变量情况。


<details>
  <summary>Details</summary>
Motivation: 解决潜在混杂下因果效应识别的挑战，特别是在单代理变量和工具变量不足的情况下。

Method: 利用高阶累积量证明因果效应的可识别性，并提供相应的估计方法。
...
### [479] [Admissibility of Completely Randomized Trials: A Large-Deviation Approach](https://arxiv.org/abs/2506.05329)
*Guido Imbens,Chao Qin,Stefan Wager*

Main category: stat.ML

TL;DR: 在最佳臂识别问题中，忽略自适应试验选项而选择非自适应试验是不可取的。研究发现，当至少有三个处理臂时，简单的自适应设计在统计效率上普遍且严格优于完全随机试验。
...
Method: 研究采用批处理臂淘汰设计，逐步淘汰表现不佳的处理臂，并分析其统计效率。

Result: 当至少有三个处理臂时，自适应设计在效率指数上严格优于完全随机试验。
...
### [480] [Robust Moment Identification for Nonlinear PDEs via a Neural ODE Approach](https://arxiv.org/abs/2506.05245)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法（如SINDy）需要密集采样数据且对噪声敏感，而神经ODE能够直接从稀疏数据中建模矩轨迹，更具鲁棒性。

Method: 利用神经ODE直接建模矩轨迹，并通过Stiefel流形优化实现数据驱动的坐标变换，以发现低维表示。

Result: 在非线性薛定谔方程和Fisher-KPP反应扩散系统中，该方法能准确恢复矩动力学，并在闭包未知时仍能有效近似。
...
### [481] [Bilevel Optimization for Improved Flexibility Aggregation Models of Electric Vehicle Fleets](https://arxiv.org/abs/2506.04843)
*Philipp Härtel,Michael von Bonin*

Main category: math.OC

TL;DR: 本文提出了一种双层优化方法，用于提升电动汽车车队的灵活性聚合，通过内外层优化减少调度偏差并提高利润。


<details>
  <summary>Details</summary>
Motivation: 电动汽车车队作为电力系统灵活性的重要来源，但其异构性导致灵活性潜力难以准确捕捉。

Method: 采用双层优化框架，外层最小化调度偏差，内层最大化利润，并引入时间尺度映射参数化聚合单元。

Result: 与简单聚合方法相比，充电功率的均方根误差减少了78%，灵活性表示更准确。

Conclusion: 该框架为未来研究提供了扩展基础，显著提升了电动汽车灵活性聚合的准确性。
...
### [482] [Regularization of non-overshooting quasi-continuous sliding mode control for chattering suppression at equilibrium](https://arxiv.org/abs/2506.05283)
*Michael Ruderman,Denis Efimov*

Main category: math.OC

TL;DR: 本文提出了一种正则化方案，用于抑制二阶系统中非过冲准连续滑模控制的固有抖振，并提供了ISS和iISS证明及数值结果支持。


<details>
  <summary>Details</summary>
Motivation: 解决二阶系统中非过冲准连续滑模控制在原点处因不连续性导致的抖振问题，以提升实际应用中的性能。

Method: 提出正则化方案，结合ISS（输入到状态稳定）和iISS（积分输入到状态稳定）理论进行分析。

Result: 通过数值结果验证了正则化方案的有效性，能够有效抑制抖振。

Conclusion: 该方案为二阶系统的非过冲准连续滑模控制提供了实用的改进方法，适用于实际应用。
...
### [483] [Was Residual Penalty and Neural Operators All We Needed for Solving Optimal Control Problems?](https://arxiv.org/abs/2506.04742)
*Oliver G. S. Lundqvist,Fabricio Oliveira*

Main category: math.OC

TL;DR: 论文提出了一种简化神经网络解决最优控制问题的方法，通过分离训练过程和优化问题，无需在训练中包含成本函数。
...
Method: 使用DeepONet等简单神经算子架构，结合无约束优化程序，通过微分方程残差的惩罚项和自动微分计算控制梯度。

Result: 在九个不同的最优控制问题上验证了方法的有效性，展示了准确且一致的性能。

Conclusion: 研究表明，简化架构和分离训练与优化过程可以高效解决多种最优控制问题。
...
### [484] [Non-linear Multi-objective Optimization with Probabilistic Branch and Bound](https://arxiv.org/abs/2506.04554)
...
Method: MOPBnB(so)在任一解处仅评估一次噪声函数，并利用邻近解估计目标函数。

Result: 算法在确定性问题上具有有限时间性能分析，且在随机问题上渐近收敛。数值结果表明其计算效率优于多复制变体和NSGA-II。

Conclusion: MOPBnB(so)在计算效率和性能上优于传统方法，适用于随机多目标优化问题。
...
### [485] [Unveiling Voices: A Co-Hashtag Analysis of TikTok Discourse on the 2023 Israel-Palestine Crisis](https://arxiv.org/abs/2501.07182)
*Rozin Hasin*

Main category: cs.SI

TL;DR: 研究探讨了TikTok上关于2023年巴勒斯坦危机的讨论，发现政治立场不同的用户通过关键词网络分析表现出明显的对立。


<details>
  <summary>Details</summary>
Motivation: 探讨TikTok作为社交媒体平台在政治议题中的影响，尤其是巴勒斯坦危机中的用户行为。

Method: 采用网络分析方法，对代表冲突的关键词进行分类，基于政治和意识形态立场编码。

Result: 发现TikTok上存在明显的政治对立，表现为活动宣传与宣传内容的竞争。

Conclusion: TikTok为政治对立提供了空间，活动宣传与宣传内容在其中激烈竞争。
...
### [486] [GARG-AML against Smurfing: A Scalable and Interpretable Graph-Based Framework for Anti-Money Laundering](https://arxiv.org/abs/2506.04292)
*Bruno Deprez,Bart Baesens,Tim Verdonck,Wouter Verbeke*

Main category: cs.SI

TL;DR: 论文提出了一种名为GARG-AML的新型图分析方法，通过量化每个节点在二阶交易网络中的结构风险，提供了一种可解释的单一指标，用于检测洗钱中的拆分交易（smurfing）行为。该方法在计算效率、检测能力和透明度之间取得了平衡，并通过实验验证了其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 洗钱行为占全球GDP的2%-5%，拆分交易（smurfing）作为一种常见的洗钱手段，其复杂性使得传统方法难以有效检测。现有方法多为黑箱模型，限制了其在商业中的应用。因此，需要一种透明且高效的方法来检测此类行为。

Method: GARG-AML通过分析每个节点在二阶交易网络中的结构特征，生成一个可解释的风险评分。该方法结合了树模型和邻居节点的评分，仅需邻接矩阵和基本网络特征即可实现高效检测。

Result: 实验表明，GARG-AML在大规模合成和开源网络上的表现优于现有方法，能够有效检测拆分交易行为。

Conclusion: GARG-AML展示了基础网络特性在欺诈检测中的潜力，为反洗钱工作流程提供了一种透明且高效的解决方案。
...
### [487] [A Framework Leveraging Large Language Models for Autonomous UAV Control in Flying Networks](https://arxiv.org/abs/2506.04404)
*Diana Nunes,Ricardo Amorim,Pedro Ribeiro,André Coelho,Rui Campos*

Main category: cs.NI

TL;DR: FLUC是一个模块化框架，将开源大语言模型（LLMs）与无人机（UAV）自动驾驶系统集成，实现飞行网络（FNs）中的自主控制。
...
Method: FLUC将高级自然语言命令翻译为可执行的无人机任务代码，并评估了三种开源LLMs（Qwen 2.5、Gemma 2和LLaMA 3.2）在代码生成和任务规划中的表现。

Result: Qwen 2.5在多步推理中表现最佳，Gemma 2在准确性和延迟之间取得平衡，LLaMA 3.2响应更快但逻辑一致性较低。案例研究验证了FLUC在实时任务驱动控制中的有效性。

Conclusion: FLUC能够有效解析结构化提示并自主执行特定领域逻辑，适用于实时任务驱动的无人机控制。
...
### [488] [BEAR: BGP Event Analysis and Reporting](https://arxiv.org/abs/2506.04514)
*Hanqing Li,Melania Fedeli,Vinay Kolar,Diego Klabjan*

Main category: cs.NI

TL;DR: BEAR是一个利用大型语言模型（LLM）自动生成BGP异常事件报告的框架，提高了检测结果的解释性和分析精度。


<details>
  <summary>Details</summary>
Motivation: BGP异常（如路由泄漏和劫持）威胁网络可靠性和安全性，现有方法依赖专家知识，BEAR旨在自动化解释和报告。

Method: BEAR通过多步推理将表格BGP数据转化为详细文本叙述，并利用LLM生成合成数据以弥补公开数据不足。

Result: 在真实和合成数据集上，BEAR达到100%准确率，优于其他基线方法。
...
### [489] [Intelligent Channel Allocation for IEEE 802.11be Multi-Link Operation: When MAB Meets LLM](https://arxiv.org/abs/2506.04594)
...
<details>
  <summary>Details</summary>
Motivation: WiFi 7的MLO功能在密集网络环境中面临信道分配挑战，现有研究多关注静态网络性能优化，缺乏动态环境下的解决方案。

Method: 将问题建模为组合优化问题，采用多臂老虎机（MAB）框架进行在线学习，提出了BAI-MCTS算法，并进一步结合大型语言模型（LLM）提出LLM-BAI-MCTS。
...
### [490] [Towards Network Data Analytics in 5G Systems and Beyond](https://arxiv.org/abs/2506.04860)
*Marcos Lima Romero,Ricardo Suyama*

Main category: cs.NI

TL;DR: 论文探讨了移动网络运营商（MNOs）如何利用数据分析提升价值，并分析了3GPP的NWDAF功能在学术研究中的局限性和潜力。


<details>
  <summary>Details</summary>
Motivation: 移动网络运营商的数据利用不足，而OTT企业却主导市场价值。数据分析被视为提升服务价值和效率的关键。

Method: 分析了70多篇文献，识别趋势和差距，并提出了两个新的NWDAF应用案例。

Result: 研究发现学术研究集中于有限方法和案例，提出了扩展NWDAF应用的建议。

Conclusion: 通过探索NWDAF的潜力，可以促进其采用并实现数据货币化。
...
### [491] [TQml Simulator: Optimized Simulation of Quantum Machine Learning](https://arxiv.org/abs/2506.04891)
*Viacheslav Kuzmin,Basil Kyriacou,Mateusz Papierz,Mo Kordzanganeh,Alexey Melnikov*

Main category: quant-ph

TL;DR: 论文提出了一种针对量子机器学习中硬件高效电路的数值模拟方法，开发了名为TQml Simulator的模拟器，性能优于现有工具。


<details>
  <summary>Details</summary>
Motivation: 量子机器学习中硬件高效电路的模拟速度对研究至关重要，现有方法效率不足。

Method: 通过数值基准测试，结合通用和门特定技术，开发了TQml Simulator，针对不同电路层选择最优模拟方法。

Result: TQml Simulator在多种电路和硬件上表现优异，性能提升2至100倍。

Conclusion: 针对电路层选择最优模拟方法可显著提升量子机器学习算法的模拟效率。
...
### [492] [Trustworthiness Preservation by Copies of Machine Learning Systems](https://arxiv.org/abs/2506.05203)
*Leonardo Ceragioli,Giuseppe Primiero*

Main category: cs.LO

TL;DR: 论文提出了一种计算工具，用于验证从已知行为的原始系统复制的复杂系统的可信赖性，定义了四种不同的可信赖性概念。


<details>
  <summary>Details</summary>
Motivation: 在负责任AI的背景下，验证复制的系统是否保持可信赖性是一个重要但未被充分研究的问题。

Method: 引入了一种计算模型，用于建模和验证数据和概率复杂查询，并定义了四种可信赖性概念。

Result: 研究了这些可信赖性概念之间的关系及其在逻辑操作下的组合。

Conclusion: 提供了一种计算工具，用于检查复制系统的可信赖性，确保其行为与原始系统一致。
...
### [493] [User Altruism in Recommendation Systems](https://arxiv.org/abs/2506.04525)
*Ekaterina Fedorova,Madeline Kitch,Chara Podimata*

Main category: cs.GT

TL;DR: 用户通过策略性互动影响推荐系统（RecSys），形成集体行为（用户利他主义），提升社会福利和系统效用。


<details>
  <summary>Details</summary>
Motivation: 研究用户如何通过策略性互动（如集体“助推”被算法压制的内容）影响推荐系统，以及这种行为对用户和系统的影响。

Method: 建立用户与推荐系统的博弈模型，分析用户真实偏好报告与利他策略下的社会福利差异，并提出有效利他策略算法。

Result: 理论分析表明，利他策略能严格提升社会福利，且对推荐系统效用也有改善；实证结果支持理论结论。
...
### [494] [MVP-Shapley: Feature-based Modeling for Evaluating the Most Valuable Player in Basketball](https://arxiv.org/abs/2506.04602)
*Haifeng Sun,Yu Xiong,Runze Wu,Kai Wang,Lan Zhang,Changjie Fan,Shaojie Tang,Xiang-Yang Li*

Main category: cs.GT

TL;DR: 该论文提出了一种基于Shapley值的MVP评估框架（\oursys），用于解决电子竞技和多人游戏中MVP评估的挑战，并通过NBA和Dunk City Dynasty数据集验证了其有效性。
...
Method: 提出\oursys框架，包括特征处理、胜负模型训练、Shapley值分配和基于贡献的MVP排名，并通过因果视角优化算法以匹配专家投票结果。

Result: 在NBA和Dunk City Dynasty数据集上验证了方法的有效性，并实现了工业界的在线部署。

Conclusion: \oursys框架提供了一种可解释且实用的MVP评估方法，适用于电子竞技和多人游戏场景。
...
### [495] [A Multi-Dataset Evaluation of Models for Automated Vulnerability Repair](https://arxiv.org/abs/2506.04987)
*Zanis Ali Khan,Aayush Garg,Qiang Tang*

Main category: cs.SE

TL;DR: 研究探讨了预训练语言模型CodeBERT和CodeT5在自动漏洞修复中的表现，发现CodeBERT在稀疏上下文表现更好，而CodeT5在复杂漏洞模式上更优，但模型在未知数据上泛化能力有限。


<details>
  <summary>Details</summary>
Motivation: 软件漏洞修复是安全关键任务，但现有自动程序修复技术对此研究不足，需探索预训练模型在漏洞修复中的潜力。

Method: 使用CodeBERT和CodeT5在六个数据集和四种语言上进行漏洞修复测试，评估其准确性和泛化能力。

Result: CodeBERT在稀疏上下文表现更好，CodeT5在复杂漏洞模式和扩展性上更优，但模型对未知数据泛化能力有限。

Conclusion: 研究为自动漏洞修复提供了基准，并指出泛化能力的不足，为实际安全应用提供了改进方向。
...
### [496] [Leveraging Reward Models for Guiding Code Review Comment Generation](https://arxiv.org/abs/2506.04464)
...
<details>
  <summary>Details</summary>
Motivation: 代码审查耗时且受主观因素影响，自动化技术（尤其是深度学习）可提升效率。

Method: CoRAL采用强化学习框架，通过语义和实用性奖励机制生成评论。

Result: 定量和定性分析显示CoRAL生成的评论优于基线方法。
...
### [497] [Interpretable LLMs for Credit Risk: A Systematic Review and Taxonomy](https://arxiv.org/abs/2506.04290)
*Muhammed Golec,Maha AlabdulJalil*

Main category: q-fin.RM

TL;DR: 本文系统回顾了基于大语言模型（LLM）的信用风险评估方法，提出了分类法，涵盖模型架构、数据类型、可解释性机制和应用领域，并指出未来研究趋势和空白。


<details>
  <summary>Details</summary>
Motivation: 近年来大语言模型的发展使其能够通过分析金融文本来评估信用风险，但缺乏系统性综述和分类法。本文旨在填补这一空白。

Method: 通过PRISMA研究策略筛选2020-2025年的60篇相关论文，分析模型架构、数据类型、可解释性机制和应用领域。

Result: 提出了基于LLM的信用风险评估分类法，涵盖四大主题，并总结了未来研究趋势和空白。

Conclusion: 本文为AI和金融领域的研究者提供了系统性参考，并指出了未来研究方向。
...
### [498] [Optical Physics-Based Generative Models](https://arxiv.org/abs/2506.04357)
*Amirreza Ahmadnejad,Somayyeh Koohi*

Main category: physics.optics

TL;DR: 该论文建立了一个连接光学物理方程与生成模型的数学框架，展示了光传播动力学如何启发强大的人工智能方法。通过分析六种光学方程，非线性模型在参数减少、模式分离和稳定性方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 探索光学物理方程与生成模型之间的联系，利用光的自组织特性提升人工智能方法的效率和性能。

Method: 分析六种光学方程（线性和非线性），比较其性能，并通过实验验证非线性模型的优越性。

Result: 非线性模型在参数减少（40-60%）、FID分数（0.0089 vs 1.0909）和训练效率（内存减少40-60%，时间缩短30-50%）方面显著优于线性模型。

Conclusion: 该框架揭示了物理自组织与人工智能的深层联系，为高效光学计算提供了新途径。
...
### [499] [A Comprehensive Survey on Bio-Inspired Algorithms: Taxonomy, Applications, and Future Directions](https://arxiv.org/abs/2506.04238)
...
<details>
  <summary>Details</summary>
Motivation: 生物启发算法通过模拟自然过程解决复杂优化问题，但其分类、应用和挑战尚未系统总结，本文旨在填补这一空白。

Method: 将BIAs分为八类（如进化算法、群体智能等），分析其核心原理、优缺点，并举例说明在机器学习等领域的应用。

Result: 总结了BIAs的最新进展（如混合策略、参数调优），并指出可扩展性、收敛性等开放性问题。

Conclusion: 本文为生物启发计算领域提供了基础性资源，并指明了未来研究方向。
...
### [500] [Experience Paper: Scaling WiFi Sensing to Millions of Commodity Devices for Ubiquitous Home Monitoring](https://arxiv.org/abs/2506.04322)
*Guozhen Zhu,Yuqian Hu,Chenshu Wu,Wei-Hsiang Wang,Beibei Wang,K. J. Ray Liu*

Main category: eess.SP

TL;DR: 论文提出了一种基于WiFi的家庭监控系统，解决了实际部署中的四大挑战，并通过大规模验证展示了其高准确性和低误报率。


<details>
  <summary>Details</summary>
Motivation: 传统家庭监控方案（如摄像头和传感器）存在覆盖范围有限和隐私问题，而WiFi感知技术利用现有无线基础设施，提供了更广泛且低侵入的解决方案。

Method: 开发并部署了一个大规模WiFi感知系统，通过优化算法减少误报、处理硬件异构性、区分多用户环境中的运动，并在边缘设备上实现高效实时处理。

Result: 系统在真实家庭环境中达到92.61%的准确率，将非人类运动误报率从63.1%降至8.4%，并减少99.72%的CSI传输开销。

Conclusion: 该研究为WiFi感知技术的实际应用提供了可行方案，并展示了其在家庭监控及其他领域的通用潜力。
...
### [501] [ChemReservoir -- An Open-Source Framework for Chemically-Inspired Reservoir Computing](https://arxiv.org/abs/2506.04249)
*Mehmet Aziz Yirik,Jakob Lykke Andersen,Rolf Fagerberg,Daniel Merkle*

Main category: cs.CE

TL;DR: ChemReservoir是一个开源框架，用于化学启发的储层计算，解决了先前DNA化学工具的限制。


<details>
  <summary>Details</summary>
Motivation: 先前基于DNA化学的储层计算工具维护不足且通用性有限，需要开源且通用的解决方案。

Method: 开发了ChemReservoir框架，支持构建和分析化学启发的储层，并增强测试、评估和可重复性。

Result: 工具在多种储层拓扑结构中表现稳定，适用于记忆容量任务。

Conclusion: ChemReservoir为化学启发的储层计算提供了通用且可靠的解决方案。
...
### [502] [Can we reconstruct a dysarthric voice with the large speech model Parler TTS?](https://arxiv.org/abs/2506.04397)
*Ariadna Sanchez,Simon King*

Main category: eess.AS

TL;DR: 论文研究了利用大型语音模型Parler TTS重建构音障碍患者发病前的声音，以作为沟通辅助工具。尽管模型能生成可理解的语音，但在控制清晰度和保持说话者身份一致性方面存在挑战。


<details>
  <summary>Details</summary>
Motivation: 构音障碍患者沟通困难，个性化文本转语音技术是一种潜在的解决方案。通过重建患者发病前的声音，可以帮助他们恢复沟通能力。

Method: 使用Parler TTS模型，通过标注的数据集进行微调，尝试生成构音障碍患者发病前的语音。

Result: 模型能够从挑战性数据中学习生成语音，但在控制清晰度和说话者身份一致性方面表现不佳。

Conclusion: 未来研究需要改进模型的可控性，以更好地完成语音重建任务。
...
### [503] [Towards Efficient Speech-Text Jointly Decoding within One Speech Language Model](https://arxiv.org/abs/2506.04518)
*Haibin Wu,Yuxuan Hu,Ruchao Fan,Xiaofei Wang,Kenichi Kumatani,Bo Ren,Jianwei Yu,Heng Lu,Lijuan Wang,Yao Qian,Jinyu Li*

Main category: eess.AS

TL;DR: 本文比较了联合语音文本解码策略，提出了一种新的早期停止交错（ESI）模式，显著加速解码并提升性能。


<details>
  <summary>Details</summary>
Motivation: 研究联合语音文本解码策略对性能、效率和对齐质量的影响，优化语音语言模型的表现。

Method: 系统比较交错和平行生成范式，提出ESI模式，并使用高质量QA数据集提升性能。

Result: 交错方法对齐效果最佳但推理慢，ESI模式显著加速解码且性能略优。

Conclusion: ESI模式是联合语音文本解码的高效解决方案，结合高质量数据可进一步提升语音QA性能。
...
### [504] [EMO-Debias: Benchmarking Gender Debiasing Techniques in Multi-Label Speech Emotion Recognition](https://arxiv.org/abs/2506.04652)
*Yi-Cheng Lin,Huang-Cheng Chou,Yu-Hsuan Li Liang,Hung-yi Lee*

Main category: eess.AS

TL;DR: 论文研究了语音情感识别（SER）中的性别偏见问题，比较了13种去偏见方法在多标签SER中的效果，提出了EMO-Debias框架，并分析了公平性与准确性的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有去偏见方法在多标签SER中的效果和鲁棒性尚未充分研究，因此需要系统比较不同方法并量化其表现。

Method: 提出了EMO-Debias框架，比较了13种去偏见方法，包括预处理、正则化、对抗学习等，并在性别不平衡条件下评估了WavLM和XLSR表示的效果。

Result: 实验量化了公平性与准确性的权衡，发现某些方法能有效减少性别性能差距而不影响整体性能。

Conclusion: 研究为选择去偏见策略提供了实用建议，并强调了数据集分布对方法效果的影响。
...
### [505] [Solving engineering eigenvalue problems with neural networks using the Rayleigh quotient](https://arxiv.org/abs/2506.04375)
...
Method: 使用神经网络离散化特征函数，结合Rayleigh商和Gram-Schmidt正交化方法，计算特征值和特征函数。

Result: 该方法适用于不规则域上的调和函数、参数和非线性特征问题以及高维特征分析，展示了工程力学中的优势。

Conclusion: 神经网络离散化结合Rayleigh商和Gram-Schmidt方法，为连续特征值问题提供了简单且鲁棒的解决方案。
...
### [506] [Can Artificial Intelligence Trade the Stock Market?](https://arxiv.org/abs/2506.04658)
*Jędrzej Maskiewicz,Paweł Sakowski*

Main category: q-fin.TR

TL;DR: 论文探讨了深度强化学习（DRL）在股票交易中的应用，比较了DDQN和PPO算法与Buy and Hold基准的表现，结果显示DRL在风险调整收益上优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 研究DRL在金融市场中的实际应用潜力，特别是在风险管理和收益优化方面。

Method: 使用DDQN和PPO算法，在多种资产（货币对、S&P 500指数、比特币）的日数据上进行测试。

Result: DRL在交易中表现优异，能通过策略性避开不利交易条件来管理风险，风险调整收益显著优于传统监督学习方法。

Conclusion: DRL在股票交易中具有显著优势，尤其在风险管理和收益优化方面，为金融市场提供了新的工具。
...
### [507] [Deep Learning-Based Breast Cancer Detection in Mammography: A Multi-Center Validation Study in Thai Population](https://arxiv.org/abs/2506.03177)
...
TL;DR: 该研究提出了一种基于改进EfficientNetV2架构的深度学习系统，用于乳腺X光检查中的乳腺癌检测，并在多个数据集上验证了其高性能和临床实用性。


<details>
  <summary>Details</summary>
Motivation: 提高乳腺癌筛查的准确性和效率，通过深度学习辅助放射科医生进行乳腺X光检查。

Method: 使用改进的EfficientNetV2架构，结合增强的注意力机制，训练于泰国医疗中心的乳腺X光数据，并在三个不同数据集上验证。

Result: 模型在癌症检测中AUROC分别为0.89、0.96和0.94；在病灶定位和临床验证中表现优异，与放射科医生的一致性高。
...
### [508] [Gradient Inversion Attacks on Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2506.04453)
*Hasin Us Sami,Swapneel Sen,Amit K. Roy-Chowdhury,Srikanth V. Krishnamurthy,Basak Guler*

Main category: eess.IV

TL;DR: 论文研究了联邦学习中参数高效微调（PEFT）的隐私风险，展示了攻击者如何通过恶意设计的预训练模型和适配器模块，利用梯度反演攻击重构用户数据。


<details>
  <summary>Details</summary>
Motivation: 探讨如何在联邦学习中同时提升隐私和训练效率，同时揭示PEFT机制中潜在的隐私漏洞。
...
### [509] [A Poisson-Guided Decomposition Network for Extreme Low-Light Image Enhancement](https://arxiv.org/abs/2506.04470)
*Isha Rao,Sanjay Ghosh*

Main category: eess.IV

TL;DR: 该论文提出了一种轻量级深度学习方法，用于极端低光条件下受泊松噪声影响的图像去噪和增强。


<details>
  <summary>Details</summary>
Motivation: 传统的高斯噪声假设在低光成像中不适用，泊松噪声更符合实际情况，因此需要一种有效的方法来处理信号依赖的噪声。

Method: 结合Retinex分解与泊松去噪，设计了一个统一的编码器-解码器网络，通过泊松去噪损失函数同时增强光照和抑制噪声。

Result: 实验表明，该方法显著提升了低光条件下的可见性和亮度，同时保持了图像结构和颜色一致性。

Conclusion: 该方法在低光图像增强中表现出高效性和实用性，无需先验知识即可实现有效的分解和去噪。
...
### [510] [DACN: Dual-Attention Convolutional Network for Hyperspectral Image Super-Resolution](https://arxiv.org/abs/2506.05041)
*Usman Muhammad,Jorma Laaksonen*

Main category: eess.IV

TL;DR: DACN是一种双注意力卷积网络，用于解决高光谱图像超分辨率任务中CNN的局限性，通过多注意力机制和优化的损失函数提升性能。
...
Method: DACN结合增强卷积和多头注意力，捕捉局部和全局特征依赖；通过通道和空间维度的注意力图优化聚焦；提出L2正则化和空间-光谱梯度损失的组合损失函数。

Result: 在两个高光谱数据集上的实验表明，多头注意力和通道注意力的组合优于单独使用任一机制。

Conclusion: DACN通过双注意力机制和优化损失函数，有效提升了高光谱图像超分辨率的性能。
...
### [511] [PixCell: A generative foundation model for digital histopathology images](https://arxiv.org/abs/2506.05127)
...
Conclusion: PixCell为计算病理学提供了强大的工具，加速研究并降低监管障碍。
...
### [512] [DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling](https://arxiv.org/abs/2506.05297)
*Hangyu Ji*

Main category: eess.IV

TL;DR: DM-SegNet是一种双Mamba架构，通过方向性状态转换和解剖感知分层解码，解决了医学图像分割中全局上下文建模与空间拓扑保持的兼容性问题。


<details>
  <summary>Details</summary>
Motivation: 现有医学状态空间模型（SSMs）存在编码器-解码器不兼容问题，1D序列扁平化破坏空间结构，而传统解码器无法利用Mamba的状态传播能力。

Method: 提出DM-SegNet，包含四方向空间Mamba模块、门控空间卷积层和Mamba驱动的解码框架，实现双向状态同步。

Result: 在Synapse和BraTS2023数据集上分别达到85.44%和90.22%的DSC，表现优异。

Conclusion: DM-SegNet通过创新设计解决了医学图像分割中的关键问题，显著提升了性能。
...
### [513] [Memory-Driven Bounded Confidence Opinion Dynamics: A Hegselmann-Krause Model Based on Fractional-Order Methods](https://arxiv.org/abs/2506.04701)
*Meiru Jiang,Wei Su,Guojian Ren,Yongguang Yu*

Main category: physics.soc-ph

TL;DR: 提出了一种新的分数阶有界置信度意见动态模型，用于描述系统状态中的记忆效应，结合Hegselmann-Krause框架和分数阶差分，研究了收敛和共识等基本性质。


<details>
  <summary>Details</summary>
Motivation: 记忆效应在社交互动和决策过程中至关重要，现有模型未能充分捕捉历史信息的持续影响。

Method: 基于Hegselmann-Krause框架和分数阶差分，建立了一个综合模型。

Result: 模型不仅保持了良好的收敛和共识特性，还解决了有界意见单调性等限制，更真实地模拟了现实场景中的意见演化。

Conclusion: 研究为理解意见形成和演化提供了新的理论和方法支持，具有理论和实践意义。
...
### [514] [What does making money have to do with crime?: A dive into the National Crime Victimization survey](https://arxiv.org/abs/2506.04240)
*Sydney Anuyah*

Main category: physics.soc-ph

TL;DR: 利用1992至2022年的国家犯罪受害调查，研究收入、教育、就业和人口因素如何影响犯罪类型（暴力与财产犯罪）。
...
Result: 高收入和学历降低暴力犯罪风险；男性、年轻人和少数族裔暴力犯罪风险更高。郊区模型预测效果最佳（准确率0.607，F1 0.590）。

Conclusion: 需针对性干预，如城市教育投资、农村经济支持和人口导向的预防策略。
...
### [515] [SSIMBaD: Sigma Scaling with SSIM-Guided Balanced Diffusion for AnimeFace Colorization](https://arxiv.org/abs/2506.04283)
*Junpyo Seo,Hanbin Koo,Jieun Yook,Byung-Ro Moon*

Main category: cs.GR

TL;DR: 提出了一种基于扩散模型的动漫风格面部线稿自动上色方法，通过SSIMBaD技术实现更平衡和忠实重建。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖预定义的噪声调度，可能导致感知一致性不足，因此需要一种更有效的方法。

Method: 采用连续时间扩散模型，引入SSIMBaD技术，通过sigma空间变换对齐感知退化。

Result: 在大规模动漫面部数据集上，该方法在像素精度和感知质量上均优于现有模型。

Conclusion: SSIMBaD框架在保持结构保真度的同时，能有效传递风格属性，适用于多样化风格。
...
### [516] [Handle-based Mesh Deformation Guided By Vision Language Model](https://arxiv.org/abs/2506.04562)
*Xingpeng Sun,Shiyang Jia,Zherong Pan,Kui Wu,Aniket Bera*

Main category: cs.GR

TL;DR: 提出一种无需训练、基于手柄的网格变形方法，利用视觉语言模型（VLM）通过提示工程实现高质量变形。
...
### [517] [VoxDet: Rethinking 3D Semantic Occupancy Prediction as Dense Object Detection](https://arxiv.org/abs/2506.04623)
*Wuyang Li,Zhu Yu,Alexandre Alahi*

Main category: cs.GR

TL;DR: 论文提出了一种实例为中心的3D语义占用预测方法VoxDet，通过将体素级分类任务解耦为偏移回归和语义预测，显著提升了实例级完整性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将3D语义占用预测视为密集分割任务，忽视了实例级的判别性，导致实例不完整和相邻模糊。论文发现体素级标签隐含实例级信息，并利用这一观察提出新方法。
...
Conclusion: VoxDet通过实例中心化设计，显著提升了3D语义占用预测的性能和效率。
...
### [518] [A Fast Unsupervised Scheme for Polygonal Approximation](https://arxiv.org/abs/2506.04664)
*Bimal Kumar Ray*

Main category: cs.GR

TL;DR: 提出了一种快速且无监督的多边形近似闭合数字曲线的方案，速度快于现有方法，且在Rosin度量与美观性上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法在速度和美观性上存在不足，需一种更高效的方案。

Method: 方案分为三阶段：初始分割、迭代顶点插入与合并、顶点调整。

Result: 方案在Rosin度量和几何变换鲁棒性上表现良好。

Conclusion: 该方案高效且美观，适用于闭合数字曲线的近似。
